...
import uuid
import zipfile
from backend.mio_library import (LibraryError, MAX_BUNDLE, MAX_DOCUMENT, encode, decode, digest,
...
def validate_content(store, kind, doc):
    ...
    if kind in ('characters', 'scenes'):
        ...
        for entry in doc['entries']:
            ...
            if typ not in ('text', 'number', 'boolean', 'json', 'image') and not __import__('re').fullmatch(r'plugin:[a-z][a-z0-9_-]{0,63}:[a-z][a-z0-9_-]{0,63}',str(typ)):raise LibraryError('变量类型不受支持。')
...
def import_resource(store, body):
    ...
    if kind=='albums':
        from backend.mio_album_html import import_books
...
def validated_album(store,doc):
    from backend.mio_album_html import validate_books
