...
import sqlite3
import time
from backend.mio_jobs import Conflict
...
def same_image(a, b):
    ...
    if (
        ...
        or not a.startswith("/images/")
        or not b.startswith("/images/")
    ):
        return False
    name = a.rsplit("/", 1)[-1]
    return (
        bool(re.fullmatch(r"[a-f0-9]{64}\.(png|jpg|jpeg|webp|svg)", name))
        and b.rsplit("/", 1)[-1] == name
    )
...
def records(root, after=None):
    ...
    with closing(sqlite3.connect(path, timeout=30)) as db:
        if not db.execute(
            ...
            return []
        rows = db.execute(
            "SELECT seq,album,idx,revision,record FROM picture_edits"
            + (
...
def apply_record(book, record):
    ...
    if not 0 <= index < book.get("totalSteps", 0):
        return
    step = next(
        (s for s in book.setdefault("steps", []) if s.get("stepIndex") == index), None
    )
    if step is None:
    ...
    step.update(
        ...
        deletedImage=record.get("removed", False),
    ...
    if not book.get("inProgress"):
        book["status"] = (
            "complete"
            if book["generatedSteps"] == book.get("totalSteps")
            else "partial"
        )
...
def validate_recipe(recipe, image_check):
    ...
    if (
        not all(finite(crop[k], 0, 1) for k in crop)
        or crop["w"] <= 0
        or crop["h"] <= 0
        or crop["x"] + crop["w"] > 1.001
        or crop["y"] + crop["h"] > 1.001
    ):
        raise ValueError("裁剪范围超出原图。")
    if recipe.get("rotation", 0) not in (0, 90, 180, 270):
        raise ValueError("旋转角度不合法。")
    for key in ("brightness", "contrast", "saturation"):
        if not finite(recipe.get(key, 1), 0, 2):
            raise ValueError("调色参数不合法。")
    ...
    for layer in layers:
        ...
        if (
            not all(finite(layer.get(k), 0, 1) for k in ("x", "y", "w", "h"))
            or not layer["w"]
            or not layer["h"]
        ):
            raise ValueError("图层范围不合法。")
        if (
            not isinstance(layer.get("text", ""), str)
            or len(layer.get("text", "")) > 2000
        ):
            raise ValueError("图层文字最多 2000 字。")
        if not finite(layer.get("fontSize", 4), 1, 15):
            raise ValueError("字号不合法。")
        for color in ("color", "fill", "stroke"):
            if color in layer and not re.fullmatch(
                r"#[0-9a-fA-F]{6}", str(layer[color])
            ):
                raise ValueError("颜色不合法。")
        if layer.get("shape", "round") not in (
            "round",
            "ellipse",
            "thought",
            "shout",
            ...
            raise ValueError("气泡形状不合法。")
        if layer.get("tail", "left") not in ("left", "right", "none"):
            raise ValueError("气泡尾部不合法。")
        if layer.get("writingMode", "horizontal-tb") not in (
            "horizontal-tb",
            "vertical-rl",
        ):
            raise ValueError("文字排版方向不合法。")
        for key in ("tailX", "tailY"):
            if key in layer and not finite(layer[key], 0, 1):
                raise ValueError("气泡尖角坐标不合法。")
        if layer.get("id") is not None and (
            not isinstance(layer["id"], str) or len(layer["id"]) > 150
        ):
            raise ValueError("图层 ID 不合法。")
        if layer.get("asset"):
            image_check(layer["asset"])
...
def mutate(host, store, body):
    ...
    if (
        ...
        or action not in ("save", "remove", "restore")
    ):
    ...
    def check_image(url, editing=True):
        ...
        from backend.mio_foundation import image_dimensions
        ...
        if (
            ...
            and width
            and height
            and (width > 8192 or height > 8192 or width * height > 32000000)
        ):
    ...
    with store.lock, host.CONFIG_LOCK:
        ...
        book = next(
            (b for b in config.get("savedGalleries", []) if b.get("id") == album), None
        )
        if not book or not 0 <= index < book.get("totalSteps", 0):
            raise Conflict("画册或分镜已不存在，未覆盖任何图片。")
        current = next(
            (s for s in book.get("steps", []) if s.get("stepIndex") == index), {}
        )
        old = book.get("pictureEdits", {}).get(str(index), {})
        if body.get("expectedRevision") != old.get("revision", 0) or not same_image(
            body.get("expectedImage"), current.get("image", "")
        ):
            raise Conflict(
                "此图已被生成结果或另一页面更新。草稿仍保留，请重新打开当前图片后再编辑。"
            )
        ...
        if action == "save":
            ...
            record = {
                **old,
                "image": "",
                "removed": True,
                "previousImage": current.get("image", ""),
            }
        else:
            if not original:
                raise ValueError("没有可恢复的原图，请替换为本地图片。")
            width, height = (
                check_image(original, False)
        ...
        for key in ("seq", "albumId", "index", "revision"):
            record.pop(key, None)
        record.update(
            originalImage=original or record.get("sourceImage", ""),
        ...
        with store.connect() as db:
            ...
            if db.execute(
                "SELECT 1 FROM deleted_albums WHERE id=?", (album,)
            ).fetchone():
                raise Conflict("画册已删除。")
            db.execute(
                "INSERT OR REPLACE INTO picture_edits(album,idx,revision,record) VALUES(?,?,?,?)",
                (album, index, revision, json.dumps(record)),
            )
            seq = db.execute(
                "SELECT seq FROM picture_edits WHERE album=? AND idx=?", (album, index)
            ).fetchone()[0]
        result = {
            "seq": seq,
            "albumId": album,
