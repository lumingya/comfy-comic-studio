...
try{
 ...
 browser=await chromium.launch({args:['--no-sandbox']});let page=await browser.newPage();const errors=[];page.on('pageerror',e=>errors.push(e.message));await page.route(/^https:\/\//,r=>r.abort());await page.goto(base);await page.waitForFunction(()=>typeof rt!=='undefined'&&!rt.booting);
 ...
 page=await browser.newPage();page.on('pageerror',e=>errors.push(e.message));await page.route(/^https:\/\//,r=>r.abort());await page.goto(base);await page.waitForFunction(()=>typeof rt!=='undefined'&&!rt.booting);await page.waitForFunction(book=>bookBy(book)?.generatedSteps===3,snapshot.book);
 check('a completed queue does not turn an unloaded album into an empty body',await page.evaluate(book=>bookBy(book)._lazy&&bookBy(book).steps.length===0,snapshot.book));await page.evaluate(book=>openArtReader(book),snapshot.book);
 ...
 check('a browser can edit a different plan without overwriting the external storyboard',await page.evaluate(async()=>{selectedPlan().title='independent browser plan';save();if(!await savePythonWorkspace())return false;const config=await(await fetch('/api/config')).json();return config.templates.some(t=>t.id==='external_storyboard'&&t.title==='External storyboard')&&config.uiConfig.comfyStudio.creation.plans.some(p=>p.title==='independent browser plan')}));
