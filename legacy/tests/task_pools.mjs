...
try{
 ...
 browser=await chromium.launch({args:['--no-sandbox']});const page=await browser.newPage({viewport:{width:1400,height:1000}}),errors=[];page.on('pageerror',e=>errors.push(e.message));await page.route(/^https:\/\//,r=>r.abort());await page.goto(base);await page.waitForFunction(()=>typeof rt!=='undefined'&&!rt.booting);
 ...
 await page.evaluate(()=>{foundationRuntime.polling=true});pending.get('pool-a-1')();await until(()=>active.has('pool-a-4'));
 ...
 // Reproduce a task-only canonical save ACK arriving before the album result poll.
 await page.evaluate(async id=>{state.queue.find(q=>q.bookId===id).done=1;foundationRuntime.polling=false;await pollFoundationJobs()},jobs[0].book);
 await page.waitForFunction(id=>bookBy(id)?.generatedSteps===1,jobs[0].book);check('task-only ACK cannot hide the out-of-order image from album polling',await page.evaluate(id=>bookBy(id).steps.length===1&&bookBy(id).steps[0].stepIndex===1,jobs[0].book));
 ...
 await page.evaluate(()=>{createUI.tab='story';ui.frameIndex=0;render()});
 check('history versions are excluded from source selector',await page.evaluate(()=>[...$('#art-book-draft').options].every(o=>!o.value.startsWith('task:'))));
 await page.locator('[data-v3-frame="prompt"]').fill('source-only-edit');await page.evaluate(async()=>{flushEditor();if(!await savePythonWorkspace())throw Error('save failed')});
 check('source edits do not alter queued frozen frames',await page.evaluate(qid=>state.queue.find(q=>q.id===qid).frames[0].prompt==='pool-c-0',jobs[2].qid));
 // Dispatcher-level correction is explicit, not an accidental binding of the source editor.
 await page.evaluate(async qid=>{const q=state.queue.find(q=>q.id===qid),b=bookBy(q.bookId),f=clone(q.frames[0]);f.prompt='pool-c-updated';q.frames[0]=clone(f);b.sourceSnapshot.frames[0]=clone(f);delete f._resolvedImagePrompt;delete f._resolvedImageNegative;delete f._imageInputs;b.sourceSnapshot.liveInputs??={};b.sourceSnapshot.liveInputs[0]={input:foundationFrameInput(f,q.rowSnapshot,b.id,0),savedAt:Date.now()};if(!await savePythonWorkspace())throw Error('save failed')},jobs[2].qid);
 check('source remains separate from an explicit task input correction',await page.evaluate(()=>currentTemplate().frames[0].prompt==='source-only-edit'));
 ...
 await page.goto(base);await page.waitForFunction(()=>typeof rt!=='undefined'&&!rt.booting);await page.evaluate(async()=>{document.querySelectorAll('dialog[open]').forEach(d=>d.close());await pollFoundationJobs()});
