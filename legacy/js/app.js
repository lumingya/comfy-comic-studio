...
const icon=(n,c='')=>`<svg class="icon ${c}" viewBox="0 0 24 24" aria-hidden="true">${(typeof MioIcons!=='undefined'&&MioIcons.resolve(n))||paths[n]||paths.grid}</svg>`,btn=(l,i,a,x='',c='')=>`<button type="button" class="btn ${c}" data-act="${a}" ${x}>${i?icon(fileOperationIcon(a,i)):''}${l}</button>`,ibtn=(i,a,l,x='')=>`<button type="button" class="ibtn" data-act="${a}" title="${esc(l)}" aria-label="${esc(l)}" ${x}>${icon(fileOperationIcon(a,i))}</button>`,opt=(v,l,c)=>`<option value="${esc(v)}" ${String(v)===String(c)?'selected':''}>${esc(l)}</option>`,input=(n,v,t='text',a='')=>`<input name="${n}" type="${t}" value="${esc(v)}" ${a}>`,field=(l,c,h='')=>`<div class="field"><label class="label">${l}</label>${c.replace(/<(input|textarea|select)\b([^>]*)>/g,(tag,name,attrs)=>/\b(?:aria-label|aria-labelledby)=/.test(attrs)?tag:`<${name} aria-label="${esc(String(l).replace(/<[^>]*>/g,''))}"${attrs}>`)}${h?`<div class="help">${h}</div>`:''}</div>`;
...
const studioActions={
  ...
  'et-delete':()=>withDeletionUndo('模板已删除',async()=>{const t=currentEditorTemplate();if(t.builtin)throw Error('内置模板不可删除，请保留作为恢复基底。');if(!await confirmAction('删除这份自定义模板？','删除后 10 秒内可以撤销，之后也能在「设置 → 数据与备份 → 回收站」找回。','删除模板'))return;state.exportTemplates=state.exportTemplates.filter(x=>x.id!==t.id);for(const p of state.installedPackages)if(p.type==='exports')p.assetIds=p.assetIds.filter(id=>id!==t.id);state.installedPackages=state.installedPackages.filter(p=>p.type!=='exports'||p.assetIds.length);ensureStudioState();studioUI.editor=clone(state.exportTemplates[0]);studioUI.editorDirty=false;save(true);renderTemplateStudio()}),
...
const actionHelp={
  ...
  'project-toggle':'切换当前画册集，或在同一个菜单中创建新画册集。',
  'new-project':'创建独立画册集。连接目录后会自动建立对应画册集文件夹。',
  'project-rename':'修改画册集显示名称。已有文件夹保持稳定，避免破坏图片引用。',
...
sendChat=async function(){if(rt.chatBusy)return;const chat=currentChat();if(!chat.templateId){const t=contextualTemplate();if(!t)throw Error('请先创建分镜，再为此模板开启对话。');chat.templateId=t.id;chat.projectId=t.projectId;chat.title=t.title.slice(0,22)+' · 对话 1'}if(!templateBy(chat.templateId))throw Error('此对话绑定的模板已被移除，请选择新的目标模板。');chat.draft='';return detailCore.sendChat()};
...
const detailActions={
  ...
  'new-project':()=>{closeProjectPopover();flushEditor();textModal('新建画册集','画册集名称','',name=>{const p={id:uid('project'),title:name,createdAt:Date.now()};state.projects.push(p);closeModal();changeProject(p.id);save();toast(disk.root?'画册集已创建，独立文件夹将随自动保存建立。':'画册集已创建，连接目录后会建立独立文件夹。')},'新画册集拥有独立的画册、模板、角色和图片目录。')},
  'project-rename':()=>{closeProjectPopover();const id=project().id;textModal('重命名画册集','画册集名称',project().title,name=>{const p=state.projects.find(x=>x.id===id);p.title=name;save();closeModal();render();toast('显示名称已修改，已落盘目录路径保持稳定。')})},
  ...
  'new-chat':()=>{if(rt.chatBusy)throw Error('请等待本次助手操作结束。');const t=assistantScope();if(!t)throw Error('请先选择目标分镜。');bindChatToTemplate(t.id,true);renderAssistant()},
...
validateState=function(s){
  ...
  if(s.creation){if(!Array.isArray(s.creation.plans)||!Array.isArray(s.creation.variableSets))throw Error('创作模型结构不完整。');for(const list of [s.creation.plans,s.creation.variableSets]){const ids=new Set();for(const item of list){if(!safeId(item.id)||ids.has(item.id)||typeof item.title!=='string'||!s.projects.some(p=>p.id===item.projectId))throw Error('创作资产 ID、名称或画册集归属不合法。');ids.add(item.id)}}
...
const v3Actions={
  ...
  'v3-plan-delete':()=>withDeletionUndo('计划已删除',async()=>{const p=selectedPlan();if(!await confirmAction('删除「'+p.title+'」计划？','删除后 10 秒内可以撤销，之后也能在「设置 → 数据与备份 → 回收站」找回。','删除计划'))return;state.creation.plans=state.creation.plans.filter(x=>x.id!==p.id);createUI.planId=null;save(true);render()}),
...
document.addEventListener('change',async e=>{const el=e.target;try{
  ...
  if(el.dataset.v3Binding){const b=state.settings.comfy.bindings.find(b=>b.id===el.dataset.id),key=el.dataset.v3Binding;if(!b)return;b[key]=el.type==='checkbox'?el.checked:el.value;if(key==='allowLink'&&el.checked){if(!await confirmAction('允许覆盖此字段原有的节点连线？','这会改变数据流向，可能导致工作流执行错误。请先预览完整 JSON。','明确允许')){b.allowLink=false;el.checked=false}}save();if(key!=='value'&&key!=='label')render()}
...
async function v3Diagnostics(){
  ...
  test('主导航不再包含我的画册集或独立引擎',()=>!primaryNavItems().some(x=>x[0]===2||x[0]===3));
...
document.addEventListener('change',event=>{const el=event.target;try{
  ...
  if(el.id==='shelf-select-all'&&typeof syncSelectionView!=='function'){for(const book of getShelfBooks())el.checked?ui.selected.add(book.id):ui.selected.delete(book.id);refreshGallery()}
}catch(error){toast(error.message,'error')}});
