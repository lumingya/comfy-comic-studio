...
> **这是旧版（3.2，功能已冻结），只作参考。** 新版在仓库根目录的 `server/` 与 `web/`，见[仓库说明](../README.md)。旧版快照另有 Git 标签 `legacy-v3.2`。
...
在本目录（仓库里的 `legacy/`；发布包解压后的根目录）运行：
...
工作区默认位于 `data/`（仓库里是根目录的 `data/`，发布包里在程序旁边），也可通过 `MIO_DATA_DIR` 指定目录。更换程序前备份工作区，保留个人数据。
