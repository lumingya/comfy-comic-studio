...
class ComfyClient:
    ...
    def upload_image(self, data: bytes, filename: str, subfolder: str = "mio") -> str:
        ...
        parts.append(f"--{boundary}--\r\n".encode())
        raw = self._request("POST", "/upload/image", b"".join(parts),
    ...
    # ------------------------------------------------------------------- run
    def run(self, graph: dict, output_nodes=None, on_event=None, timeout: float = 900.0) -> RunResult:
        ...
        for node_id, out in (entry.get("outputs") or {}).items():
            ...
            for img in out.get("images") or []:
                ref = OutputImage(node_id, img["filename"], img.get("subfolder", ""), img.get("type", "output"))
                ref.data = self.view(ref.filename, ref.subfolder, ref.type)
                images.append(ref)
        return RunResult(prompt_id, images, time.monotonic() - started, node_seconds,
                         cached, previews, texts, live=ws is not None)
    ...
    def _follow(self, ws, prompt_id, started, timeout, node_seconds, cached, emit) -> int:
        ...
        while True:
            ...
            if kind == "execution_cached":
                ...
                emit({"type": "progress", "node": data.get("node"), "value": data.get("value"), "max": data.get("max")})
...
def parse_preview(message: bytes) -> dict:
    ...
    if event == PREVIEW_IMAGE:
        return {"type": "preview", "format": IMAGE_TYPES.get(image_type, "unknown"), "data": message[8:]}
    if event == PREVIEW_IMAGE_WITH_METADATA:
        (meta_len,) = struct.unpack(">I", message[4:8])
        meta = json.loads(message[8:8 + meta_len] or b"{}")
        return {"type": "preview", "format": (meta.get("image_type") or "image/png").split("/")[-1],
                "data": message[8 + meta_len:], "meta": meta}
...
def describe_prompt_error(detail) -> str | None:
    ...
    if err.get("message"):
        lines.append(str(err["message"]) + (f"：{err.get('details')}" if err.get("details") else ""))
    for node_id, info in (detail.get("node_errors") or {}).items():
        for e in info.get("errors") or []:
            lines.append(f"节点 {node_id}（{info.get('class_type')}）：{e.get('message')} {e.get('details', '')}".strip())
