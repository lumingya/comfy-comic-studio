...
def consistency_prompt(story: dict, panel: dict) -> tuple[str, list[str]]:
    ...
    for i, ch in enumerate(cast, 1):
        lines.append(f'{i}. "{P.display_name(ch)}" — reference image {i}. Key features: {"; ".join(ch["signature"])}. '
                     f"Full description: {ch['description']}.")
    expected = "\n".join(lines) if lines else "(none — this panel should show no people)"
    example = json.dumps({"characters": [{"name": P.display_name(ch), "present": True, "identity": 4,
                                          "features": {f: True for f in ch["signature"]}, "note": ""} for ch in cast],
                          "extra_people": 0, "scene": 4, "quality": 4}, ensure_ascii=False)
...
def score(story: dict, panel: dict, result: dict) -> list[dict]:
    ...
    for i, c in enumerate(panel.get("characters") or []):
        ...
        entry = next((r for r in reported if isinstance(r, dict) and str(r.get("name", "")).lower() == name), None)
        ...
        verdicts.append({"panel": panel["id"], "character": ch["id"], "present": present, "identity": identity,
                         "features_ok": len(right), "features_wrong": wrong,
                         "pass": present and identity >= PASS_IDENTITY and not wrong, "note": entry.get("note", "")})
    return verdicts
...
def judge_panel(llm, story: dict, panel: dict, anchors: dict[str, bytes], image: bytes) -> dict:
    ...
    return {"panel": panel["id"], "raw": data, "verdicts": score(story, panel, data), "model": reply.model,
...
def faces_prompt(story: dict, panel: dict) -> str:
    chars, _ = P.index(story)
    cast = [chars[c["id"]] for c in panel.get("characters") or [] if c.get("id") in chars]
    known = "; ".join(f"{P.display_name(ch)} ({ch['description']})" for ch in cast) or "none expected"
    return (f"Locate the face of every person in this image. Known characters: {known}.\n"
...
def normalize_faces(result, size: tuple[int, int]) -> list[dict]:
    ...
    for face in faces if isinstance(faces, list) else []:
        ...
        if x2 - x1 < 0.01 or y2 - y1 < 0.01:
            continue
        out.append({"name": str(face.get("name") or "unknown"), "box": [round(v, 4) for v in (x1, y1, x2, y2)]})
    return out
...
def locate_faces(llm, story: dict, panel: dict, image: bytes, size: tuple[int, int]) -> list[dict]:
    ...
    chars, _ = P.index(story)
    names = {P.display_name(chars[c["id"]]).lower(): c["id"] for c in panel.get("characters") or [] if c.get("id") in chars}
    for face in faces:
...
def summarize(judgements: list[dict]) -> dict:
    ...
    return {
        ...
        "feature_accuracy": round(sum(v["features_ok"] for v in verdicts) / feature_total, 3) if feature_total else None,
