...
MIN_AGE = 20
MAX_BUBBLE_CHARS = 36
BLOCKED = ("nsfw", "nude", "naked", "sex", "explicit", "nipples", "underwear", "lingerie", "cleavage",
           "loli", "shota", "child", "kid", "toddler", "underage", "minor", "schoolgirl", "school uniform",
           "萝莉", "幼女", "正太", "儿童", "小学生", "初中生")
...
def normalize_story(data) -> dict:
    ...
    for i, ch in enumerate(story.get("characters") if isinstance(story.get("characters"), list) else []):
    ...
    for i, pn in enumerate(story.get("panels") if isinstance(story.get("panels"), list) else []):
        ...
        pn["shot"] = str(pn.get("shot") or "medium").strip().lower().replace("-", "_").replace(" ", "_")
        ...
        for c in pn.get("characters") if isinstance(pn.get("characters"), list) else []:
            if isinstance(c, dict):
                cast.append({"id": _slug(c.get("id"), ""), "expression": str(c.get("expression") or "").strip(),
                             "action": str(c.get("action") or "").strip(), "tags": _tags(c.get("tags"))})
        pn["characters"] = cast
        lines = []
        for d in pn.get("dialogue") if isinstance(pn.get("dialogue"), list) else []:
            if isinstance(d, dict) and str(d.get("text") or "").strip():
                ...
                if kind == "narration":
                    speaker = NARRATOR
                lines.append({"speaker": _slug(speaker, NARRATOR), "kind": kind, "text": str(d["text"]).strip()})
        pn["dialogue"] = lines
    return story
...
def validate_story(story: dict, min_panels: int = 12, max_panels: int = 16) -> list[str]:
    ...
    visual = [t for c in chars for t in c.get("tags", [])] + [c.get("description", "") for c in chars]
    visual += [t for s in scenes for t in s.get("tags", [])] + [s.get("description", "") for s in scenes]
    for p in panels:
...
def generate_script(sentence: str, client, min_panels: int = 12, max_panels: int = 16, models=None):
    ...
    system = SYSTEM_PROMPT.format(min_panels=min_panels, max_panels=max_panels, max_chars=MAX_BUBBLE_CHARS)
    messages = [{"role": "system", "content": system}, {"role": "user", "content": sentence.strip()}]
    data, reply = client.chat_json(
        messages, models or TEXT_MODELS,
        validate=lambda d: validate_story(normalize_story(d), min_panels, max_panels), repairs=2,
    )
...
def stats(story: dict) -> dict:
    ...
    return {
        ...
        "shots": {s: sum(1 for p in panels if p.get("shot") == s) for s in SHOTS if any(p.get("shot") == s for p in panels)},
