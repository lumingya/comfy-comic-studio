...
def ellipse_size(lines: list[str], font, gap: int, pad: tuple[int, int] = (22, 16)) -> tuple[int, int]:
...
class EdgeMap:
    ...
    def mean(self, box) -> float:
        ...
        x0, y0, x1, y1 = (max(0, min(lim, round(v * self.scale))) for v, lim in
                          zip(box, (self.w, self.h, self.w, self.h)))
        if x1 <= x0 or y1 <= y0:
            return 0.0
        I = self.integral
...
def place_bubbles(panel_id: str, panel_box, specs: list[dict], faces: list[dict], edges: EdgeMap | None,
                  style: Style, fonts: dict, occupied: list, gutter_above: int, strip_width: int) -> list[Placement]:
    ...
    for f in faces or []:
        ...
        face_boxes.append({"character": f.get("character"),
                           "box": (px0 + x1 * pw - fw * 0.12, py0 + y1 * ph - fh * 0.12,
                                   px0 + x2 * pw + fw * 0.12, py0 + y2 * ph + fh * 0.12)})
    ...
    for spec in specs:
        ...
        if kind in ("speech", "thought"):
            ...
            bw, bh = tw + 40, th + 30
        speaker_face = next((f["box"] for f in face_boxes if spec.get("speaker") and f["character"] == spec["speaker"]), None)
        ...
        for cx in xs:
            ...
            for cy in ys:
                ...
                if box[0] < 6 or box[2] > strip_width - 6 or box[1] < py0 - hang - 1:
                    continue
                if any(_overlap(box, o) > 0 for o in occupied) or any(_overlap(box, p.box) > 0 for p in placed):
                    continue
                area = _area(box)
                face_hit = sum(_overlap(box, f["box"]) for f in face_boxes) / area
                inside = (box[0] - px0, max(box[1], py0) - py0, box[2] - px0, min(box[3], py1) - py0)
        ...
        if kind in ("speech", "thought"):
            ...
            if speaker_face:
                ...
                tail = (round(cx + (tx - cx) * reach / dist), round(cy + (ty - cy) * reach / dist)) if dist else None
            else:
                tail = (round(cx + (box[2] - box[0]) * 0.12), round(box[3] + 34))
        area = _area(box)
        overlap = sum(_overlap(box, f["box"]) for f in face_boxes) / area if area else 0.0
        placed.append(Placement(panel_id, kind, text, lines, box, tail, spec.get("speaker"), round(overlap, 3)))
    return placed
    ...
    if p.kind in ("speech", "thought"):
        ...
        if p.tail and p.kind == "thought":
            ...
            for t, r in ((0.35, 9), (0.75, 5)):
                ...
                draw.ellipse((px_ - r, py_ - r, px_ + r, py_ + r), fill="white", outline="black", width=2)
...
# ----------------------------------------------------------------------------- strip
def layout_strip(story: dict, images: dict, faces: dict | None = None, style: Style | None = None) -> StripResult:
    """``images``: panel id -> PIL image (panels without an image are skipped)."""
    style = style or Style()
    fonts = {"text": load_font(style.font_path, style.font_size),
    ...
    for p in panels:
        ...
        if p["scene"] != prev_scene and scenes.get(p["scene"], {}).get("location"):
            specs.append({"kind": "caption", "text": scenes[p["scene"]]["location"], "speaker": None})
        for d in p.get("dialogue") or []:
            specs.append({"kind": d["kind"], "text": d["text"], "speaker": None if d["speaker"] == "narrator" else d["speaker"]})
        edges = EdgeMap(scaled[p["id"]])
        gutter_above = box[1] - prev_bottom
        placements += place_bubbles(p["id"], box, specs, (faces or {}).get(p["id"], []), edges, style, fonts,
                                    [q.box for q in placements], gutter_above, W)
    ...
    if title:
        ...
        draw.text(((W - tw) / 2, (style.top - style.title_size) / 2 - 10), title, font=fonts["title"], fill="black")
    end = "— 完 —"
    ew = fonts["text"].getlength(end)
    draw.text(((W - ew) / 2, height - style.bottom / 2 - style.font_size / 2), end, font=fonts["text"], fill=(90, 90, 90))
    report = {"height": height, "panels": len(panels), "bubbles": len(placements),
              "bubbles_on_faces": sum(1 for q in placements if q.face_overlap > 0.02),
              "font_px": style.font_size, "font": os.path.basename(style.font_path or "default")}
