...
from mio_next.comfy.client import ComfyClient, ComfyError, parse_preview, describe_prompt_error
from mio_next.comfy.ws import GUID, OP_BINARY, OP_CLOSE, OP_CONT, OP_PING, OP_PONG, OP_TEXT, encode_frame
...
class FakeComfy(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path == "/prompt":
            ...
            if any(n.get("class_type") == "Broken" for n in payload["prompt"].values()):
                return self._json(400, {"error": {"message": "Prompt outputs failed validation", "details": ""},
                                        "node_errors": {"5": {"class_type": "Broken", "errors": [
                                            {"message": "Value not in list", "details": "ckpt_name: 'x' not in []"}]}}})
    ...
    def _websocket(self):
        ...
        send = lambda op, data, fin=True: sock.sendall(encode_frame(op, data, mask=False, fin=fin))
        text = lambda obj: send(OP_TEXT, json.dumps(obj).encode())
        ...
        if self.server.scenario == "error":
            text({"type": "execution_error", "data": {"prompt_id": "p1", "node_id": "3", "node_type": "KSampler",
                                                      "exception_message": "CUDA out of memory\n"}})
        else:
            half = json.dumps({"type": "executing", "data": {"prompt_id": "p1", "node": "8"}}).encode()
...
class ClientTests(unittest.TestCase):
    ...
    def test_run_follows_websocket_events_and_collects_outputs(self):
        ...
        events = []
        result = client.run({"9": {"class_type": "SaveImage", "inputs": {}}}, output_nodes=["9"], on_event=events.append, timeout=20)
...
class WebSocketTimeoutTests(unittest.TestCase):
    ...
    def test_recv_survives_timeouts_mid_frame_and_between_fragments(self):
        import socket
        import time
        from mio_next.comfy.ws import WebSocket
        ...
        def serve():
            ...
            with conn:
                ...
                key = [l.split(b":", 1)[1].strip() for l in request.split(b"\r\n") if l.lower().startswith(b"sec-websocket-key")][0]
...
class ParsingTests(unittest.TestCase):
    def test_parse_preview_variants(self):
        ...
        self.assertEqual((parsed["format"], parsed["data"], parsed["meta"]["node_id"]), ("webp", b"img", "3"))
