...
FOLDERS = {'js', 'vendor', 'backend', 'packaging', 'docs', 'examples', 'tests', 'tools', '.github'}
...
def source_files():
    ...
    for path in sorted(ROOT.rglob('*')):
        ...
        if any(part in EXCLUDED or (part.startswith('acceptance') and part not in {'acceptance-review', 'acceptance-extensibility'}) for part in rel.parts) or path.is_symlink() or not path.is_file():
            continue
        if len(rel.parts) == 1:
...
def build(output):
    ...
    from backend.mio_docs import source_documents, render_document
    documents = {p.relative_to(ROOT).with_suffix('.html').as_posix(): render_document(ROOT, p.relative_to(ROOT)).encode('utf-8') for p in source_documents(ROOT)}
    files = [(path, rel) for path, rel in files if rel not in documents]
    paths = {rel for _, rel in files} | documents.keys()
    required = {'js/extension-runtime.js','js/extension-host.js','js/customization.js','backend/ecosystem/customization.py','docs/EXTENSIBILITY_V2.html','tests/extensibility.mjs','tests/extension_runtime.mjs','tests/test_customization.py','examples/extensions/studio-toolkit/index.js','examples/themes/unrestricted-atelier/theme.css','js/home.js','tests/home_workspace.mjs','docs/HOME_WORKSPACE_2026-09-18.md','tests/locale_audit.mjs','tests/inline_credentials.mjs','docs/UI_LOCALE_INLINE_KEYS_2026-09-18.md','js/first-run.js','backend/mio_connection_check.py','tests/first_use_journeys.mjs','tests/journey_fixture_server.py','tests/test_first_use.py','docs/FIRST_USE_2026-09-18.md','packaging/requirements.txt','backend/mio_media.py','backend/mio_http.py','backend/mio_proxy.py','backend/mio_services.py','backend/mio_export.py','js/dom-updates.js','tests/review.mjs','tests/test_review_reliability.py','js/architecture.js','tests/architecture.mjs','docs/ARCHITECTURE_ACCEPTANCE.md','backend/production/store.py','backend/production/queue.py','backend/production/api.py','js/assembly-workshop.js','js/preferences-workbench.js','backend/ecosystem/acorn.cjs','backend/ecosystem/ACORN_LICENSE','backend/ecosystem/macro_worker.mjs','backend/ecosystem/macro_analyze.mjs','backend/ecosystem/api.py','js/ecosystem.js','js/safe-mode.js','docs/ECOSYSTEM_GUIDE.html','docs/COMPUTED_VARIABLES.html','examples/extensions/scene-notebook/index.js','examples/macros/prerequisite-variables.json','docs/RELEASE_CURRENT.md','tests/preset_isolation.mjs','data/distribution.json','data/catalog/index.json','data/catalog/reader-sizing.json','backend/mio_content.py','backend/mio_album_html.py','backend/mio_resource_sharing.py','js/content-loader.js','js/album-metadata.js','js/contextual-sharing.js','docs/guide/CONTENT_AND_SHARING.html','docs/en/CONTENT_AND_SHARING.html','server.py', 'backend/mio_lifecycle.py', 'backend/mio_frame_jobs.py', 'js/foundation.js',
    ...
    manifest = {'name': 'Mio', 'version': version, 'kind': 'complete-source-runtime',
                'upstreamBaseline': '50fbeca582e2571bce654ea42821ded22d26ea5f', 'edition': 'extensibility-v2', 'runtime': 'Python >= 3.10 + Pillow >= 11.3,<13; computed variables: Node.js >= 20; Git installs: Git',
                'includesUserData': False, 'includesShippedData': True,
                'files': {**{rel: hashlib.sha256(path.read_bytes()).hexdigest() for path, rel in files}, **{rel: hashlib.sha256(raw).hexdigest() for rel, raw in documents.items()}}}
    with zipfile.ZipFile(target, 'w', zipfile.ZIP_DEFLATED, compresslevel=9) as archive:
        for path, rel in files:
            archive.write(path, name + '/' + rel)
        for rel, raw in documents.items():
            archive.writestr(name + '/' + rel, raw)
    ...
    print(f'ZIP verified: {target}\nFiles: {len(files) + len(documents)} + manifest\nBytes: {target.stat().st_size}\nSHA256: {checksum}')
    return target
