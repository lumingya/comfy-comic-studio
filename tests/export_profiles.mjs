...
try{
 ...
 const auto=await p.evaluate(async id=>{const t=exportTemplateBy(presentationUI.templateId);const clean=await prepareExportBooks([id],t,null,null,{imageProfile:'clean'}),publish=await prepareExportBooks([id],t,null,null,{imageProfile:'publish'});const budget=Math.floor((clean.stats.inlineBytes+publish.stats.inlineBytes)/2);const fallback=await prepareExportBooks([id],t,null,null,{imageProfile:'auto',inlineBudget:budget});const fits=await prepareExportBooks([id],t,null,null,{imageProfile:'auto',inlineBudget:clean.stats.inlineBytes+1});const errors={};for(const profile of ['clean','publish']){try{await prepareExportBooks([id],t,null,null,{imageProfile:profile,inlineBudget:publish.stats.inlineBytes-1})}catch(e){errors[profile]=e.message}}return {clean:clean.stats,publish:publish.stats,fallback:fallback.stats,fits:fits.stats,webp:fallback.books[0].steps.every(s=>s.image.startsWith('data:image/webp')),errors}},fixture.book);
 ...
 check(auto.errors.clean.includes('轻量发布')&&auto.errors.publish.includes('ZIP'),'explicit profiles fail with profile-aware guidance instead of silently degrading');
