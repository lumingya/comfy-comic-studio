 ...
 try{
  ...
  if(person.provider==='comfyui'){
   ...
   if(person.provider==='novelai'){
    ...
    if(person.mobile){await p.locator('#provider-model-status').filter({hasText:'获取失败：'}).waitFor();await p.getByRole('combobox',{name:'模型 ID'}).fill('fixture-chat');await p.locator('[data-image-config="protocol"]').selectOption('chat');check(await p.locator('[data-image-config="sendSize"]').isDisabled(),'Chat user can type model after /models 404; Images-only parameters are disabled')}
