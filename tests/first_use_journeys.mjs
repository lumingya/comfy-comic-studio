 ...
 try{
  ...
  if(person.provider==='comfyui'){
   ...
   for(const [filename,doc,bad] of [['canvas.json',{nodes:[],links:[]},true],['my-api-workflow.json',workflow,false]]){
    const chooser=p.waitForEvent('filechooser');await p.getByRole('button',{name:'添加工作流',exact:false}).first().click();await p.getByRole('button',{name:'选择或拖入工作流文件',exact:true}).click();await(await chooser).setFiles({name:filename,mimeType:'application/json',buffer:Buffer.from(JSON.stringify(doc))});
    if(bad){await p.locator('#wf-import-filelist .filerow.is-bad').waitFor();check((await p.locator('#wf-import-filelist').innerText()).includes('导出 API 格式')&&await p.locator('#wf-unified-submit-btn.disabled').count()===1,'ComfyUI canvas JSON is rejected in the import sheet with API export instructions');await p.locator('#modal [data-act="close-modal"]').first().click();await p.waitForFunction(()=>!document.querySelector('#modal').open)}
    else{await p.locator('#wf-unified-submit-btn:not(.disabled)').click();await p.getByRole('heading',{name:'批量导入结果',exact:true}).waitFor();check((await p.locator('#modal').innerText()).includes('成功导入 1 份'),'ComfyUI API workflow imports via real file picker');await p.getByRole('button',{name:'完成',exact:true}).click()}
   }
  }else{
