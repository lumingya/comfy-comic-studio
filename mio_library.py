...
def image_type(raw):
    ...
    if raw.lstrip(b'\xef\xbb\xbf \t\r\n').startswith((b'<svg', b'<?xml')):
        from mio_safe_svg import validate_svg
        try:
            validate_svg(raw)
...
def scrub(value):
    ...
    if isinstance(value, dict):
        return {k: scrub(v) for k, v in value.items()
                if (k != 'key' or value.get('type') in ('image', 'text', 'number', 'boolean', 'json') or value.get('kind') == 'mio-image')
...
def assert_no_credentials(value, pointer=''):
    ...
    if isinstance(value, dict):
        typed_variable = value.get('type') in ('image', 'text', 'number', 'boolean', 'json') and 'value' in value or value.get('kind') == 'mio-image'
...
def share_document(kind, doc):
    fields = {'id', 'schema', 'kind', 'title', 'description', 'outline', 'createdAt',
              ...
              'bindings', 'outputNodeId', 'sharedSources'}
