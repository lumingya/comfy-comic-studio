...
import uuid
import urllib.parse
from mio_library import LibraryError, MAX_ASSET, decode, digest, encode, image_refs, image_type, owned_path
...
def leaves(value, pointer=''):
    if isinstance(value, dict):
        for key, child in value.items():
            ...
            typed_key = key == 'key' and (value.get('type') in ('image', 'text', 'number', 'boolean', 'json') and 'value' in value or value.get('kind') == 'mio-image')
