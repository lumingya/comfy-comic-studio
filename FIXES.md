...
- **单击即打开 / 切换**：画册、分幕、工作流、映射行都不再需要双击；单击直接进入。
- **唯一多选入口**：拖动框选（Windows / ComfyUI 画布式）、Ctrl/⌘ 点击加选、Shift 连选、Ctrl/⌘ A 全选。
  删除了「多选」「批量管理」「选择」等手动进入多选的按钮及右键菜单里的"加入多选/批量管理"入口（仅触屏保留复选框回退）。
- **无勾选小方块、无工具栏**：多选期间不再渲染 checkbox 与 `sel-bar` / `shelf-bulk` 之类的操作条，只留一行状态文字"已选 N · 右键操作"。
- **右键菜单分流**：右键落在选中项上 → 多选菜单（批量星标/重命名/导出/补齐/删除、全选/取消）；
  右键落在未选中项上 → 选区解散，显示该项的单项菜单。Delete 键删除选区，Esc 清空。
- 分幕多选新增：批量插入起手模板、批量复制；映射多选新增：批量启用 / 停用。
- 修复：右键菜单被打开瞬间的滚动事件误关闭。
