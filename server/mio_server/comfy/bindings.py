...
def tag_bindings(graph: dict) -> tuple[list[Binding], list[str]]:
    ...
    for node_id, node in graph.items():
        ...
        for kind, qualifier, field in parse_tags(title_of(node)):
            ...
            if not target:
                problems.append(
                    f"节点 {node_id}：[mio:{kind}] 找不到可写字段，请写成 [mio:{kind}=字段名]"
                )
                continue
            if target not in inputs:
                problems.append(
                    f"节点 {node_id}：字段 {target} 不存在（可用：{', '.join(inputs)}）"
                )
                continue
...
def select_variant(
    graph: dict, bindings: list[Binding], variant: str | None
) -> tuple[dict, list[str]]:
...
def text_sources(graph: dict) -> list[tuple[str, str, str]]:
    ...
    encoders = [
        k for k, n in graph.items() if any(h in n.get("class_type", "") for h in TEXT_ENCODER_HINT)
    ]
...
FILE_KEYS = re.compile(
    r"(^|_)(name|names|lora|loras|ckpt|model|vae|file|filename|path|dir|prefix|format)$", re.I
)
    ...
    for node_id in outputs:
        ...
        for field, value in inputs.items():
            if value is False and (
                "preview" in field.lower() or field.lower() in ("enable", "enabled")
            ):
                warnings.append(
