...
class ClientTests(unittest.TestCase):
    def setUp(self):
        ...
        self.client = L.Client(
            f"http://127.0.0.1:{self.server.server_address[1]}/v1", timeout=5, retries=0
        )

    def test_fallback_to_next_model_and_fenced_json(self):
        reply = self.client.chat(
            [{"role": "user", "content": "hi"}], models=("bad", "empty", "fence")
        )
    ...
    def test_pace_spaces_requests_across_threads(self):
        ...
        with ThreadPoolExecutor(3) as pool:
            list(
                pool.map(
                    lambda _: client.chat([{"role": "user", "content": "x"}], models=("plain",)),
                    range(3),
                )
            )
    ...
    def test_generate_image_downloads_and_multi_ref_falls_back(self):
        ...
        data, reply = self.client.generate_image(
            "two", refs=[PNG, PNG], models=("max-broken", "img"), retries=0
        )
        self.assertEqual(data, PNG)
        sent = self.server.requests[-1]["messages"][0]["content"]
        self.assertEqual(
            sum(1 for part in sent if part.get("type") == "image_url"), 1
        )  # single-ref fallback
    ...
    def test_cloud_panel_falls_back_to_a_lineup_reference(self):
        from mio_server.pipeline import cloud as render
        from mio_server.pipeline import script
        ...
        try:
            data, meta = render.CloudRenderer(self.client).panel(
                story, two_shot, {"lin": PNG, "zhou": PNG}
            )
        finally:
            L.IMAGE_MULTI, L.IMAGE_SINGLE = saved
        self.assertEqual(
            (meta["mode"], meta["model"], meta["refs"]), ("lineup", "img", ["lin", "zhou"])
        )
        sent = self.server.requests[-1]["messages"][0]["content"]
...
class HelperTests(unittest.TestCase):
    ...
    def test_image_urls_from_markdown_and_images_field(self):
        msg = {
            "content": "![a](http://x/1.png) and ![b](http://x/2.png)",
            "images": [{"image_url": {"url": "http://x/1.png"}}],
        }
