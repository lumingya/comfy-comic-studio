...
from mio_server.comfy import workflow_slots as s
...
def fixtures():
    ...
    add('alice',json.loads((Path(__file__).parents[1]/"fixtures/alice_copy_workflow.json").read_text(encoding="utf-8"))['workflow'],{'model':'new.safetensors','loras':[{'name':'A','strength':.8}]})
    return cases
