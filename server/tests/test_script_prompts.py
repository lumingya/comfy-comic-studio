...
from mio_server.pipeline import prompts as P
...
class ScriptValidationTests(unittest.TestCase):
    ...
    def test_normalize_cleans_tags_ids_and_narration(self):
        raw = make_story()
        raw["characters"][0]["tags"] = ["Black_Hair", "black hair", "  Bob   Cut "] + raw[
            "characters"
        ][0]["tags"]
    ...
    def test_rejects_structural_problems(self):
        ...
        self.check(
            lambda s: s["panels"][3]["characters"].append({"id": "ghost"}), "角色 ghost 不存在"
        )
        self.check(
            lambda s: s["panels"][1]["dialogue"].append({"speaker": "zhou", "text": "一" * 40}),
            "超过",
        )
        self.check(
            lambda s: s["panels"][1]["dialogue"].append({"speaker": "boss", "text": "嗯"}),
            "说话人 boss",
        )
        self.check(
            lambda s: s["characters"][0].update(signature=["red scarf", "bob cut"]), "取自 tags"
        )
...
class CompilerTests(unittest.TestCase):
    ...
    def test_lineup_wording_and_composite(self):
        ...
        from mio_server import imaging
        ...
        combo = Image.open(
            io.BytesIO(imaging.lineup([png((400, 800)), png((300, 900))], height=600, gap=20))
        )
...
class GoldenStoryTests(unittest.TestCase):
    ...
    def test_golden_story(self):
        ...
        raw = json.loads(
            (Path(__file__).resolve().parent.parent / "fixtures" / "golden_story.json").read_text(
                encoding="utf-8"
            )
        )
        story = S.normalize_story(raw)
        ...
        for panel in story["panels"]:
            ...
            for cid in refs:
                self.assertIn(
                    P.display_name(next(c for c in story["characters"] if c["id"] == cid)), text
                )
