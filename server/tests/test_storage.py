...
class StorageTests(unittest.TestCase):
    def test_series_and_episodes_roundtrip(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            series = Series(title="长雨街", bible={"characters": [Character(name="苏晚", age=24).model_dump()]})
            ...
            episode = Episode(series_id=series.id, title="第一话", order=0, panels=[Panel(order=0, description="雨幕")])
