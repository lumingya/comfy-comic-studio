...
def multi_stage_workflow():
    ...
    return {
        ...
        "2": {
            ...
            "inputs": {
                "text": "<lora:style_a:0.8>",
                "loras": {
                    "__value__": [
                        {"name": "style_a", "active": True},
                        {"name": "style_b", "active": False},
                    ]
                },
                "model": link("1"),
            },
        ...
        "10": {
            "class_type": "FancyPromptUI",
            "inputs": {"positive": "old subject tags", "auto_random": False},
            "_meta": {"title": "Positive editor [mio:prompt=positive]"},
        },
        "11": {
            "class_type": "FancyPromptUI",
            "inputs": {"positive": "old negative", "auto_random": False},
            "_meta": {"title": "Negative editor [mio:negative=positive]"},
        },
        "12": {
            "class_type": "PromptSelector",
            "inputs": {"selected_prompts": "masterpiece, best quality, forbidden_style,"},
            "_meta": {"title": "Fixed prefix"},
        },
        "13": {
            "class_type": "PromptSelector",
            "inputs": {"selected_prompts": "forbidden_tag,"},
            "_meta": {"title": "Fixed suffix"},
        },
        "14": {
            "class_type": "PromptConcat",
            "inputs": {"a": link("12"), "b": link("10"), "c": link("13")},
            "_meta": {"title": "concat"},
        },
        "15": {
            "class_type": "PCTextEncode",
            "inputs": {"text": link("14"), "clip": link("2", 1)},
            "_meta": {"title": "encode +"},
        },
        "16": {
            "class_type": "CLIPTextEncode",
            "inputs": {"text": link("11"), "clip": link("2", 1)},
            "_meta": {"title": "encode -"},
        },
        "20": {
        ...
        "21": {
            ...
            "inputs": {"width": link("20", 0), "height": link("20", 1), "batch_size": 1},
            "_meta": {"title": "latent"},
        },
        "30": {
        ...
        "31": {
            ...
            "inputs": {
                ...
                "positive": link("15"),
                "negative": link("16"),
                "latent_image": link("30"),
            },
            "_meta": {"title": "Refiner [mio:seed=noise_seed]"},
        },
        "40": {
            "class_type": "VAEDecode",
            "inputs": {"samples": link("31"), "vae": link("1", 2)},
            "_meta": {"title": "decode"},
        },
        "50": {
            "class_type": "PreviewImage",
            "inputs": {"images": link("40")},
            "_meta": {"title": "Base [mio:output:draft]"},
        },
        "60": {
        ...
        "61": {
            ...
            "inputs": {"image": link("40"), "upscale_model": link("60")},
            "_meta": {"title": "upscale"},
        },
        "62": {
            "class_type": "SaveImage",
            "inputs": {"images": link("61"), "filename_prefix": "final"},
            "_meta": {"title": "Final [mio:output:final]"},
        },
        "70": {
            "class_type": "ShowText",
            "inputs": {"text": link("14")},
            "_meta": {"title": "Final prompt [mio:inspect]"},
        },
        "80": {
            "class_type": "easy globalSeed",
            "inputs": {"value": 5, "mode": True},
            "_meta": {"title": "frontend-only seed"},
        },
    }
...
class ResolveTests(unittest.TestCase):
    ...
    def test_field_selection_on_custom_prompt_nodes(self):
        ...
        self.assertEqual(
            [(b.node, b.field) for b in self.by_kind["negative"]], [("11", "positive")]
        )

    def test_seed_fans_out_to_both_samplers(self):
        self.assertEqual(
            sorted((b.node, b.field) for b in self.by_kind["seed"]),
            [("30", "noise_seed"), ("31", "noise_seed")],
        )
    ...
    def test_outputs_carry_variants(self):
        self.assertEqual(
            sorted((b.node, b.qualifier) for b in self.by_kind["output"]),
            [("50", "draft"), ("62", "final")],
        )
    ...
    def test_apply_values_writes_every_target_and_keeps_input_untouched(self):
        ...
        out = B.apply_values(
            self.graph,
            self.bindings,
            {"prompt": "1girl, adult, seaside", "seed": 42, "width": 832, "height": 1216},
        )
        self.assertEqual(self.graph, before)
        self.assertEqual(out["10"]["inputs"]["positive"], "1girl, adult, seaside")
        self.assertEqual(
            (out["30"]["inputs"]["noise_seed"], out["31"]["inputs"]["noise_seed"]), (42, 42)
        )
        self.assertEqual((out["20"]["inputs"]["width"], out["20"]["inputs"]["height"]), (832, 1216))
        self.assertEqual(
            out["11"]["inputs"]["positive"], "old negative"
        )  # no value given -> unchanged
...
class OverrideAndGuardTests(unittest.TestCase):
    ...
    def test_guard_finds_terms_in_every_prompt_source_until_overridden(self):
        ...
        self.assertEqual(
            sorted((n, t) for n, _, t in hits), [("12", "forbidden_style"), ("13", "forbidden_tag")]
        )
        clean = B.apply_overrides(
            self.graph,
            {"/13/inputs/selected_prompts": "", "/12/inputs/selected_prompts": "masterpiece"},
        )
...
class WildcardGuardLintTests(unittest.TestCase):
    def test_wildcard_switches_off_every_lora_entry(self):
        out = B.apply_overrides(
            multi_stage_workflow(), {"/2/inputs/loras/__value__/*/active": False}
        )
        self.assertEqual(
            [e["active"] for e in out["2"]["inputs"]["loras"]["__value__"]], [False, False]
        )
        with self.assertRaises(B.BindingError):
    ...
    def test_guard_reads_nested_strings_but_not_asset_names(self):
        ...
        graph["90"] = {
            "class_type": "TriggerToggle",
            "inputs": {
                "words": {"__value__": [{"text": "forbidden_tag", "active": True}]},
    ...
    def test_guard_skips_fields_bound_as_negative(self):
        ...
        clean = B.apply_overrides(
            graph, {"/12/inputs/selected_prompts": "", "/13/inputs/selected_prompts": ""}
        )
    ...
    def test_guard_text_on_final_prompt(self):
        self.assertEqual(
            B.guard_text("masterpiece, forbidden_tag, 1girl", ["forbidden_tag", "x"]),
            ["forbidden_tag"],
        )
