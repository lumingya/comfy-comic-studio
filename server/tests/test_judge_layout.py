...
from mio_server.pipeline import judge as J
from mio_server.pipeline import layout as LY
from mio_server.pipeline import script as S
...
class JudgeScoringTests(unittest.TestCase):
            ...
            "characters": [
                {
                    ...
                    "present": True,
                    "identity": 4,
                    "features": {"bob cut": True, "round eyewear": None},
                },
                {
                    "name": "ZHOU",
                    "present": True,
                    "identity": 5,
                    "features": {"green hoodie": False},
                },
            ]
        }
        verdicts = J.score(self.story, self.two, result)
        self.assertEqual(
            [(v["character"], v["pass"]) for v in verdicts], [("lin", True), ("zhou", False)]
        )
    ...
    def test_missing_or_garbage_entries_fail(self):
        ...
        verdicts = J.score(
            self.story, self.two, {"characters": [{"present": True, "identity": "5"}, "junk"]}
        )
        self.assertEqual(
            [v["pass"] for v in verdicts], [True, False]
        )  # index fallback for the first
        self.assertEqual(
            J.score(
                self.story,
                self.two,
                {"characters": [{"name": "Lin", "present": False, "identity": 5}]},
            )[0]["pass"],
            False,
        )
    ...
    def test_summary(self):
        ...
        self.assertEqual(
            (summary["appearances"], summary["passed"], summary["pass_rate"]), (2, 1, 0.5)
        )
...
class LayoutTests(unittest.TestCase):
    ...
    def test_balanced_wrap_evens_lines_and_prefers_punctuation(self):
        ...
        for text in (
            "原来……我眼里的光还在啊。",
            "陆店主，那我们下次雨天见！",
            "hello brave new world of comics",
        ):
    ...
    def test_strip_bubbles_avoid_faces_and_slices_fit(self):
        ...
        expected = sum(len(p["dialogue"]) for p in panels) + len(
            self.story["scenes"]
        )  # + scene captions
    ...
    def test_speech_tail_points_at_speaker(self):
        ...
        fonts = {
            "text": LY.load_font(self.style.font_path, 27),
            "caption": LY.load_font(self.style.font_path, 22),
            "sfx": LY.load_font(self.style.font_path, 40),
        }
        placed = LY.place_bubbles(
            "p",
            (0, 100, 800, 900),
            [{"kind": "speech", "text": "你好。", "speaker": "lin"}],
            faces,
            LY.EdgeMap(im.resize((800, 800))),
            self.style,
            fonts,
            [],
            64,
