> 2.3.0：留白与无缝模板在桌面使用完整画面自适应分页，手机仍连续；[规则与验收](../RELEASE_2_3_0.md)。

> 1.2.1 调整 [余光 · AFTERGLOW](../RELEASE_1_2_1.md)：正文改为居中连续画面，标题和旁白贴近图下，保留拱形封面、目次、大图和日夜模式。
...
[离线演示](../../examples/seamless/无缝_纯图阅读_演示画册.html) · [发布与验收](../RELEASE_2_1_0.md)


> 历史 1.1.1 更新（默认浏览方式已在 2.3 调整）：展开、收起或搜索模板面板不重建当前阅读文档。内置模板预览与导出底部增加细小的项目小诗和链接。详见 [更新说明](../RELEASE_1_1_1.md)。
