...
日常创作入口不变：选渠道、写分镜、设变量、生成画册。但**真实画册的执行者已经由浏览器改为 Python 服务**。浏览器只负责编辑、保存本册输入、提交和显示结果；不会再负责维持整册的生成循环。

- 任务保存在 `data/runtime/execution/jobs.sqlite3`，以 SQLite WAL 事务记录状态、每幕结果、上游 ID 和事件。
...
- OpenAI/NovelAI 同步接口通常没有可供 Mio 查询的任务 ID。先自行检查供应商记录，再明确确认风险并**继续未完成分镜**。如不再保留画册，可正常确认删除，不需先核对记录。删除不能取消上游请求或退款。
- 对未确认的原画册，普通补齐优先进入原任务恢复确认，不新建画册。未经明确确认，不重发未知幕；其他尚未请求的分镜默认仍可继续。

`archive` 用于无在途请求的任务（包括未知结果），隐藏任务记录但不删除画册：保留幂等记录，不再列入正常任务列表或占用服务端素材引用。原画册和变量仍然会独立保护其文件。
...
| `abandon` | 旧客户端兼容接口；界面不再提供此步骤，也不是删除前置条件 |
| `archive` | 隐藏无在途请求的任务，保留画册 |
...
- 清理只移到 `data/trash/<时间>/...`，没有自动永久删除。误操作可以停服务后按原相对路径搬回 `data/runtime/staging/images/`。
...
界面直接使用原分镜故事编辑区，服务在每次发送前读取该画册最新已保存的输入。右上角「当前画册」可选择具体画册版本，详见 [队列与编辑说明](../QUEUE_AND_COLLECTION_UPDATE.md)。`requestHistory` 返回最近 100 次准备发送的输入：索引、尝试号、时间、完整输入摘要、提示词、图片引用和参数。完整输入保存在 SQLite；不代表上游已经接受请求。核对 ComfyUI 时使用原请求输入，不使用新草稿。
...
`POST /api/v1/albums/delete`，请求 `{"ids":["album-id"]}`。同一事务记录删除标记并停止、移除这些画册的全部关联任务；返回 `deletedAlbumIds` 与 `removedJobIds`。重复删除幂等，不生成图片、不确认计费、不撤销上游请求。`jobs/{id}` 的 `remove` 具有同样的整册删除语义。读取任务的 `allowedActions` 获取当前允许操作，`requiresRecoveryConsent` 仅用于重新发送未确认结果。
...
创作画册 → 生成队列 → 失败策略新增「连续失败暂停上限」，默认 5、允许 0–100，0 关闭保护。每次失败尝试计数，包含逐幕首次尝试和重试；成功清零。达到上限后暂停当前任务的重试与后续分镜，其他画册继续。正在执行的请求仍可保存结果，但迟到成功/刷新/重启不会自动解除已触发暂停。修好服务后，确认继续未完成分镜才重置；结果不明仍需计费风险确认，删除仍一次确认。详见 [2.2.3 说明](../RELEASE_CURRENT.md)。
