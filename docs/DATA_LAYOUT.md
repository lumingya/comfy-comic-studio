...
│   └── YYYY/MM/DD/comfy_*.png        旧下载接口新保存的图片
├── cache/marketplace.json            可重新获取的资源缓存
└── .config-transaction.json          保存过程的恢复日志，正常完成后自动移除
images/                              旧版图片目录，兼容读取，不自动删除
```

## 按画册找图片

进入 **设置 → 服务与保存 → 数据目录与迁移 → 按画册名称查找图片目录**，可以查看名称与实际目录的对应关系。也可在 `albums/index.json` 查找标题和 `id`。
...
- 内置 SVG 演示图片、旧版已内嵌的图片暂不批量转换，仍保留在画册 JSON 中。不会因迁移擅自丢弃旧内容。
- 不自动清理孤立图片，避免误删仍被历史快照或外部备份引用的文件。

## 第一次升级

1. **先退出旧程序，备份原 `data/`、`images/` 和 `comfy_comic_data.json`（如有）。**
2. 使用新版程序启动服务。
3. 读取配置时，检测旧版六个 `data/*.json`；完整且可解析时，写入新目录。
4. 原六个文件保留不动，作为迁移前备份；新版以新目录为准，不会持续双写旧文件。
5. 单文件 `comfy_comic_data.json` 仍支持迁移。
6. 如发现旧文件缺失、JSON 损坏或新配置集合不完整，停止读取并显示错误，**不以默认/空数据覆盖**。请恢复备份中的完整配置后再试。

后端 `/api/config` 仍然返回原来的顶层字段，前端无需理解内部拆分方式。画册计划、预设和其他未知元数据读写时会重新组合。
...
退出程序后复制完整 `data/` 与旧 `images/`；在新电脑同一程序目录下恢复它们。两者应一同备份，单独复制 `albums/index.json` 不包含外部图片。
...
服务默认只监听 `127.0.0.1`。若确需局域网部署，可以配置 `COMFY_COMIC_HOST` 与逗号分隔的 `COMFY_COMIC_ORIGINS` 明确允许来源；此应用没有增加用户认证，不建议直接暴露到公网。
