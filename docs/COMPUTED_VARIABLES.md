...
> 脚本、前端扩展、Python 扩展都是可信代码机制，不是针对恶意代码的安全沙箱。脚本可以请求已保存的模型渠道并产生费用。导入只保存数据，生成时仍须明确确认信任。

## 从界面开始

1. 打开“创作画册 → 角色与画面设定”，展开一个属性分组。
2. 点击属性上的“可执行变量”入口，选择结果类型，编辑函数体。
3. 设置本册的 uint32 随机种子，以及 1–180 秒执行上限。单段同步运算最多运行 30 秒。
4. 点击“生成画册”。确认信任后，界面显示各节点的等待、执行、缓存命中或失败状态。
5. 只有全部成功，生成队列才接收已经求值的变量；源画册的脚本保持不变。

在脚本节点等待期间关闭窗口，会请求取消。已经提交到外部提供商的请求可能仍完成或计费；取消不会撤销外部账单，也不会自动补发。
...
图像 options 支持 `channelId`（保存的图像渠道 ID，省略使用当前渠道）、`negative`、`images`（本地资产数组）、`width`、`height`、`seed`、`steps`、`cfg`、`timeout`、`name`。ComfyUI 还可使用保存的 `workflowId` 或显式 API 格式 `workflow`。不要把 API Key 写入脚本；鉴权留在后端设置。

不同模型支持的尺寸、参数、参考图数量不同，提供商会按其适配器验证；这里不是所有渠道都支持全部参数的承诺。ComfyUI 自定义节点应检查最终工作流；复杂映射可直接传入已构造好的 API workflow。使用 sceneParameter 来源时，在 options 中显式设置 renderOverride: true；variables 可提供绑定所需的已解析变量值。
...
- `compute={script,dependsOn,timeout}` 保存在原属性内，类型与分组一起保存/导出。不能把共享来的脚本视为普通无害数据。
...
- “生成队列 → 强制重算并生成”会明确提示忽略缓存和未确认标记，可能再次计费。不会悄悄换种子。
- 服务重启会将未结束记录标为 interrupted；不会恢复付费请求。
...
生成队列获得独立求值快照，不再跟随源画册脚本变化。修改源画册影响下一次生成；已有结果在历史记录中查看。单幕字面量覆盖仍优先于本册值；可执行脚本应放在本册属性中，单幕不支持独立脚本节点。

模型服务发生错误不会自动降级为另一付费提供商。异步 API 的实际超时可能长于脚本等待时限；仍在进行的网络线程会占用全局模型请求槽，直到该请求退出。
