/* One selection model for desktop asset surfaces. No mode switch, no render during gestures. */
'use strict';
function installDesktopSelection() {
  const scopes = [
    {root:'#gallery-results', item:'[data-sort-book]', key:e=>e.dataset.sortBook,
      get:()=>ui.selected, set:s=>{ui.selected=s;ui.bulk=s.size>0}, render:()=>refreshGallery(), open:e=>handleAction('read',{id:e.dataset.sortBook})},
    ...
    {root:'.workshop-frames-list', item:'button[data-index]', key:e=>Number(e.dataset.index),
      get:()=>workshop.pickedFrames, set:s=>{workshop.pickedFrames=s;workshop.selMode=s.size>0}, render:()=>render(), open:e=>e.click()},
    {root:'#wf-rail-list', item:'[data-workflow-id]', key:e=>e.dataset.workflowId,
      get:()=>mapperUI.libSel, set:s=>{mapperUI.libSel=s;mapperUI.libSelMode=s.size>0}, render:()=>render(), open:e=>e.querySelector('.wf-item-body')?.click()},
    {root:'#wm-binding-rows', item:'[data-binding-row]', key:e=>e.dataset.bindingRow,
      get:()=>mapperUI.sel, set:s=>{mapperUI.sel=s;mapperUI.selMode=s.size>0}, render:()=>render(), open:e=>e.querySelector('.wf-row-main')?.click()}
  ];
  let gesture=null, active=null, bypass=false, suppress=false, raf=0, lastClick=null;
  ...
  function paint(c){for(const e of items(c)){const on=c.s.get().has(c.s.key(e));e.classList.toggle('desktop-selected',on);e.classList.toggle('is-picked',on);if(c.s.root==='.designer-presets')e.classList.toggle('selected',on);if(c.s.root==='#gallery-results')e.classList.toggle('is-selected',on);const cb=e.querySelector('input[type="checkbox"]:not([role="switch"])');if(cb)cb.checked=on}}
  function commit(c){c.s.render();const root=document.querySelector(c.s.root);if(root){active={...c,root};root.tabIndex=0;root.focus({preventScroll:true});paint({...c,root})}}
  ...
  window.addEventListener('pointermove',e=>{
    ...
    if(!g.drag&&Math.hypot(g.x-g.startX,g.y-g.startY)>5){g.drag=true;g.box=document.createElement('div');g.box.className='desktop-marquee';(g.c.root.closest('dialog')||document.body).append(g.box);document.body.classList.add('desktop-selecting');update()}
  ...
  window.addEventListener('click',e=>{
    ...
    e.preventDefault();e.stopImmediatePropagation();active=c;
    const now=performance.now(), clicked=c.item?c.s.key(c.item):null;
    if(!control?.matches('input')&&c.s.open&&clicked!==null&&lastClick?.s===c.s&&lastClick.id===clicked&&now-lastClick.time<360&&!e.ctrlKey&&!e.metaKey&&!e.shiftKey){lastClick=null;bypass=true;try{c.s.open(c.item)}finally{bypass=false}return}
    lastClick={s:c.s,id:clicked,time:now};
    const before=c.s.get(), id=c.item?c.s.key(c.item):null, toggle=e.ctrlKey||e.metaKey||control?.matches('input[type="checkbox"]');
  ...
  window.addEventListener('keydown',e=>{
    if(editable(e.target)||e.isComposing)return;lastClick=null;
