...
if(!s.projects.some(p=>p.id===s.activeProjectId))throw Error('当前画册集不存在。');
...
function typedVariableValue(entry){if(entry.type==='image'){const value=entry.value;if(value&&typeof value==='object'&&value.kind==='mio-image')return clone(value);return{kind:'mio-image',src:typeof value==='string'?value:'',name:entry.key}}if(entry.value===undefined||entry.value===null||typeof entry.value==='string'&&!entry.value.trim())return'';switch(entry.type){case'text':return String(entry.value);case'number':if(!Number.isFinite(Number(entry.value)))throw Error('变量 '+entry.key+' 需要有效数字，或留空使用默认值。');return Number(entry.value);case'boolean':if(typeof entry.value==='boolean')return entry.value;if(['true','false'].includes(entry.value))return entry.value==='true';throw Error('变量 '+entry.key+' 需要 true 或 false。');case'json':if(typeof entry.value==='string'){try{return JSON.parse(entry.value)}catch(e){throw Error('变量 '+entry.key+' 的 JSON 格式不合法。')}}return clone(entry.value);default:throw Error('不支持的变量类型。')}}
...
function ensureCreationModel(s=state){
  ...
  s.settings.comfy.outputNodeId??=s.settings.comfy.mapping?.output||'';s.settings.comfy.randomizeSeeds??=false;s.settings.comfy.objectInfo??={};s.settings.comfy.slots??={};if(!Array.isArray(s.settings.comfy.slotPresets))s.settings.comfy.slotPresets=[];if(!s.settings.comfy.modelCatalog||typeof s.settings.comfy.modelCatalog!=='object')s.settings.comfy.modelCatalog={checkpoints:[],unets:[],loras:[],vaes:[],fetchedAt:0};
