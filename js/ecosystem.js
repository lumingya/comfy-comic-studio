...
function macroEditor(owner,key){
  const p=settingsTargetById(owner),e=p&&mergedSettingEntries(p).find(v=>v.key===key);if(!e)throw Error('变量不存在。');rt.ecoMacro={owner,key,id:e.id};
...
async function prepareComputedPlan(source){
  if(Object.values(source.sceneOverrides||{}).some(o=>[...(o.variables||[]),...(o.variableSetIds||[]).flatMap(id=>setBy(id)?.entries||[])].some(e=>e.compute)))throw Error('可执行变量应放在本册变量中；单幕仅支持字面量覆盖。');
...
function installEcosystem(){
  ...
  Object.assign(v3Actions,{
    ...
    'eco-extension-remove':d=>{rt.ecoRemoveId=d.id;const p=ecoState.status.extensions.find(x=>x.id===d.id);modal(p?.source==='link'?'取消链接扩展':'卸载扩展',`<p>${p?.source==='link'?'链接将被移除，你的文件夹原样保留。':'代码仓库将移除。'}选择保留哪些分层数据：</p><p class="soft small">当前占用：${ecoUsageLabel(p?.data)}</p><label class="row"><input type="radio" name="eco-purge" value="none" checked>保留配置与工作区数据</label><label class="row"><input type="radio" name="eco-purge" value="cache">同时清理缓存与临时文件</label><label class="row"><input type="radio" name="eco-purge" value="all">删除全部数据（含已安装的 pip 依赖，不可恢复）</label><div class="modal-footer">${btn('取消','','close-modal','','ghost')}${btn(p?.source==='link'?'确认取消链接':'确认卸载','trash','eco-extension-remove-confirm','','danger')}</div>`)},
    ...
    'eco-macro-save':()=>{
      const m=rt.ecoMacro,p=settingsTargetById(m.owner),entries=p&&mergedSettingEntries(p),e=entries?.find(x=>x.key===m.key&&x.id===m.id);if(!e)throw Error('变量已变化。');
    ...
    'eco-cache-clear':async()=>{if(await confirmAction('清空前置变量缓存？','会移除成功结果和未确认标记，后续准备可能重新调用模型并计费。请确认没有外部请求仍在运行。','确认清空')){await ecoRequest('cache/clear',{trusted:true});toast('前置缓存已清空。')}},
