...
function productionCostNote(tasks,{forcePrepare=false,indices=false}={}){
 ...
 return local?'将通过本地 ComfyUI 开始生成，占用显卡但不产生费用。':'将开始生成，云端服务可能产生费用。';
}
...
async function openProductionFrameEditor(id,index){
 ...
 modal(localeString('第 {n} 幕 · 编辑提示词',{n:index+1}),`<p class="help" style="margin-bottom:12px">${esc('修改会覆写这个生成任务自己的分镜快照，其他任务不受影响。')}</p>
...
function installAssemblyWorkshop(){
 ...
 Object.assign(v3Actions,{
  ...
  'workshop-frame-delete-bulk':async()=>{
    ...
    if(!await confirmAction(localeString('删除选中的 {n} 幕？',{n:count}),'只修改分镜，不影响已装配的任务；删除后可撤销。','删除'))return;
