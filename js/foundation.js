...
const foundationRuntime={activityAfter:0,diagnosticKey:"",polling:false,submitting:null,enabled:true,runtimeLoaded:false,runtime:{concurrency:1,requestTimeoutSeconds:600},runningCount:0,failurePolicy:{mode:'retry',maxRetries:5,delaySeconds:15,onExhausted:'continue',maxConsecutiveFailures:5}};
function foundationTaskIsMock(q){if(q.serverId||q.serverInput)return false;return (q.indices||[]).every(i=>{const ex=q.frames?.[i]?._execution||q.execution||bookBy(q.bookId)?.sourceSnapshot?.execution||mappedExecutionSnapshot();return (!ex.provider||ex.provider==='comfyui')&&ex.mode==='mock'})}
function foundationIsMock(){const active=state.queue.filter(q=>['pending','running','paused'].includes(q.status)||q.serverAwaitingRelease);return active.length?active.every(foundationTaskIsMock):state.settings.comfy.mode==='mock'&&activeImageProfile().provider==='comfyui'}
...
async function submitFoundationQueue(){
  ...
  foundationRuntime.submitting=(async()=>{
    const requested=state.queue.filter(q=>!foundationTaskIsMock(q)&&(q.status==='pending'||q.serverAwaitingRelease));
...
async function pollFoundationJobs(){
  if(document.hidden||rt.booting||foundationRuntime.polling||!foundationRuntime.enabled)return;
  foundationRuntime.polling=true;
  try{
    const result=await foundationRequest('jobs');foundationRuntime.connectionError='';foundationRuntime.connectionWarned=false;if($('#queue-service-status'))$('#queue-service-status').hidden=true;const deleted=applyDeletedAlbums(result.deletedAlbumIds);if(foundationIsMock()){if(deleted){save(true);render()}return}let changed=deleted||rt.paused!==result.paused;
    ...
    for(const q of state.queue){let job=result.jobs.find(j=>j.id===q.serverId||j.owner===q.id);if(!job||job.state==='archived'||foundationRuntime.deletedAlbums?.has(q.bookId)||q.serverUpdated>job.updated)continue;if(!q.serverId){q.serverId=job.id;changed=true}
      const b=bookBy(q.bookId);if(!b)continue;
      const projectedDone=q.indices.length-(q.serverIndices||q.indices).length+job.cursor;
      const visibleDone=b._lazy?job.cursor:(q.serverIndices||q.indices).filter(index=>b.pictureEdits?.[index]||b.steps.some(s=>s.stepIndex===index&&s.image)).length;
      if(projectedDone>q.done||job.cursor>visibleDone||(job.error&&q.serverUpdated!==job.updated))job=await foundationRequest('jobs/'+job.id);
      if(foundationRuntime.deletedAlbums?.has(q.bookId)||!state.books.includes(b))continue;
      if(job.error?.message==='Open details for the original response')job.error={message:q.error};
      q.serverUpdated=job.updated;
      for(const [field,key] of [['retry_count','serverRetries'],['ready_at','serverReadyAt'],['attempts','serverAttempts'],['cancel','serverCancel'],['cursor','serverCursor'],['active_timeout','serverTimeout'],['nextIndex','serverNextIndex'],['running_count','serverRunningCount'],['enabled','serverEnabled'],['blocked','serverBlocked'],['consecutive_failures','serverConsecutiveFailures'],['failure_limit_reached','serverFailureLimit']])if(q[key]!==job[field]){q[key]=job[field];changed=true}
      if(JSON.stringify(q.serverActions)!==JSON.stringify(job.allowedActions)){q.serverActions=job.allowedActions;changed=true}
      q.serverRequiresRecoveryConsent=job.requiresRecoveryConsent;
      ...
      if(b._lazy&&job.results.length){await globalThis.ComfyComic.fileLibrary.hydrate(b.id);if(!state.books.includes(b))continue}
      for(const item of job.results){const index=(q.serverIndices||q.indices)[item.index],f=q.frames[index];if(!f||b.pictureEdits?.[index])continue;const existing=b.steps.find(s=>s.stepIndex===index);
      ...
      if(!b._lazy){b.steps.sort((a,b)=>a.stepIndex-b.stepIndex);b.generatedSteps=b.steps.filter(s=>s.image).length}b.inProgress=job.state==='running';b.status=job.state==='complete'?(missingIndices(b).length?'partial':'complete'):['pending','running','paused'].includes(status)?'generating':status;
    }
...
async function openFoundationAssets(){
  ...
  modal('素材索引与完整性',`<p class="help">${report.items.length} 个文件 · ${report.missing.length} 个缺失引用。未引用文件至少保留 24 小时；清理只移进回收站（设置 → 数据与备份 → 回收站），可以恢复。运行中或结果未确认的任务会阻止清理。</p><div style="max-height:55vh;overflow:auto">${report.missing.map(x=>`<p class="danger">缺失：${esc(x.url)}</p>`).join('')}${report.items.map(x=>`<details><summary>${esc(x.name)} · ${(x.bytes/1024).toFixed(1)} KiB · ${x.references.length} 处引用 ${x.integrity==='ok'?'':'· 校验不一致'}</summary><code>${esc(x.url)}</code><pre>${esc(x.references.join('\n')||'未被当前工程或任务引用')}</pre>${x.cleanupEligible?`<label><input type="checkbox" data-recycle-asset="${esc(x.url)}">移进回收站</label>`:''}</details>`).join('')}</div><div class="modal-footer">${btn('关闭','','close-modal')}${btn('回收所选文件','trash','foundation-cleanup',`data-token="${report.cleanup.token}"`,'small')}</div>`);
}
...
async function foundationAction(action,d={}){
    ...
    if(action==='foundation-reconcile'){const q=state.queue.find(x=>x.id===d.id);if(!q?.serverId)return toast('请在原服务查看已有结果。');await foundationRequest('jobs/'+q.serverId,{action:'reconcile'});return pollFoundationJobs()}
    if(action==='foundation-jobs')return openFoundationJobs();
    if(action==='foundation-job-detail')return openFoundationJobs(d.job);
    if(action==='foundation-job-control'){if(d.operation==='remove'){const q=state.queue.find(q=>q.serverId===d.job);if(q&&bookBy(q.bookId))await deleteBooks([q.bookId]);else{if(!await confirmAction('删除任务及对应画册？','停止本地跟踪并移除这条服务端任务记录；不能取消上游请求或退款。任务记录移除后不能恢复。','停止并删除'))return;await foundationRequest('jobs/'+d.job,{action:'remove'});await pollFoundationJobs()}return openFoundationJobs()}if(d.operation==='continue'){await continueServiceJob(d.job);return openFoundationJobs()}if(['cancel','archive','retry'].includes(d.operation)&&!await confirmAction('确认任务操作？','停止本地执行并保留已完成图片；不能取消上游请求或退款。',({cancel:'停止任务',archive:'隐藏记录',retry:'重试'})[d.operation]||'确认'))return;await foundationRequest('jobs/'+d.job,{action:d.operation});await pollFoundationJobs();return openFoundationJobs()}
    if(action==='foundation-assets')return openFoundationAssets();
    if(action==='foundation-cleanup'){const paths=$$('[data-recycle-asset]:checked').map(el=>el.dataset.recycleAsset);if(!paths.length)return toast('先选择文件。');if(!await confirmAction('回收所选素材？','仅处理未引用且超过 24 小时的文件，移进回收站，不永久删除；可以在 设置 → 数据与备份 → 回收站 恢复。','移进回收站'))return;await foundationRequest('assets/cleanup',{token:d.token,paths});return openFoundationAssets()}
}
...
function applyDeletedAlbums(ids=[]){
  ...
  const deleted=new Set(ids);foundationRuntime.deletedAlbums??=new Set();ids.forEach(id=>foundationRuntime.deletedAlbums.add(id));
  const changed=state.books.some(b=>deleted.has(b.id))||state.queue.some(q=>deleted.has(q.bookId));
  for(const [key,controller] of rt.refineControllers||[])if(ids.some(id=>key.startsWith(id+':')))controller.abort();
  state.books=state.books.filter(b=>!deleted.has(b.id));state.queue=state.queue.filter(q=>!deleted.has(q.bookId));
  for(const id of ids)ui.selected.delete(id);
  if(ui.exportIds?.some(id=>deleted.has(id)))studioUI.exportController?.abort();
  if(deleted.has(ui.bookId)&&$('#reader')?.open)closeReader();
  return changed;
}
...
async function saveQueuePolicy(){
  ...
  if(!Number.isInteger(policy.maxConsecutiveFailures)||policy.maxConsecutiveFailures<0||policy.maxConsecutiveFailures>100||!$('#queue-policy-consecutive').value.trim())throw Error('连续失败暂停上限须为 0–100 的整数；0 表示关闭。');
  if(policy.mode==='retry'&&!await confirmAction('启用可能产生额外费用的重试？',`每幕最多额外请求 ${policy.maxRetries} 次，每次等待 ${policy.delaySeconds} 秒。${policy.maxConsecutiveFailures?'连续失败 '+policy.maxConsecutiveFailures+' 次会暂停当前画册。':'连续失败暂停保护已关闭。'}即使接口报错，也可能已经计费。结果不明不自动重发。此策略用于整个服务端队列。`,'确认启用有限重试'))return;
  const feedback=$('#queue-policy-feedback');if(feedback)feedback.textContent=' 正在保存…';
  try{
    const data=await foundationRequest('jobs/scheduler',{action:'policy',policy});foundationRuntime.failurePolicy=data.failurePolicy;
    if($('#queue-policy')){
      const values={mode:$('#queue-policy-mode').value,onExhausted:$('#queue-policy-exhausted').value,maxRetries:Number($('#queue-policy-count').value),delaySeconds:Number($('#queue-policy-delay').value),maxConsecutiveFailures:Number($('#queue-policy-consecutive').value)};
      if(JSON.stringify(values)===JSON.stringify(policy)){$('#queue-policy').outerHTML=queuePolicyHTML();$('#queue-policy').open=true;$('#queue-policy-feedback').textContent=' 已保存到服务端'}
      else $('#queue-policy-feedback').textContent=' 上次提交已保存；当前修改尚未保存';
    }
  }catch(error){if($('#queue-policy-feedback'))$('#queue-policy-feedback').textContent=' 保存未确认，请检查错误后重试';throw error}
  await pollFoundationJobs();
}
function queueNextFrame(q){return (q.serverIndices||q.indices)?.[q.serverNextIndex??q.serverCursor??q.done]??q.indices?.[q.done]}
function queueCanContinue(q){if(q.serverActions)return !!q.serverId&&q.serverActions.includes('continue');return !!q.serverId&&!q.serverRunningCount&&(['failed','unknown','canceled','paused'].includes(q.serverState)||q.serverReadyAt>0)&&Number.isInteger(queueNextFrame(q))}
...
async function continueServiceJob(id,q=null){
  ...
  if(job.state==='pending'&&job.allowedActions?.includes('continue')){job=await foundationRequest('jobs/'+id,{action:'hold'})}
...
function queueErrorSummary(q){
  ...
  const failed=(q.serverFrameStates||[]).find(f=>['failed','skipped','unknown'].includes(f.state)),index=failed?(q.serverIndices||q.indices)?.[failed.index]:null,prefix=Number.isInteger(index)?'第 '+(index+1)+' 幕：':'';
  if(q.serverState==='canceled')return '已停止。已完成图片保留；可继续未完成分镜，也可直接删除。';
  if(q.serverState==='unknown')return prefix+'请求结果不明，不自动重发。可核对已有结果或确认重复计费风险后继续。';
