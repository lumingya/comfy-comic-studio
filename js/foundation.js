...
async function pollFoundationJobs(){
  if(rt.booting||foundationRuntime.polling||!foundationRuntime.enabled)return;
...
function queuePolicyHTML(){
  ...
  return `<details class="queue-policy" id="queue-policy"><summary>失败策略 <strong>${esc(name)}</strong><span>调整策略</span></summary><p class="help">每幕重试单独计数；连续失败跨分镜累计，保存后生效。失败或暂停的任务会自动让出顺序位置，不必手动放行其他画册。没有明确 HTTP 状态的未知结果不会被悄悄重发。</p><div class="grid2">${field('发生明确失败时',`<select id="queue-policy-mode">${opt('pause','暂停本任务后续分镜，不影响其他任务',p.mode)}${opt('retry','自动重试 5xx（默认），耗尽后按下方策略处理',p.mode)}${opt('continue','不自动重试，保留失败幕并继续其他分镜',p.mode)}</select>`)}${field('重试耗尽 / 不可自动重试时',`<select id="queue-policy-exhausted" ${p.mode==='retry'?'':'disabled'}>${opt('pause','暂停本任务',p.onExhausted)}${opt('continue','保留失败幕，其他分镜继续',p.onExhausted)}</select>`)}${field('每幕自动重试上限（1–100）',`<input id="queue-policy-count" ${p.mode==='retry'?'':'disabled'} type="number" min="1" max="100" step="1" value="${p.maxRetries}">`)}${field('每次重试前等待秒数（5–300）',`<input id="queue-policy-delay" ${p.mode==='retry'?'':'disabled'} type="number" min="5" max="300" step="1" value="${p.delaySeconds}">`)}${field('连续失败暂停上限（0–100）',`<input id="queue-policy-consecutive" type="number" min="0" max="100" step="1" value="${p.maxConsecutiveFailures??5}">`)}</div><p class="help">跨分镜累计首次请求和每次重试的失败，成功一次清零。达到上限后暂停当前画册的重试及后续请求，其他画册继续；0 表示关闭保护。在途请求仍保留结果，但不会自动解除暂停，需要手动继续。</p><p class="help">所有 HTTP 5xx 可自动重试；4xx 不自动重试。422 不自动重试，未达到连续失败上限时跳过并继续。每幕重试上限只限制自动重试，手动重试无次数上限。</p>${btn('应用失败策略','check','queue-policy-save','','small')}<span class="help" id="queue-policy-feedback" role="status"></span></details>`;
}
