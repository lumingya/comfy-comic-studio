...
async function startUpdateApply(version){
  ...
  const lines=[`将下载 ${check.latest.asset?.name||'程序包'}，校验 SHA-256 后替换程序文件。`,'你的数据目录（画册、分镜、预设、设置、密钥）不会被修改；被替换的旧程序文件会先备份，可一键回退。'];
...
async function rollbackUpdate(){
  ...
  if(!await confirmAction(`回退到 v${last.from}？`,'恢复更新前备份的程序文件，数据目录不受影响。回退后需要重启。','回退'))return;
...
function renderAboutSettings(){
  ...
  const dataLine=facts?`<div class="service-context"><strong>安装位置</strong><br>程序目录：<code>${esc(facts.programDir)}</code><br>数据目录：<code>${esc(facts.dataDir)}</code>${facts.dataInsideProgram?'（位于程序目录内，更新时受保护，不会被覆盖或删除）':'（独立于程序目录）'}<br>安装方式：${esc(facts.kind==='git'?'Git 检出 · 建议用 git pull 更新，也可在此安装发布包':facts.kind==='frozen'?'打包的可执行程序':'发布包 / 源码目录')}</div>`:'';
