...
function flushCreationEditor(){
  ...
  if(f){
    ...
    const target=live||(own?p.sceneOverrides[f.id]:f);
    $$('[data-v3-frame]').forEach(el=>{const key=el.dataset.v3Frame,value=el.type==='number'?Number(el.value):el.value;if(target[key]!==value){target[key]=value;changed=true}});
