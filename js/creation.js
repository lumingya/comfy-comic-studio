...
const step={stepIndex:i,name:f.name,prompt:interpolate(f.prompt,row,f),caption:story?.captions[f.id]??interpolate(f.caption,row,f),image:result.image,offlineFallback:result.offlineFallback},old=b.steps.findIndex(s=>s.stepIndex===i);
...
const frames=t.frames.map(clone);const chunk=clamp(Number(state.settings.llm.chunkSize)||3,2,6),start=single===null?0:single,end=single===null?frames.length:single+1;
for(let i=start;i<end;i+=chunk){const count=Math.min(single===null?chunk:1,end-i);let lines;
if(llmMock()){await delay(650,signal);lines=Array.from({length:count},(_,k)=>{const n=i+k,f=sceneFor(frames[n]);return interpolate(f.caption,runtime,f)+' '+['她终于明白，有些相遇不需要答案。','风带来了远方的消息，也带来了勇气。','在这一刻，所有等待都有了意义。'][n%3]})}
else{const msg=await chatCompletion([{role:'system',content:'你是漫画编剧。只返回 JSON 对象 {"captions":["逐幕中文台词"]}。旁白须推进情节并保持角色、服装、时空一致。使用 character_display_name 称呼角色；character 仅为出图识别标签，不要把标签写入旁白。展示名为空时使用自然代词。每幕提供的展示名优先于全局名称。'},{role:'user',content:JSON.stringify({character_display_name:identity.displayName,character:identity.promptName,outline:row.storyOutline,tone:row.tone,previousCaptions:frames.slice(0,i).map(f=>v.captions[f.id]??''),startScene:i+1,count,scenes:frames.slice(i,i+count).map(sceneFor).map(f=>({name:f.name,character_display_name:resolveCharacterNames(f._scope||runtime._scope||runtime).displayName,prompt:interpolate(f.prompt,runtime,f)}))})}],null,signal);lines=parseJson(msg.content).captions;if(!Array.isArray(lines)||lines.length!==count||lines.some(x=>typeof x!=='string'))throw Error('LLM 台词数量或类型不符合本批幕数。')}
lines.forEach((line,k)=>v.captions[frames[i+k].id]=line);v.updatedAt=Date.now();save();log('剧情已保存：'+row.character+' / '+Math.min(i+count,end)+' / '+t.frames.length+' 幕');if($('#story-progress'))$('#story-progress').textContent='已保存 '+Math.min(i+count,end)+' / '+t.frames.length+' 幕';
}return v;
...
try{for(const row of rows){await generateStory(row,t,null,true);if(rt.llmController.signal.aborted)break}toast('版本已保存至角色矩阵。')}
...
async function sendScopedChat(){
  ...
  const content=attachments.length?[{type:'text',text:value||'请参考这些附件，帮助我调整当前分镜。'},...attachments.map(a=>a.image?{type:'image_url',image_url:{url:a.image}}:{type:'text',text:'附件 '+a.name+':\n'+a.text})]:value;
...
function effectivePlanFrame(plan,frame){const override=planFrameOverrides(plan,frame),f={...clone(frame)};if(plan.storyVersionId){const version=rowBy(plan.rowId)?.storyVersions?.[plan.templateId]?.find(v=>v.id===plan.storyVersionId);if(version?.captions[frame.id]!==undefined)f.caption=version.captions[frame.id]}for(const key of ['name','prompt','caption','negative','renderOverride','width','height','steps','cfg','denoise','seed'])if(Object.hasOwn(override,key))f[key]=clone(override[key]);f._captionOverride=Object.hasOwn(override,'caption');f._scope=effectivePlanScope(plan,frame).values;f._scopeResolved=true;f._execution=plan._creationTemplate&&frame._execution?clone(frame._execution):workflowExecutionFor(plan,frame);const imageBindings=resolveImageVariables(f.prompt,f._scope,false,f.negative||f._execution.globalNegative||'');f._imageInputs=imageBindings.images;f._resolvedImagePrompt=imageBindings.prompt;f._resolvedImageNegative=imageBindings.negative;f.nodeOverrides={...(frame.nodeOverrides||{}),...(override.nodeOverrides||{})};return f}
...
function flushCreationEditor(){
  const p=selectedPlan(),t=currentTemplate(),task=createUI.sceneScope==='plan'?liveStoryboardTask(p):null,f=task?.frames[ui.frameIndex]||t?.frames[ui.frameIndex];let changed=false,templateChanged=false;
  if(f){
    const own=!!task||createUI.sceneScope==='plan'&&p?.templateId===t?.id,live=own?liveStoryboardFrame(p,f):null;
    if(own&&!live)p.sceneOverrides[f.id]??={};
    const target=live||(own&&t.ownerPlanId!==p.id?p.sceneOverrides[f.id]:f);
    $$('[data-v3-frame]').forEach(el=>{const key=el.dataset.v3Frame,value=el.type==='number'?Number(el.value):el.value;if(target[key]!==value){target[key]=value;changed=true}if(target===f&&own&&p.sceneOverrides[f.id])delete p.sceneOverrides[f.id][key]});
    if(changed){if(live)saveLiveStoryboardFrame(p,live);else if(own)p.updatedAt=Date.now();else templateChanged=true}
  }
  if(t)$$('[data-v3-template]').forEach(el=>{const key=el.dataset.v3Template;if(t[key]!==el.value){t[key]=el.value;changed=templateChanged=true}});
  if(changed){if(t&&templateChanged)t.updatedAt=Date.now();save()}
}


function enqueuePlanSnapshot(plan,existing=null,selectedIndices=null){
  if(existing&&state.queue.some(q=>q.bookId===existing.id&&q.serverState==='unknown'))throw Error('此画册有结果未确认的请求，请使用原任务的“继续未完成分镜”确认重发风险。');
  flushEditor();const t=plan._creationTemplate||templateBy(plan.templateId);if(!t)throw Error('计划「'+plan.title+'」尚未选择有效分镜。');const row=planRuntimeRow(plan),frames=t.frames.map(f=>effectivePlanFrame(plan,f)),execution=mappedExecutionSnapshot();
  const report={missing:[]};
  for(const f of frames){resolveImageVariables(f.prompt,f._scope,true,f.negative||f._execution?.globalNegative||'',report);for(const key of missingVariableNames(f.caption,f._scope))if(!report.missing.includes(key))report.missing.push(key);validatePrompt(f.prompt);scopeText(f.prompt,f._scope,true);scopeText(f.caption,f._scope,true);buildMappedWorkflow(f,row,{execution:f._execution||execution,preview:true})}
  if(report.missing.length)toast(missingVariablesNotice(report.missing),'warn');
  const source={planId:plan.id,row:clone(row),frames:clone(frames),execution,templateTitle:t.title};
  let book=existing;const v=plan.storyVersionId?row.storyVersions?.[t.id]?.find(v=>v.id===plan.storyVersionId):null;if(plan.storyVersionId&&!v)throw Error('选中的版本不存在，请重新选择台词来源。');if(!book){book={id:uid('book'),projectId:plan.projectId,title:row.bookTitle,characterName:resolveCharacterNames(row).displayName||'原创画册',rowId:plan.rowId,templateId:t.id,templateTitle:t.title,storyVersionId:v?.id,storyTitle:v?.title,synopsis:t.outline||'使用分镜与变量素材生成的连续画册。',tags:['原创','创作计划'],totalSteps:frames.length,generatedSteps:0,status:'generating',inProgress:false,liked:false,likes:0,createdAt:Date.now(),updatedAt:Date.now(),theme:hash(row.character||plan.title)%6,steps:[],planId:plan.id,sourceSnapshot:source};state.books.push(book)}
  if(state.queue.some(q=>q.bookId===book.id&&['pending','running','paused'].includes(q.status)))return book;
  const indices=missingIndices(book).filter(i=>!selectedIndices||selectedIndices.includes(i));if(indices.length){const snapshot=book.sourceSnapshot||source;state.queue.push({id:uid('task'),bookId:book.id,rowId:book.rowId,templateId:book.templateId,planId:plan.id,indices,done:0,status:'pending',frames:clone(snapshot.frames),rowSnapshot:clone(snapshot.row),execution:clone(snapshot.execution),waitForStory:!!plan.storyVersionId&&rt.lockedRows.has(book.rowId),createdAt:Date.now()});book.status='generating';log('计划入队：'+book.title+' · '+indices.length+' 帧 · 工作流已绑定，未发送分镜读取本册最新保存输入')}
...
async function runFlexibleQueue(){
  ...
  try{let q;while((q=state.queue.find(q=>q.status==='pending'&&foundationTaskIsMock(q)))){while(rt.paused&&!signal.aborted)await delay(200);if(signal.aborted)break;q=state.queue.find(q=>q.status==='pending'&&foundationTaskIsMock(q));if(!q)break;const b=bookBy(q.bookId),storedRow=rowBy(q.rowId);if(!b){q.status='failed';continue}q.status='running';delete q.error;b.inProgress=true;log('开始生成 '+b.title);
    try{while(rt.lockedRows.has(q.rowId)){log('等待此计划的剧情推演完成...');await delay(1000,signal)}const row=clone(q.rowSnapshot||storedRow||b.sourceSnapshot?.row),t=templateBy(q.templateId);if(!row)throw Error('缺少原始变量快照。');if(q.waitForStory&&storedRow){row.storyVersions=clone(storedRow.storyVersions);row.activeStoryVersionIds=clone(storedRow.activeStoryVersionIds);const v=activeVersion(row,q.templateId);b.storyVersionId=v?.id;b.storyTitle=v?.title}
      ...
      for(let pos=q.done;pos<q.indices.length;pos++){if(!bookBy(b.id)||!state.queue.includes(q))break;while(rt.paused){q.status='paused';updateQueueUI();await delay(350,signal)}if(!bookBy(b.id)||!state.queue.includes(q))break;q.status='running';const i=q.indices[pos],original=q.frames?.[i]||b.sourceSnapshot?.frames?.[i]||t?.frames[i];if(!original)throw Error('缺少第 '+(i+1)+' 幕快照。');if(!missingIndices(b).includes(i)){q.done=pos+1;continue}
