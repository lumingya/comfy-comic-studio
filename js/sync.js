...
async function buildDiskPackage(source,progress=()=>{},options={}){
  ...
  for(const group of ['books','rows','templates'])for(const entity of snapshot[group]){entity.projectId ||= snapshot.activeProjectId;if(!snapshot.projects.some(p=>p.id===entity.projectId))throw Error('资产「'+(entity.title||entity.character||entity.id)+'」的画册集不存在，未保存。')}
  ...
  const externalize=async(value,base)=>{if(value?.kind==='image'&&typeof value.url==='string'&&value.url.startsWith('/images/')){const stored=await asset(base,'result',value.url);return{...value,url:{__cc_asset:stored.path}}}if(isImageVariable(value)&&value.src){const stored=await asset(base,'variable',value.src);return{...value,src:{__cc_asset:stored.path}}}if(typeof value==='string'&&(value.startsWith('data:image/')||value.startsWith('/images/'))){const stored=await asset(base,'asset',value);return {__cc_asset:stored.path}}if(Array.isArray(value)){const out=[];for(const item of value)out.push(await externalize(item,base));return out}if(value&&typeof value==='object'){const out={};for(const[k,v]of Object.entries(value)){if(['__proto__','prototype','constructor'].includes(k))continue;out[k]=await externalize(v,base)}return out}return value};
  ...
  if(manifest.projects.reduce((n,p)=>n+snapshot.books.filter(b=>b.projectId===p.id).length,0)!==snapshot.books.length)throw Error('有画册缺少有效画册集归属，未提交磁盘工程。');
...
async function loadDiskManifest(readFile,rootText){
  ...
  const readJSON=async path=>{checkedPath(path);const f=await readFile(path);if(f.size>100000000)throw Error('JSON 文件超过 100 MB：'+path);return JSON.parse(await f.text())};
  const readImage=async path=>{checkedPath(path);const f=await readFile(path);if(f.size>200000000)throw Error('单张图片超过 200 MB：'+path);return blobData(f.type?f:new Blob([f],{type:path.endsWith('.svg')?'image/svg+xml':path.endsWith('.jpg')?'image/jpeg':'image/png'}))};
...
async function attachWorkspaceDirectory(handle,auto=false){
  ...
  try{
    ...
    if(!auto)toast(loaded?'磁盘工作室已载入，画册集和图片已恢复。':'目录已连接，完整工程已分层写入磁盘。');
    return true;
...
async function zipDirectory(files){
  ...
  for(const[path,blob]of files){const name=new TextEncoder().encode(path),bytes=new Uint8Array(await blob.arrayBuffer()),crc=crc32(bytes),size=bytes.length;if(offset+size>2000000000)throw Error('目录包超过 2 GB，请改用直接连接本地目录。');const header=new Uint8Array(30+name.length),v=new DataView(header.buffer);v.setUint32(0,0x04034b50,true);v.setUint16(4,20,true);v.setUint16(6,0x800,true);v.setUint16(10,time,true);v.setUint16(12,date,true);v.setUint32(14,crc,true);v.setUint32(18,size,true);v.setUint32(22,size,true);v.setUint16(26,name.length,true);header.set(name,30);local.push(header,bytes);
...
async function savePythonWorkspace(allow=false){
  ...
  backendRuntime.tail=backendRuntime.tail.catch(()=>{}).then(async()=>{
    ...
    }catch(e){backendRuntime.error=e.message;backendRuntime.dirty=true;log('保存未完成：'+e.message,'error');toast('保存未完成：'+e.message,'error');return false}finally{backendRuntime.saving=false;renderStatus();const status=$('#v3-backend-status');if(status)status.textContent=backendStatusText()}
  });return backendRuntime.tail;
}


/* B22: the status bar speaks to creators, not to the people who wrote the backend. */
function backendStatusText(){if(backendRuntime.loading)return'正在打开工作室…';if(backendRuntime.saving)return'正在保存…';if(backendRuntime.error)return'保存未完成 · 点击查看';if(backendRuntime.connected)return backendRuntime.dirty?'有待保存修改':'已保存'+(backendRuntime.savedAt?' · '+new Date(backendRuntime.savedAt).toLocaleTimeString('zh-CN'):'');return'尚未连接保存服务 · 当前为临时会话'}
...
function createNativeConfigSync(options){
  ...
  async function read(){runtime.loading=true;runtime.error='';notify();try{const response=options.initialConfig?{ok:true,json:async()=>{const c=options.initialConfig;options.initialConfig=null;return c}}:await getFetch()(options.loadURL||'/api/config',{method:'GET',credentials:'same-origin',cache:'no-store',headers:{Accept:'application/json'}});if(!response.ok)throw Error('GET /api/config: HTTP '+response.status);const config=await response.json(),validated=options.convert.fromApi(config,options.defaults()),converted=validated;const deleted=new Set(config._deletedAlbumIds||[]);converted.books=converted.books.filter(b=>!deleted.has(b.id));converted.queue=converted.queue.filter(q=>!deleted.has(q.bookId));runtime.previous=options.contract.copy(config);runtime.loaded=true;runtime.savedAt=Number(config.updatedAt)||null;return converted}catch(e){runtime.error=e.message;throw e}finally{runtime.loading=false;notify()}}
  ...
  function save(force=false){
    ...
    runtime.tail=runtime.tail.catch(()=>false).then(async()=>{
      ...
      try{options.beforeSnapshot?.();const studio=options.getState();options.validate(studio);const payload=options.convert.toApi(studio,runtime.previous,forceSerial>0),response=await getFetch()('/api/config',{method:'POST',credentials:'same-origin',headers:{'Content-Type':'application/json',Accept:'application/json'},body:JSON.stringify(options.prepare?options.prepare(studio,payload,runtime.previous):payload)});if(!response.ok){let detail='';try{const problem=await response.json();detail=typeof problem.error==='string'?problem.error:''}catch(_){}if(response.status===409){try{const fresh=await(await getFetch()(options.loadURL||'/api/config?summaries=1',{credentials:'same-origin',cache:'no-store',headers:{Accept:'application/json'}})).json();if(fresh._fileRevisions)runtime.previous._fileRevisions={...runtime.previous._fileRevisions,...fresh._fileRevisions}}catch(_){}}throw Error((response.status===409?'文件版本冲突；本页草稿未覆盖磁盘。':'保存失败（HTTP '+response.status+'）。')+detail)};if(response.status!==204){const text=await response.text();if(text.trim()){let data;try{data=JSON.parse(text)}catch(e){throw Error('The config endpoint returned non-JSON content.')}if(data.ok===false||data.status==='error'||data.success===false)throw Error(data.message||data.error||'The backend rejected the save.');if(Number.isFinite(data.revision))payload.updatedAt=data.revision;if(data.revisions)payload._fileRevisions={...runtime.previous._fileRevisions,...data.revisions};options.ack?.(studio,payload,data)}}runtime.previous=payload;runtime.committed=serial;if(runtime.forceSerial===forceSerial)runtime.forceSerial=0;runtime.dirty=runtime.serial!==serial;runtime.savedAt=payload.updatedAt;return true}
