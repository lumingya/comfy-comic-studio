...
function settingsMoveToast(p,moved,groupId){if(moved)toast(groupId?`已将 ${moved} 项属性移到「${settingsGroupTitle(p,groupId)}」。`:`已将 ${moved} 项属性移出分组。`)}
...
function openSettingMetadata(owner,key){
  ...
  modal('编辑属性',`${field('显示名称',input('label',entry.label||settingLabel(entry),'text','id="setting-meta-label" maxlength="80"'))}${field('变量标识符',input('key',key,'text','id="setting-meta-key" maxlength="60" spellcheck="false" autocomplete="off" placeholder="例如：weapon"'))}<p class="help">提示词与台词里用 <code>{${esc(key)}}</code> 引用这个属性；只能包含中英文字母、数字和下划线。</p><label class="row setting-meta-rewrite"><input type="checkbox" id="setting-meta-rewrite" checked><span>改名时同步替换本画册集分镜里的 <code>{${esc(key)}}</code> 引用${references?`（${references} 处）`:''}</span></label>${field('归属分组',`<select id="setting-meta-group">${settingsGroupOptions(p,entry.groupId)}</select>`)}<p class="help">调整分组只影响整理方式，不改变提示词与属性值。</p><div class="modal-footer">${btn('取消','','close-modal','','ghost')}${btn('保存修改','check','settings-meta-save','','primary')}</div>`);
}
...
function renameSettingKey(p,entries,entry,newKey,rewrite){
  ...
  if(entries.some(e=>e!==entry&&e.key===newKey))throw Error('已有同名属性，请换一个标识符。');
  entry.key=newKey;
...
function installSettingsWorkbench(){
  ...
  Object.assign(v3Actions,{
    ...
    'settings-group-delete':async d=>{
      ...
      if(!await confirmAction('删除分组「'+g.title+'」？','组内属性会回到平铺列表，属性值、参考图片和提示词引用全部保留。','删除分组'))return;
    ...
    'settings-pick-move':d=>{const p=settingsTargetById(d.owner);if(!p)return;const groupId=document.querySelector(`[data-pick-target="${CSS.escape(p.id)}"]`)?.value||'',keys=settingsPickedKeys(p);if(!keys.length)throw Error('请先勾选要移动的属性。');const moved=moveSettingEntries(p,keys,groupId);render();settingsMoveToast(p,moved,groupId)},
