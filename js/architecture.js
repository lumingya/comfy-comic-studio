...
function designerSeedHTML(){const d=assemblyDesign,ready=designerSeedReady(),on=d.seedEnabled&&ready;return `<div class="seed-setting ${ready?'':'is-unmapped'}"><div class="seed-setting-row"><label class="row seed-setting-switch"><span class="switch"><input id="designer-seed-enable" type="checkbox" role="switch" aria-label="启用可复现种子" ${on?'checked':''} ${ready?'':'disabled'}><span class="switch-track"></span></span><span class="seed-setting-label">可复现种子<small>${ready?(on?'按固定种子 + 幕序号生成':'关闭时每幕使用新的随机种子'):'当前工作流没有种子映射'}</small></span></label><input id="designer-seed" type="number" min="0" max="4294967295" value="${d.seed}" ${on?'':'disabled'} aria-label="可复现种子"></div><p class="help">${ready?'已找到有效种子映射。关闭时每幕、每次重跑都使用新的随机种子；开启后按固定种子 + 幕序号生成，可复现。':'未找到种子映射：所有分幕会沿用工作流中的固定种子，画面可能雷同。请在「工作流与 API 配置」中点击「添加分镜参数映射」，或手动把采样器的 seed 映射为分镜参数 seed / 每次生成新种子。'}</p></div>`}
/* Cloud channels have no workflow to pick; show what the request will go to instead of an empty card. */
function designerChannelFactsHTML(p){if(!p)return '';const issues=providerSetupIssues(p),labels={novelai:'NovelAI 云端渠道',openai:'OpenAI 兼容图像接口'};let host='';try{host=p.baseUrl?new URL(p.baseUrl).host:''}catch{host=String(p.baseUrl||'')}
 const fact=(label,value,cls='')=>`<div class="designer-fact ${cls}"><small>${label}</small><span>${value}</span></div>`;
 return `<div class="designer-facts">${fact('类型',esc(labels[p.provider]||p.provider||'—'))}${p.model?fact('模型',esc(p.model)):''}${host?fact('服务地址',esc(host)):''}${fact('状态',issues.length?'⚠️ '+esc(issues[0]):'✅ 已就绪',issues.length?'is-warn':'is-ok')}</div><p class="help">云端渠道不需要选择工作流；模型与参数在「工作流与 API 配置」中设置。</p>`}
function designerEnvironment(kicker='02 · 生成环境'){const d=assemblyDesign,channel=designerChannel(),comfy=channel?.provider==='comfyui';return designerCard(kicker,comfy?'渠道与工作流':'图像渠道',`${designerChannelNotice()}${field('图像生成渠道',`<select id="designer-channel">${ensureImageProviders().profiles.map(p=>opt(p.id,p.title,d.channelId)).join('')}</select>`)}${comfy?`<div class="designer-workflow">${designerWorkflowPickerHTML()}${designerSeedHTML()}</div>`:designerChannelFactsHTML(channel)}`,'designer-environment')}
function designerStoryCard(){const d=assemblyDesign,stories=projectTemplates();return designerCard('01 · 故事','要生成哪个分镜',field('分镜资产',`<select id="designer-story">${stories.map(t=>opt(t.id,t.title+' · '+t.frames.length+' 幕',d.storyId)).join('')}</select>`,stories.length?'':'当前画册集还没有分镜，请先在「分镜工坊」中新建。'))}
...
function designerModelResultsHTML(slots){const d=assemblyDesign,{options,total}=designerModelChoices(slots),current=String(d.overrides.model||''),fallback=d.overrideDefaults.model,query=d.overrideUI.modelQuery.trim();
 let html=`<div class="designer-model-option ${!current?'is-on':''}" data-designer-model-pick=""><span data-user-content>${fallback?`蓝图默认 · ${WorkflowSlots.loraStem(fallback)}`:'蓝图默认（未填模型）'}</span><small>沿用蓝图</small></div>`;
...
function designerOverridesPanel(){const d=assemblyDesign,w=designerWorkflow(),slots=w?designerSlots():null;if(!w||!slots)return '';const catalog=comfyModelCatalog(),modelOn=slots.model.enabled,loraOn=slots.lora.mode!=='off',synced=catalog.fetchedAt?`已同步 ${catalog.checkpoints.length+catalog.unets.length} 个模型 · ${catalog.loras.length} 个 LoRA`:'尚未同步模型列表';
 if(!modelOn&&!loraOn)return `<section class="designer-slots is-inert" aria-label="模型与 LoRA"><header><div class="designer-slots-title"><span class="designer-kicker">03 · 模型</span><h4>模型与 LoRA</h4></div><p class="help">此工作流未识别出模型或 LoRA 槽，将按蓝图原样提交；可在「工作流与 API 配置」里手动指定。</p></header></section>`;
...
function renderAssemblyDesigner(){const d=assemblyDesign;const steps=['分镜与渠道','视觉预设','命名与入队'],comfy=designerChannel()?.provider==='comfyui';let content;
 const environment=()=>`<div class="designer-columns"><div class="designer-main">${designerStoryCard()}${designerEnvironment()}</div><aside class="designer-side">${comfy?`<details class="designer-advanced"><summary>模型与 LoRA <span>可选覆写</span></summary>${designerOverridesPanel()}</details>`:''}</aside></div>`;
 ...
 else{const titles=['选择故事与生成环境','为故事搭配视觉资产','让这次创作有自己的名字'],notes=['选择要生成的分镜与图像渠道；使用 ComfyUI 时，再选择工作流，并可只为这次任务替换模型与 LoRA。','拖动框选视觉预设，Ctrl / ⌘ 加选，Shift 连选。同名变量按预设顺序覆盖。','为画册命名、选择归入的画册集，确认摘要后加入待命队列。任务不会自动开始，仍需在任务卡上点击开始。'];
 ...
 const footHint=d.mode==='canvas'?`${d.stories.length} 个分镜节点 · ${d.edges.length} 条关联 · 每个分镜节点生成一张待命任务卡`:d.step<2?`第 ${d.step+1} / ${steps.length} 步`:'加入待命队列后不会自动开始生成';
 modal('新建生成任务',`<div class="designer-head"><div class="designer-modes" role="group" aria-label="装配模式">${[['wizard','分步向导'],['canvas','画布连线']].map(([id,label])=>btn(label,id==='wizard'?'list':'nodes','designer-mode',`data-mode="${id}" aria-pressed="${d.mode===id}"`,d.mode===id?'active':'ghost')).join('')}</div><p class="designer-head-note">${d.mode==='canvas'?'把多个分镜与预设连线，一次装配多张任务卡。':'创作配置 / NEW GENERATION'}</p></div>${content}<div class="modal-footer designer-footer"><span class="designer-footer-hint">${footHint}</span><span class="grow"></span>${btn('取消','','close-modal','','ghost')}${d.mode==='wizard'&&d.step>0?btn('上一步','arrow-back','designer-back'):''}${d.mode==='wizard'&&d.step<2?btn('下一步','arrow','designer-next','','primary'):btn(d.mode==='canvas'?'批量生成 · 加入待命':'添加待命任务','plus','designer-submit',d.busy?'disabled':'','primary')}</div>`,'',true);
...
async function submitAssemblyDesigner(){const d=assemblyDesign;if(d.busy)return;validateDesignerEnvironment();if(designerChannel()?.provider==='comfyui')await ensureWorkflowSlotPlan(designerWorkflow());let items;
 ...
 d.busy=true;const button=$('[data-act="designer-submit"]');if(button)button.disabled=true;
  try{
    ...
    save();try{await savePythonWorkspace()}catch(_){}await productionRequest('assemble-batch',{items});closeModal();ui.workspace=1;workshop.view='production';await refreshProduction();toast(`已添加 ${items.length} 张待命任务卡`);if(missingKeys.length)toast(missingVariablesNotice(missingKeys),'warn');
