...
function renderAssemblyDesigner(){const d=assemblyDesign;const steps=['分镜与图像服务','预设','命名与入队'],comfy=designerChannel()?.provider==='comfyui';let content;
 ...
 else{const titles=['选择故事与生成环境','为故事搭配视觉资产','让这次创作有自己的名字'],notes=['选择要生成的分镜与图像服务；使用 ComfyUI 时，再选择工作流，并可只为这次任务替换模型与 LoRA。','拖动框选预设，Ctrl / ⌘ 加选，Shift 连选。同名变量按预设顺序覆盖。','为画册命名、选择归入的画册集，确认摘要后加入待命队列。加入后，在生成任务上点「开始」才会生成。'];
