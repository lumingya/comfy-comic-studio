...
/* The seed binding must point at a number the workflow really has. Shared with the help drawer's setup checklist. */
function workflowSeedReady(w){return !!w?.bindings?.some(b=>{if(!b.enabled||!isSeedBinding(b))return false;try{let v=w.workflow[String(b.nodeId)].inputs;for(const part of (b.path.startsWith('/')?b.path.slice(1).split('/'):b.path.split('.')))v=v[part.replace(/~1/g,'/').replace(/~0/g,'~')];return typeof v==='number'&&Number.isFinite(v)}catch{return false}})}
function designerSeedReady(){return workflowSeedReady(designerWorkflow())}
function designerChannelNotice(){const d=assemblyDesign,current=designerChannel(),previous=ensureImageProviders().profiles.find(p=>p.id===d.previousChannelId),skipped=ensureImageProviders().profiles.find(p=>p.id===d.channelFallback);if(current&&skipped&&skipped.id!==current.id)return `<p class="notice designer-channel-notice" role="status">${esc(localeString('设置中启用的「{active}」还没配置好，已改选上次装配使用的「{current}」。',{active:skipped.title,current:current.title}))}</p>`;if(!current||!previous||previous.id===current.id||current.id!==ensureImageProviders().active)return '';return `<p class="notice designer-channel-notice" role="status">已按设置中当前启用的图像服务预选「${esc(current.title)}」；上次装配使用的是「${esc(previous.title)}」。请确认后再入队，避免发送到非预期的付费图像服务。</p>`}
...
function refreshDesignerModelAlert(){const slot=$('#designer-model-alert'),note=$('#designer-model-note');if(slot)slot.innerHTML=designerModelAlertHTML();if(note)note.textContent=designerModelProblems().length?'需要选择模型':'可选覆写';refreshDesignerChecks()}
...
function designerOverridesPanel(){const d=assemblyDesign,w=designerWorkflow(),slots=w?designerSlots():null;if(!w||!slots)return '';const catalog=comfyModelCatalog(),modelOn=slots.model.enabled,loraOn=slots.lora.mode!=='off',synced=catalog.fetchedAt?`已同步 ${catalog.checkpoints.length+catalog.unets.length} 个模型 · ${catalog.loras.length} 个 LoRA`:'尚未同步模型列表';
 ...
 return `<section class="designer-slots" aria-label="模型与 LoRA"><header><div class="designer-slots-title"><span class="designer-kicker">03 · 模型</span><h4>模型与 LoRA</h4></div>${btn('同步模型列表','refresh','designer-sync-catalog','','ghost small')}<p class="help">${esc(synced)} · 只用于本次任务</p></header>${modelBody}${loraBody}<p class="help" role="status">${esc(WorkflowSlots.describe(slots))}</p></section>`}
...
function openAssemblyDesigner(){
 ...
 /* B10: the wizard follows the image service enabled in Settings, never silently the last one billed; a differing history is only a notice.
    T3: when the enabled service is not set up yet and the last used one is, the last used one is preselected, with a notice. */
 const active=providers.profiles.find(p=>p.id===providers.active),last=providers.profiles.find(p=>p.id===lastUsed),fallback=!!(active&&last&&last.id!==active.id&&providerSetupIssues(active).length&&!providerSetupIssues(last).length),channelId=fallback?last.id:providers.active||lastUsed;
 Object.assign(d,{mode:'wizard',step:0,storyId:workshopStory()?.id||'',channelId,previousChannelId:!fallback&&lastUsed&&lastUsed!==providers.active?lastUsed:'',channelFallback:fallback?active.id:'',workflowId:(prefs.channelId===channelId?prefs.workflowId:'')||state.settings.comfy.activeWorkflowId,workflowQuery:'',seedEnabled:false,seed:1,concurrency:null,presets:new Set(),presetsTouched:false,autoPresets:{ids:[],reason:''},title:'',titleTouched:false,projectId:state.projects.some(p=>p.id===prefs.projectId)?prefs.projectId:state.activeProjectId,stories:[],visuals:[],edges:[],requestId:uid('assembly'),busy:false,drag:null});
 ...
 designerRestoreResume();
 designerApplyDefaults();
...
function designerPresets(){const d=assemblyDesign,sets=projectVariableSets();return `${designerAutoNoteHTML()}<div class="designer-presets" role="group" aria-label="预设">${sets.map(p=>{const on=d.presets.has(p.id),kind=p.category==='scenes'?'场景 / 画风':'角色';return `<label class="designer-preset ${on?'selected':''}"><input type="checkbox" data-designer-preset="${esc(p.id)}" ${on?'checked':''}><span class="designer-preset-mark" aria-hidden="true">${icon('check','sm')}</span><span class="designer-preset-icon">${icon('box')}</span><span class="designer-preset-body"><strong data-user-content>${esc(p.title)}</strong><small>${esc(kind)} · ${p.entries.length} 个变量</small>${designerPresetKeys(p)}</span></label>`}).join('')||'<div class="designer-empty"><strong>这个画册集还没有预设</strong><p>可以不使用预设，直接生成不含变量的分镜；也可以先到「预设工坊」建立角色、服装或画风预设。</p></div>'}</div><p class="help designer-presets-status" id="designer-presets-status" role="status" aria-live="polite">${designerPresetsStatus()}</p>`}
/* T3 · C2 smart defaults. Presets: the ones last queued with this storyboard (while they all still exist), else the single
   preset that defines every variable the storyboard uses; nothing is picked when the choice would be a guess. The album
   name follows 分镜名 · 角色名 until the creator types their own. */
function designerRememberedPresets(storyId){const ids=state.settings.productionAssembly?.storyPresets?.[storyId],available=new Set(projectVariableSets().map(p=>p.id));return Array.isArray(ids)&&ids.length&&ids.every(id=>available.has(id))?[...ids]:null}
function designerCoveringPreset(storyId){const story=templateBy(storyId);if(!story)return null;const defined=definedPromptNames({}),vars=[...new Set(story.frames.flatMap(f=>[...promptPlaceholders(f.prompt),...promptPlaceholders(f.negative),...promptPlaceholders(f.caption)]))].filter(k=>defined.has(k));if(!vars.length)return null;
 const hits=projectVariableSets().filter(p=>{const keys=new Set((p.entries||[]).map(e=>e.key));return vars.every(k=>keys.has(k))});return hits.length===1?[hits[0].id]:null}
const DESIGNER_NAME_KEYS=['character_display_name','display_name','character_name','name','角色名','名字'];
function designerCharacterName(ids){for(const id of ids){const p=setBy(id);if(!p||p.category==='scenes')continue;const entry=(p.entries||[]).find(e=>DESIGNER_NAME_KEYS.includes(e.key)&&e.type!=='image'&&typeof e.value==='string'&&e.value.trim());return entry?entry.value.trim():String(p.title||'').split(' · ')[0].trim()}return ''}
function designerDefaultTitle(){const d=assemblyDesign,story=templateBy(d.storyId);if(!story)return '';return [story.title,designerCharacterName([...d.presets])].filter(Boolean).join(' · ').slice(0,150)}
function designerApplyDefaults(){const d=assemblyDesign;
 if(!d.presetsTouched){const remembered=designerRememberedPresets(d.storyId),cover=remembered?null:designerCoveringPreset(d.storyId);d.autoPresets={ids:remembered||cover||[],reason:remembered?'remembered':cover?'cover':''};d.presets=new Set(d.autoPresets.ids)}
 if(!d.titleTouched)d.title=designerDefaultTitle()}
function designerAutoNoteHTML(){const d=assemblyDesign,a=d.autoPresets||{},same=!!a.ids?.length&&a.ids.length===d.presets.size&&a.ids.every(id=>d.presets.has(id));
 const text=!same?'':a.reason==='remembered'?localeString('已沿用上次为这个分镜选择的预设。'):a.reason==='cover'?localeString('已自动选中「{title}」：它定义了分镜用到的全部变量。',{title:setBy(a.ids[0])?.title||''}):'';
 return `<p class="designer-auto-note" id="designer-auto-note" ${text?'':'hidden'}>${text?`${icon('spark','sm')}<span>${esc(text)}</span>`:''}</p>`}
function designerSelectionChanged(){const d=assemblyDesign;d.presetsTouched=true;if(!d.titleTouched)d.title=designerDefaultTitle();refreshDesignerChecks()}
function designerStoryChanged(){designerApplyDefaults();refreshDesignerChecks()}
/* A ready image service to offer when ComfyUI cannot be reached: the last one used, else any service that is set up. */
function designerReadyAlternative(){const providers=ensureImageProviders(),prefs=state.settings.productionAssembly||{},ready=p=>!!p&&p.provider!=='comfyui'&&!providerSetupIssues(p).length,last=providers.profiles.find(p=>p.id===prefs.channelId);return ready(last)?last:providers.profiles.find(ready)||null}
/* T3 · C2 errors early: every problem of the wizard in one list, each owned by a step and carrying its fixes. */
function designerChecks(){const d=assemblyDesign,channel=designerChannel(),comfy=channel?.provider==='comfyui',workflow=comfy?designerWorkflow():null;
 const items=[...assemblyEnvironmentChecks(channel,workflow),...assemblyStoryChecks(d.storyId,[...d.presets],channel,workflow)];
 if(comfy&&workflow){
  const problems=designerModelProblems();
  if(problems.length)items.push({step:0,level:'warn',text:modelProblemText(problems[0])+'请在「模型与 LoRA」里选一个已安装的模型。',fixes:[{kind:'model',label:localeString('选择模型')}]});
  if(!designerSeedReady())items.push({step:0,level:'warn',text:localeString('所选工作流没有种子映射，每一幕都会沿用工作流里的同一个种子，画面可能几乎一样。'),fixes:[findSeedInput(workflow.workflow)?{kind:'seed',label:localeString('一键添加种子映射')}:{kind:'workflow',label:localeString('打开工作流')}]});
  if(state.settings.comfy.mode==='real'&&rt.engineChecked&&!rt.engineOk){const alt=designerReadyAlternative();items.push({step:0,level:'warn',text:localeString('ComfyUI 当前连不上（{url}）。开始生成前需要先启动它。',{url:baseURL()}),fixes:[{kind:'engine',label:localeString('测试连接')},alt?{kind:'channel',id:alt.id,label:localeString('改用「{title}」',{title:alt.title})}:null].filter(Boolean)})}
 }
 if(comfy&&d.seedEnabled&&(!designerSeedReady()||!Number.isInteger(d.seed)||d.seed<0||d.seed>4294967295))items.push({step:0,level:'block',text:localeString('可复现种子需为 0–4294967295 之间的整数。'),fixes:[]});
 const max=workshop.queue.maxConcurrency||128;
 if(d.concurrency!==null&&(!Number.isInteger(d.concurrency)||d.concurrency<1||d.concurrency>max))items.push({step:2,level:'block',text:localeString('分幕并发需为 1–{max} 的整数，或留空跟随全局',{max}),fixes:[{kind:'advanced',label:localeString('修改分幕并发')}]});
 if(d.mode==='wizard'&&!String(d.title||'').trim()){const fallback=designerDefaultTitle();items.push({step:2,level:'block',text:localeString('请填写画册名称。'),fixes:fallback?[{kind:'title-default',label:localeString('使用「{title}」',{title:fallback})}]:[]})}
 return items}
function designerFixButtonHTML(fix){const attrs=[['fix',fix.kind],['index',fix.index],['field',fix.field],['id',fix.id],['step',fix.step]].filter(([,v])=>v!==undefined&&v!==null&&v!=='').map(([k,v])=>`data-${k}="${esc(String(v))}"`).join(' ');return `<button type="button" class="designer-fix" data-act="designer-fix" ${attrs}>${esc(fix.label)}</button>`}
/* In the step list a problem owned by an earlier step leads with a way back; the step 3 recap keeps two buttons per item. */
function designerIssueListHTML(items,recap=false,cls='designer-issues'){const d=assemblyDesign;return `<ul class="${cls}">${items.map(i=>{const back=d.mode==='wizard'&&i.step<d.step?{kind:'step',step:i.step,label:localeString(i.step===1?'返回选择预设':'返回第 1 步')}:null,fixes=(recap?[...i.fixes,back]:[back,...i.fixes]).filter(Boolean).slice(0,recap?2:3);
 return `<li class="designer-issue is-${i.level}"><span class="designer-issue-mark" aria-hidden="true">${i.level==='block'?'⛔':'⚠️'}</span><span class="designer-issue-text">${esc(i.text)}</span>${fixes.length?`<span class="designer-issue-fixes">${fixes.map(designerFixButtonHTML).join('')}</span>`:''}</li>`}).join('')}</ul>`}
function designerStepIssuesHTML(items=designerChecks()){const d=assemblyDesign,list=items.filter(i=>i.step===d.step||(i.level==='block'&&i.step<d.step)).sort((a,b)=>(a.level==='block'?0:1)-(b.level==='block'?0:1)),blocks=list.filter(i=>i.level==='block').length,warns=list.length-blocks;
 if(!list.length)return '<div id="designer-step-issues" class="designer-step-issues" hidden></div>';
 const head=[blocks?`<span>${blocks} 个问题需要处理</span>`:'',warns?`<span>${warns} 项提示</span>`:''].filter(Boolean).join('<i aria-hidden="true">·</i>');
 return `<div id="designer-step-issues" class="designer-step-issues ${blocks?'is-blocked':'is-advised'}" role="status"><p class="designer-step-issues-head">${head}</p>${designerIssueListHTML(list)}</div>`}
/* The primary button is disabled while the step has blocking problems; the reason sits next to it and jumps to the list. */
function designerFooterSync(items){const d=assemblyDesign,button=document.querySelector('#modal [data-act="designer-next"],#modal [data-act="designer-submit"]'),reason=document.querySelector('#designer-footer-reason');if(!button)return;
 let text='';if(d.mode==='canvas'){if(!d.stories.length)text=localeString('先调入一个分镜')}
 else{const n=(items||designerChecks()).filter(i=>i.level==='block'&&(d.step===2||i.step<=d.step)).length;if(n)text=`先处理 ${n} 个问题`}
 button.disabled=!!text||!!d.busy;if(reason){reason.hidden=!text;reason.textContent=text}}
function designerOpen(){const m=document.querySelector('#modal');return !!m?.open&&m.classList.contains('assembly-designer')}
/* Swap a list only when its content changed, so a focused fix button or an opened advisory list survives the refresh. */
function designerSwap(el,html){if(!el)return;const t=document.createElement('template');t.innerHTML=html.trim();const next=t.content.firstElementChild,closed=x=>{const c=x.cloneNode(true);[c,...c.querySelectorAll('details')].forEach(n=>{if(n.matches('details'))n.removeAttribute('open')});return c.outerHTML};if(!next||closed(el)===closed(next))return;const open=!!el.querySelector('details[open]')||el.matches('details[open]');el.replaceWith(next);if(open)next.querySelectorAll('details').forEach(x=>{x.open=true})}
/* In-place refresh after an edit that does not re-render the wizard (preset clicks, title typing, story choice). */
function refreshDesignerChecks(){const d=assemblyDesign;if(!designerOpen())return;const items=d.mode==='wizard'?designerChecks():null;
 if(items){designerSwap($('#designer-step-issues'),designerStepIssuesHTML(items));designerSwap($('#designer-recap'),assemblyPreflightHTML(items))}
 const status=$('#designer-presets-status');if(status)status.textContent=designerPresetsStatus();
 const note=$('#designer-auto-note');if(note)note.outerHTML=designerAutoNoteHTML();
 const title=$('#designer-title');if(title&&!d.titleTouched&&title.value!==d.title)title.value=d.title;
 designerFooterSync(items)}
function designerShowIssues(){if(assemblyDesign.mode==='canvas'){$('#canvas-story-choice')?.focus();return}const box=$('#designer-step-issues:not([hidden])')||$('#designer-recap');if(!box)return;box.scrollIntoView({block:'center',behavior:'smooth'});box.querySelector('.designer-fix')?.focus({preventScroll:true})}
/* Fixes that leave the wizard keep its choices; reopening it for the same storyboard within half an hour resumes them. */
let designerResume=null;
function designerLeave(){const d=assemblyDesign;designerResume={at:Date.now(),storyId:d.storyId,step:d.step,channelId:d.channelId,workflowId:d.workflowId,presets:[...d.presets],presetsTouched:d.presetsTouched,autoPresets:d.autoPresets,title:d.title,titleTouched:d.titleTouched,projectId:d.projectId,concurrency:d.concurrency,seedEnabled:d.seedEnabled,seed:d.seed,overrides:clone(d.overrides||{}),overrideMemory:clone(d.overrideMemory||{})};closeModal()}
function designerRestoreResume(){const d=assemblyDesign,r=designerResume;designerResume=null;if(!r||r.storyId!==d.storyId||Date.now()-r.at>30*60e3)return false;
 if(ensureImageProviders().profiles.some(p=>p.id===r.channelId)&&r.channelId!==d.channelId){d.channelId=r.channelId;d.channelFallback='';d.previousChannelId=''}
 const workflowChanged=state.settings.comfy.presets.some(p=>p.id===r.workflowId)&&r.workflowId!==d.workflowId;if(workflowChanged)d.workflowId=r.workflowId;initDesignerOverrides();
 if(r.workflowId===d.workflowId){d.overrides=r.overrides;d.overrideMemory=r.overrideMemory}
 Object.assign(d,{step:Math.min(2,Math.max(0,r.step)),presets:new Set(r.presets.filter(id=>setBy(id))),presetsTouched:r.presetsTouched,autoPresets:r.autoPresets,titleTouched:r.titleTouched,title:r.titleTouched?r.title:d.title,concurrency:r.concurrency,seedEnabled:r.seedEnabled,seed:r.seed});
 if(state.projects.some(p=>p.id===r.projectId))d.projectId=r.projectId;return true}
function designerFocusWorkshopField(field,tries=12){const sel=`[data-workshop-frame="${field}"]`,el=[...document.querySelectorAll(sel)].find(e=>e.closest('.wm-focus')||!e.closest('[hidden]'));
 if(!el){if(tries>0)setTimeout(()=>designerFocusWorkshopField(field,tries-1),80);return}
 const details=el.closest('details');if(details)details.open=true;el.scrollIntoView({block:'center'});el.focus({preventScroll:true})}
async function designerFix(data){const d=assemblyDesign;
 switch(data.fix){
  case 'step':d.step=Math.min(2,Math.max(0,Number(data.step)||0));renderAssemblyDesigner();return;
  case 'preset':if(setBy(data.id)){d.presets.add(data.id);d.presetsTouched=true;if(!d.titleTouched)d.title=designerDefaultTitle()}renderAssemblyDesigner();return;
  case 'model':if(d.step!==0){d.step=0;renderAssemblyDesigner()}{const panel=$('details.designer-advanced');if(panel){panel.open=true;panel.scrollIntoView({block:'nearest',behavior:'smooth'});panel.querySelector('input,button')?.focus({preventScroll:true})}}return;
  case 'seed':addWorkflowSeedMapping(d.workflowId);renderAssemblyDesigner();toast('已添加种子映射：每一幕都会写入自己的种子。');return;
  case 'engine':await testEngine(false);refreshDesignerChecks();return;
  case 'channel':if(ensureImageProviders().profiles.some(p=>p.id===data.id)){d.channelId=data.id;d.channelFallback='';d.previousChannelId='';d.seedEnabled=false;initDesignerOverrides()}renderAssemblyDesigner();return;
  case 'advanced':if(d.step!==2){d.step=2;renderAssemblyDesigner()}{const more=$('details.designer-more');if(more)more.open=true;$('#designer-concurrency')?.focus()}return;
  case 'title-default':d.title=designerDefaultTitle();d.titleTouched=false;{const t=$('#designer-title');if(t){t.value=d.title;t.focus()}}refreshDesignerChecks();return;
  case 'frame':case 'story':{const storyId=d.storyId;designerLeave();workshop.view='stories';if(templateBy(storyId))workshop.storyId=storyId;workshop.pickedFrames?.clear();navigate(1);
   if(data.fix==='frame'){selectWorkshopFrame(Number(data.index)||0);designerFocusWorkshopField(data.field==='caption'||data.field==='width'||data.field==='cfg'?data.field:'prompt')}return}
  case 'service':case 'workflow':designerLeave();navigate(3);return;
 }
}
...
function designerNaming(items=designerChecks()){const d=assemblyDesign;return `${assemblyPreflightHTML(items)}<div class="designer-columns designer-naming"><div class="designer-main">${designerCard('01 · 命名','这本画册叫什么',`${field('画册名称',input('title',d.title,'text','id="designer-title" maxlength="150" placeholder="例如：雨夜里的第二次相遇" autocomplete="off"'))}${designerProjectField()}<details class="designer-more" ${d.concurrency!==null?'open':''}><summary>高级选项</summary>${designerConcurrencyField()}</details>`)}</div><aside class="designer-side">${designerCard('02 · 核对','任务摘要',designerSummaryHTML(),'designer-summary-card')}</aside></div>`}
...
function renderAssemblyDesigner(){const d=assemblyDesign;const steps=['分镜与图像服务','预设','命名与入队'],comfy=designerChannel()?.provider==='comfyui',items=d.mode==='wizard'?designerChecks():[];let content;
 ...
 else{const titles=['选择故事与生成环境','为故事搭配视觉资产','让这次创作有自己的名字'],notes=['选择要生成的分镜与图像服务；使用 ComfyUI 时，再选择工作流，并可只为这次任务替换模型与 LoRA。','单击选择预设；Ctrl / ⌘ 点击加选，Shift 连选，也可以拖动框选。同名变量按预设顺序覆盖。','为画册命名、选择归入的画册集，确认摘要后加入待命队列。加入后，在生成任务上点「开始」才会生成。'];
  content=`${`<div class="designer-workbench"><aside class="designer-navigation"><div class="designer-brand">MIO <span>PRODUCTION STUDIO</span></div><h2>从故事，<br>到画面。</h2><p>一次专注一件事。<br>让创作自然发生。</p>${designerStepperHTML(steps)}<div class="designer-safety">${icon('clock','sm')} 先准备，再生成<small>入队后由你决定何时开始。</small></div></aside>`}<section class="designer-step" data-step="${d.step}"><header class="designer-step-head"><h3>${titles[d.step]}</h3><p>${notes[d.step]}</p></header>${d.step===0?environment():d.step===1?designerPresets():designerNaming(items)}${d.step<2?designerStepIssuesHTML(items):''}</section></div>`}
 const footHint=d.mode==='canvas'?`${d.stories.length} 个分镜节点 · ${d.edges.length} 条关联 · 每个分镜节点生成一个待命的生成任务`:`第 ${d.step+1} / ${steps.length} 步`;
 modal('新建生成任务',`<div class="designer-head"><div class="designer-modes" role="group" aria-label="装配模式">${[['wizard','分步向导'],['canvas','画布连线']].map(([id,label])=>btn(label,id==='wizard'?'list':'nodes','designer-mode',`data-mode="${id}" aria-pressed="${d.mode===id}"`,d.mode===id?'active':'ghost')).join('')}</div><p class="designer-head-note">${d.mode==='canvas'?'把多个分镜与预设连线，一次装配多个生成任务。':'创作配置 / NEW GENERATION'}</p></div>${content}<div class="modal-footer designer-footer"><span class="designer-footer-hint">${footHint}</span><span class="grow"></span><button type="button" class="link-button designer-footer-reason" id="designer-footer-reason" data-act="designer-show-issues" hidden></button>${btn('取消','','close-modal','','ghost')}${d.mode==='wizard'&&d.step>0?btn('上一步','arrow-back','designer-back'):''}${d.mode==='wizard'&&d.step<2?btn('下一步','arrow','designer-next','','primary'):btn(d.mode==='canvas'?'批量生成 · 加入待命':'添加待命任务','plus','designer-submit',d.busy?'disabled':'','primary')}</div>`,'',true);
...
async function submitAssemblyDesigner(){const d=assemblyDesign;if(d.busy)return;if(d.mode==='wizard'&&designerChecks().some(i=>i.level==='block')){designerFooterSync();designerShowIssues();return}validateDesignerEnvironment();if(designerChannel()?.provider==='comfyui')await ensureWorkflowSlotPlan(designerWorkflow());let items;
 ...
 d.busy=true;const button=$('[data-act="designer-submit"]');if(button)button.disabled=true;
  try{
    ...
    /* T3: the presets queued with each storyboard become its default next time (the 50 most recent storyboards). */
    const storyPresets={...(state.settings.productionAssembly?.storyPresets||{})};
    for(const item of items){delete storyPresets[item.storyId];storyPresets[item.storyId]=item.presets.map(p=>p.id)}
    for(const key of Object.keys(storyPresets).slice(0,-50))delete storyPresets[key];
    state.settings.productionAssembly={channelId:d.channelId,workflowId:d.workflowId,projectId:designerProjectId(),overrides:remembered,storyPresets};
...
function optionalModulesHTML(){const p=state.settings.studio,on=p.visibility.extensions!==false;return `<section class="settings-section"><h2>只留下你需要的功能</h2><p>核心创作始终保留，其余工作区由你选择。</p>${settingsRow('画册集与创作工坊','管理画册、分镜、预设与生成任务。','<span class="tiny muted">核心功能</span>','book')}${settingsRow('模板与扩展市场','独立全屏资源工作区。',switchControl('visibility.marketplace','显示模板与扩展市场',p.visibility.marketplace===true),'box')}${settingsRow('运行日志','查看运行记录与错误。',switchControl('visibility.logs','显示运行日志',p.visibility.logs),'terminal')}${settingsRow('AI 写故事','按需启用故事辅助工作区。',switchControl('visibility.llm','启用 AI 写故事',p.visibility.llm),'spark')}${settingsRow('开发者选项','在设置里显示“开发者”分类：原生配置接口、数据目录结构、服务端任务与素材索引。',switchControl('visibility.developer','显示开发者选项',p.visibility.developer===true),'terminal')}</section><section class="settings-section optional-settings"><header><div><h2>可选功能</h2></div><label class="switch"><input id="ws-extensions" role="switch" type="checkbox" aria-label="启用可选功能" ${on?'checked':''}><span class="switch-track"></span></label></header>${on?`<div class="optional-children">${laboratoryExtensions().map(x=>settingsRow(x.title,x.description,`<label class="switch"><input role="switch" type="checkbox" data-art-extension="${x.key}" aria-label="启用${x.title}" ${x.enabled?'checked':''}><span class="switch-track"></span></label>`,x.icon)).join('')}</div>`:''}</section>${typeof productionLiveSyncSettingsHTML==='function'?productionLiveSyncSettingsHTML():''}`}
function installArchitecture(){
 paths['arrow-back']='<path d="M19 12H5m6-6-6 6 6 6"/>';
 /* A ComfyUI probe that lands while the wizard is open updates its reachability warning. */
 if(typeof onEngineChecked==='function'){const engineChecked=onEngineChecked;onEngineChecked=function(rec){engineChecked(rec);if(designerOpen())refreshDesignerChecks()}}
 studioDefaults.visibility.extensions=true;studioDefaults.visibility.marketplace=false;studioDefaults.visibility.developer=false;studioDefaults.features.assistant??=true;
 ...
 Object.assign(v3Actions,{
  ...
  'workshop-negative-all':async()=>{const s=workshopStory(),value=s.frames[workshop.frame]?.negative||'';if(await confirmAction('覆盖所有分幕的负向提示词？',`将更新「${s.title}」的 ${s.frames.length} 幕。`,'应用到所有分幕')){s.frames.forEach(f=>f.negative=value);save();render()}},
  'designer-mode':d=>{assemblyDesign.drag=null;assemblyDesign.mode=d.mode;document.querySelectorAll('#toasts .toast').forEach(t=>t.remove());renderAssemblyDesigner()},
