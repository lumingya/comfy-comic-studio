...
function designerSeedAlertHTML(){const w=designerWorkflow(),frames=templateBy(assemblyDesign.storyId)?.frames.length||0,target=w?findSeedInput(w.workflow):null;
 return `<p class="seed-setting-alert" role="alert">${icon('alert','sm')}<span>${frames>1?`这 ${frames} 幕会使用同一个种子，画面可能几乎一样。`:'每一幕都会使用蓝图里的同一个种子，画面可能几乎一样。'}</span>${target?'':'<small>没有找到采样器的种子输入，请在「工作流与 API 配置」里手动映射。</small>'}${target?btn('一键添加种子映射','plus','designer-add-seed-mapping','','small'):''}</p>`}
...
function designerModelResultsHTML(slots){const d=assemblyDesign,{options,total}=designerModelChoices(slots),current=String(d.overrides.model||''),fallback=d.overrideDefaults.model,query=d.overrideUI.modelQuery.trim();
 let html=`<div class="designer-model-option ${!current?'is-on':''}" data-designer-model-pick=""><span data-user-content>${fallback?`蓝图默认 · ${WorkflowSlots.loraStem(fallback)}`:'蓝图默认（未填模型）'}</span><small>${placeholderModelName(fallback)?'占位名，请换一个':'沿用蓝图'}</small></div>`;
