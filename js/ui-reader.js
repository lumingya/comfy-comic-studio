/* Mio reader: feature-owned UI functions. Loaded before ui.js; initialization remains in app.js. */
'use strict';
...
function closeArtReader(){artUI.readerObserver?.disconnect();if(document.fullscreenElement)document.exitFullscreen?.().catch(()=>{});for(const id of ['#toasts','#ui-tooltip','#assistant']){const el=$(id);if(el&&$('#reader').contains(el))document.body.append(el)}$('#reader').close();ui.bookId=null;artUI.showInfo=false;refreshGallery()}
...
function renderArtReader(){
  ...
  $('#reader').innerHTML=`<header class="room-head">${ibtn('close','close-reader','关闭画册')}<span class="room-brand">Mio.</span><span class="room-title grow" data-user-content>${esc(b.title)}</span>${versions.length>1?`<select id="reader-version" aria-label="切换此画册的版本">${versions.map(v=>opt(v.id,v.storyTitle||v.title,b.id)).join('')}</select>`:''}<nav class="room-controls" aria-label="阅读模式">${[['spread','book','对开本'],['webtoon','list','卷轴'],['gallery','image','画廊']].map(([mode,ic,label])=>`<button class="${artUI.readerMode===mode?'active':''}" data-act="room-mode" data-mode="${mode}" aria-pressed="${artUI.readerMode===mode}">${label}</button>`).join('')}</nav>${ibtn('expand','room-fullscreen','网页全屏 · 电脑也可使用 F11')}${btn('导出','download','export-book',`data-id="${b.id}"`,'small room-export')}</header><section class="room-stage"><div class="room-canvas" id="reader-canvas"></div><button class="room-open-details" data-act="room-info">${icon('eye')}本幕文字</button><aside class="room-info" id="room-info" ${artUI.showInfo?'':'hidden'} aria-label="当前分镜信息"></aside></section><div class="room-filmstrip" id="room-filmstrip" ${artUI.readerMode==='gallery'||artUI.filmstrip?'':'hidden'} aria-label="分镜缩略图">${readerFilmstripHTML(b,sequence)}</div><footer class="room-footer"><span>PRIVATE READING ROOM</span><span class="spacer"></span><div class="room-pagination">${ibtn('up','page-prev','上一页')}<span class="page-label" id="page-position"></span>${ibtn('down','page-next','下一页')}</div><span class="spacer"></span><span class="room-scene-title" id="room-scene-title" data-user-content></span>${ibtn('story','room-thumbnails','显示或收起分镜胶卷')}${ibtn('eye','room-info','查看本幕文字')}${ibtn('nodes','room-laboratory','到实验室使用精修扩展')}</footer>`;
  const filmstrip=$('#room-filmstrip');if(filmstrip)filmstrip.hidden=!artUI.filmstrip;
  renderArtCanvas(false);
}

function readerFilmstripHTML(b,sequence){return (artUI.filmstrip?sequence:[]).map(s=>`<button data-act="reader-step" data-index="${s.stepIndex}" class="${s.stepIndex===ui.step?'active':''}" aria-label="跳到第 ${s.stepIndex+1} 幕：${esc(s.name)}" ${s.stepIndex===ui.step?'aria-current="page"':''}>${imgTag(thumbnailURL(s.image),s.name,`data-book="${b.id}" data-step="${s.stepIndex}" data-page-img="${b.id}:${s.stepIndex}" loading="lazy" decoding="async"`)}<small>${pad(s.stepIndex+1)}</small></button>`).join('')}

/* Live update while reading: when the production queue publishes pages of the open album, swap only the
   pages that changed (webtoon) or re-render the visible spread, so the reading position survives. */
function refreshArtReaderPages(book,changed=[],structure=false){
  const dialog=$('#reader'),canvas=$('#reader-canvas');if(!dialog?.open||!canvas||!book||ui.bookId!==book.id)return false;
  const sequence=readerSequence(book),total=sequence.length;if(!total)return false;
  const indices=[...new Set(changed.map(Number).filter(i=>i>=0&&i<total))];
  const inPlace=artUI.readerMode==='webtoon'&&!structure&&!(typeof studioUI==='object'&&studioUI?.exportDraft)&&indices.length>0&&indices.every(i=>canvas.querySelector(`[data-scroll-step="${i}"]`));
  if(inPlace){
    for(const i of indices){const article=canvas.querySelector(`[data-scroll-step="${i}"]`),frame=sequence[i];if(!article||!frame)continue;article.innerHTML=roomPage(book,frame)+(artUI.showCaptions?`<div class="scroll-caption" data-user-content>${esc(frame.caption)}</div>`:'');if(typeof localizeWorkspace==='function')localizeWorkspace(article)}
  }else if(artUI.readerMode==='webtoon'){
    if(!structure&&!indices.length)return false;
    const step=clamp(ui.step,0,Math.max(0,total-1)),anchor=canvas.querySelector(`[data-scroll-step="${step}"]`),offset=anchor?anchor.getBoundingClientRect().top-canvas.getBoundingClientRect().top:null;
    renderArtCanvas(false,offset===null);
    if(offset!==null)requestAnimationFrame(()=>{const next=canvas.querySelector(`[data-scroll-step="${step}"]`);if(!next)return;artUI.readerAnchorUntil=Date.now()+650;canvas.scrollTop+=next.getBoundingClientRect().top-canvas.getBoundingClientRect().top-offset});
  }else{
    const visible=artUI.readerMode==='gallery'?[ui.step]:readerSpreadIndices(ui.step,total);
    if(structure||indices.some(i=>visible.includes(i)))renderArtCanvas(false);
  }
  const strip=$('#room-filmstrip');if(strip&&(structure||indices.length)){strip.innerHTML=readerFilmstripHTML(book,sequence);if(typeof localizeWorkspace==='function')localizeWorkspace(strip)}
  syncArtReader();return true;
}

function renderArtCanvas(animate=false,scrollToStep=true){
  ...
  if(artUI.readerMode==='webtoon'){
    ...
    if(scrollToStep)requestAnimationFrame(()=>{if(ui.step>0)scrollRoomTo(ui.step,false)});
  ...
  $$('img',canvas).forEach(img=>{if(img.complete){if(artUI.showInfo)renderRoomInfo()}else{img.addEventListener('load',()=>{if(artUI.showInfo)renderRoomInfo()},{once:true})}});
  syncArtReader();
}
