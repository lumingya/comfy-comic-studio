...
function compileTemplateDocument(t,books,options={}){
  ...
  const csp=doc.createElement('meta');csp.httpEquiv='Content-Security-Policy';csp.content="default-src 'none'; img-src data:"+(opts.preview?" blob:":"")+"; style-src 'unsafe-inline'; font-src data:; script-src 'nonce-"+nonce+"'; connect-src 'none'; object-src 'none'; frame-src 'none'; base-uri 'none'; form-action 'none';";
...
function renderTemplateStudio(){
  ...
  $('#template-studio').innerHTML=`<header class="template-dialog-head">${icon('book')}<div class="grow"><h2 id="template-studio-title">画册导出模板</h2></div>${btn('导入 HTML / JSON','upload','et-import','','small')}${featureEnabled('marketplace')?btn('模板市场','box','et-market','','small'):''}${ibtn('close','et-close','关闭模板工作室')}</header>
