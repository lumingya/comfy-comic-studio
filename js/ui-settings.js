/* Mio settings: feature declarations; initialization remains in app.js. */
'use strict';
function renderEngine(){const c=state.settings.comfy,w=c.workflow,m=c.mapping;const bind=(key,label)=>field(label,`<select data-setting="comfy.mapping.${key}">${opt('','未绑定（可选）',m[key])}${Object.entries(w).map(([id,n])=>opt(id,'#'+id+' · '+n.class_type,m[key])).join('')}</select>`);return heading('ComfyUI 引擎管线','从提示词到画面，构建可追溯的生成链路。',btn('导入工作流','upload','import-workflow')+btn('单帧试跑','play','dry-run','','primary'),'WORKFLOW / ENGINE ROOM')+`<div class="pipeline-grid"><div class="stack"><section class="panel"><h3 class="panel-title">${icon('nodes')}引擎连接 <span class="chip green" style="margin-left:auto">${c.mode==='mock'?'OFFLINE SVG':'REAL GPU'}</span></h3>${field('运行模式',`<select data-setting="comfy.mode">${opt('real','真实 ComfyUI GPU 服务',c.mode)}</select>`)}${field('服务端地址',setting('comfy.baseUrl',c.baseUrl))}<div class="row">${btn('测试连接','refresh','test-engine')}<span class="tiny muted" id="connection-result">${rt.connected?'已连接 · '+rt.latency+' ms'+(rt.vram!==null?' · VRAM '+rt.vram+'%':''):'尚未连接真实 GPU'}</span></div><div class="divider"></div><div class="notice amber" style="margin-top:17px">生成与连通性检查都由 Mio 后端发起，ComfyUI 无需 <code>--enable-cors-header</code>；只需保证运行 Mio 的机器能访问该地址。基础工作流中的模型文件名需要替换为实际模型。</div></section><section class="panel"><h3 class="panel-title">${icon('image')}单帧验证预览</h3><div class="preview-box" id="dry-preview">${rt.preview?imgTag(rt.preview,'单帧试跑结果'):icon('image')+'<p>运行一帧，验证你的创作管线</p>'}</div></section></div><div class="stack"><section class="panel"><h3 class="panel-title">${icon('nodes')}智能节点映射器</h3>${field('工作流名称',setting('comfy.workflowTitle',c.workflowTitle))}<div class="row" style="margin-bottom:15px"><select id="workflow-preset" aria-label="工作流预设">${opt('','选择已保存的工作流','')}${c.presets.map((p,i)=>opt(i,p.title,'')).join('')}</select>${btn('保存预设','disk','save-workflow','','small')}</div><div class="node-chain"><div class="node-box">Text Encode<small>#${esc(m.positive)}</small></div><span class="node-line"></span><div class="node-box">KSampler<small>#${esc(m.sampler)}</small></div><span class="node-line"></span><div class="node-box">VAE Decode<small>LATENT → RGB</small></div><span class="node-line"></span><div class="node-box">Save Image<small>#${esc(m.output)}</small></div></div><div class="binding-grid">${bind('positive','正向提示词节点')}${bind('negative','负向提示词节点')}${field('正向文本字段',setting('comfy.mapping.positiveField',m.positiveField))}${field('负向文本字段',setting('comfy.mapping.negativeField',m.negativeField))}${bind('sampler','采样器 / KSampler')}${bind('size','尺寸 / EmptyLatentImage')}${bind('output','输出 / SaveImage')}${bind('image','图像输入 / LoadImage（精修可选）')}</div><div class="help">text、opt_text、text_g 均可绑定。图生图 / 蒙版精修需导入含 VAEEncode 或 VAEEncodeForInpaint 的工作流，再绑定 LoadImage。</div><div class="divider"></div>${btn('编辑 API JSON','edit','edit-workflow')}${btn('导出工作流 JSON','download','export-workflow','style="margin-left:8px"')}</section></div></div>`}

function llmSettings(){const c=state.settings.llm;modal('大模型连接设置',`${field('运行模式',`<select data-setting="llm.mode">${opt('real','OpenAI 兼容 API',c.mode)}</select>`)}${field('服务商预设',`<select id="llm-provider">${[['openai','OpenAI'],['deepseek','DeepSeek'],['ollama','Ollama 本地'],['custom','自定义兼容网关（含 Claude）']].map(x=>opt(...x,c.provider)).join('')}</select>`)}${field('Base URL',setting('llm.baseUrl',c.baseUrl))}${field('API Key',setting('llm.key',c.key,'password'))}${field('模型名称',setting('llm.model',c.model))}<div class="notice amber">密钥仅存于此浏览器。真实调用会将指定文本和图片发送至配置的服务商。请使用可信的 OpenAI 兼容端点，Anthropic 原生协议不适用。</div><div class="modal-footer">${btn('测试连接','refresh','test-llm')}${btn('保存并关闭','check','close-modal','','primary')}</div>`,'使用 POST /chat/completions，支持多模态和工具调用')}
...
function identitySettingsHTML(){
  ...
  return `<section class="settings-section workspace-identity"><h2>这是你的工作室</h2><p>名称显示在侧栏与工作室文件中。创作者署名可用于画册落款。</p><div class="row" style="margin:22px 0"><div class="workspace-monogram">${esc([...workspaceName()][0])}</div><div><strong style="font-size:.875rem;font-weight:500">${esc(workspaceName())}</strong><p class="tiny muted" style="margin:7px 0 0">PRIVATE CREATIVE WORKSPACE</p></div></div>${field('工作室名称',input('workspaceName',identity.workspaceName,'text','id="identity-workspace" maxlength="40" placeholder="例如：纸间漫画工作室"'))}${field('创作者署名',input('creatorName',identity.creatorName||'','text','id="identity-creator" maxlength="60" placeholder="你的名字或笔名"'))}<label class="row small soft" style="margin-bottom:20px"><input id="identity-sync-signature" type="checkbox" checked>同时用此署名作为默认画册签名</label>${btn('保存名称与署名','check','identity-save','','primary')}</section><section class="settings-section"><h2>你的作品在哪里？</h2>${disk.root?`<div class="disk-path">${esc(disk.root.name)} / workspace.json</div>`:'<div class="disk-warning">尚未连接本地目录。当前更改只在本次会话内，关闭前请连接目录或下载备份。</div>'}<div class="row" style="margin-top:15px">${btn('管理文件保存位置','folder','storage-settings')}${btn('下载目录备份','download','disk-archive')}</div></section>`;
}

function storageSettingsHTML(){
  const supported=diskSupported(),connected=!!disk.root;
  return `<section class="settings-section"><h2>普通文件，真正属于你</h2><p>作品使用本地文件保存。每个画册集、每本画册都有自己的文件夹，图片单独保存。</p><div class="disk-info"><div class="disk-info-header">${icon('folder')}<div class="grow"><h3>${esc(connected?disk.root.name:disk.remembered?'待连接：'+disk.remembered.name:'尚未选择工作室根目录')}</h3><p id="disk-settings-status">${esc(diskStatusText())}</p></div><span class="chip ${connected?'green':''}">${connected?'磁盘目录':'临时会话'}</span></div><div class="row wrap">${btn(connected?'切换工作室目录':'选择本地根目录','folder','disk-connect','','primary')}${disk.remembered&&!connected||disk.phase==='permission'?btn('重新授权','refresh','disk-reconnect'):''}${btn('立即保存','disk','disk-save',connected&&!disk.busy?'':'disabled')}${connected?btn('重新读取磁盘','refresh','disk-reload',disk.busy?'disabled':''):''}</div><div class="disk-progress-caption" id="disk-save-progress">${esc(disk.progress||'只在所选目录范围内读写。')}</div>${disk.lastError?`<div class="disk-warning">${esc(disk.lastError)}</div>`:''}${disk.phase==='recovery'?`<div class="disk-warning">当前索引未完整读取，已从 workspace.previous.json 恢复上一次完整记录。请检查画册后确认恢复，再继续保存。${btn('确认恢复并继续保存','check','disk-accept-recovery','style="margin-top:12px"','small')}</div>`:''}</div>${!supported?'<div class="disk-warning">当前浏览器或嵌入页面未开放本地目录写入。请在桌面 Chrome / Edge 的 HTTPS 或 localhost 页面直接打开；当前仍可下载 ZIP 目录包、只读载入已解压目录。</div>':''}<div class="settings-note">${icon('shield')}<span>浏览器仅记住目录授权、工作室名称和主题偏好。图片、剧情、模板与队列保存为磁盘文件。浏览器缓存工程只用于迁移，不再自动写回缓存。</span></div></section><section class="settings-section"><h2>清晰的目录层级</h2><p>名称便于辨认，短 ID 避免同名冲突。重命名只改变显示名称，已建立的目录路径保持稳定。</p><pre class="folder-tree">${esc(disk.root?.name||workspaceName())}/
...
function renderGithubPublisher(){
  ...
  if(tab==='publish')body=`<div class="publish-layout"><section>${selector}${githubLocationFields()}${field('提交说明',input('message',d.message,'text','id="github-message" maxlength="200"'))}<label class="publish-consent"><input type="checkbox" id="github-consent"><span>我确认有权发布所选资源，并已检查其中的 HTML、提示词和台词。公开仓库中的文件任何人都可读取。</span></label></section><aside class="publish-summary" id="github-package-preview"></aside></div>`;
  else if(tab==='manual')body=`<div class="publish-layout"><section>${selector}${githubLocationFields(false)}<ol class="publish-steps"><li>下载模板包<p>点击下方“下载所选模板”，得到可直接导入的 JSON 文件。</p></li><li>在 GitHub 上传文件<p>打开已初始化的仓库，选择 Add file → Upload files，上传模板包并 Commit changes。文件路径请与左侧设置一致。</p></li><li>复制 Raw 地址供安装<p>打开仓库中的 JSON 文件，点击 Raw。把地址粘贴到 Mio 市场的“从仓库导入”即可。</p></li></ol></section><aside class="publish-summary"><h3>不想填写 Token？用网页上传。</h3><div class="stack">${btn('下载所选模板','download','github-download-package')}${btn('打开 GitHub 上传页面','upload','github-open-upload')}${btn('复制预计 Raw 地址','copy','github-copy-expected')}</div><p class="help">预计链接只有实际上传成功后才可访问；上传页需在 GitHub 登录。</p></aside></div>`;
  else body=`<div class="publish-layout"><section>${githubLocationFields()}<div class="service-note">私有仓库需填写 Token。</div></section><aside class="publish-summary"><h3>把托管的模板装回工作室</h3><p class="service-note">填写明确的仓库、分支与文件路径。系统读取文件后会先校验，再询问是否安装为独立副本。支持画册 HTML 模板包、分镜 JSON 和 API 工作流。</p>${btn('读取并校验仓库文件','download','github-import','','primary')}</aside></div>`;
  $('#github-dialog').innerHTML=`<header class="modal-head">${icon('upload')}<div class="grow"><h2 id="github-title">GitHub 模板发布与安装</h2></div>${ibtn('close','github-close','关闭发布窗口并清除 Token')}</header><div class="service-body"><div class="tabbar" style="margin-bottom:21px">${[['publish','直接上传到仓库'],['manual','GitHub 网页上传教程'],['import','从仓库安装']].map(([v,l])=>`<button class="tab ${v===tab?'active':''}" data-act="github-tab" data-tab="${v}">${l}</button>`).join('')}</div>${body}<div id="github-result" class="service-test-result"><span class="service-status">${tab==='publish'?'先检查仓库，再确认提交。已有文件会使用 SHA 防止并发覆盖。':'所有仓库操作均由你主动触发。'}</span></div><div class="row wrap" style="margin-top:13px"><a href="https://github.com/new" target="_blank" rel="noopener noreferrer" class="tiny">创建仓库</a><a href="https://github.com/settings/personal-access-tokens/new" target="_blank" rel="noopener noreferrer" class="tiny">创建 Fine-grained Token</a><a href="https://docs.github.com/en/rest/repos/contents#create-or-update-file-contents" target="_blank" rel="noopener noreferrer" class="tiny">权限与接口文档</a></div></div><footer class="service-footer"><span class="grow service-status">Token 不写入磁盘，关闭窗口即清除。</span>${releaseUI.githubBusy?btn('停止等待','stop','github-stop'):tab==='publish'?btn('检查仓库与文件','eye','github-check')+btn('上传并提交','upload','github-publish','','primary'):btn('关闭','','github-close')}</footer>`;
...
async function installGithubFile(){
  ...
  try{const target=await verifyGithubTarget(cfg,releaseUI.githubController.signal);if(!target.file||target.file.encoding!=='base64'||!target.file.content)throw Error('没有可读取的普通文件，或文件超出 1 MB 限制。');const text=base64UTF8(target.file.content);let value;try{value=JSON.parse(text)}catch(e){}
    ...
    else{validateWorkflow(value);if(!await confirmAction('导入 API 工作流？','保存任务配置。','保存工作流'))return;state.settings.comfy.presets.push({id:uid('wf'),title:cfg.path.split('/').at(-1),workflow:value,mapping:clone(state.settings.comfy.mapping)});save();$('#github-dialog').close();releaseUI.githubToken=''}
...
function githubSummarySettings(){const c=state.settings.github;return `<section class="settings-section"><h2>让模板在 GitHub 上流转</h2><p>上传、版本管理和安装链接有独立入口。你可以直接通过 GitHub API 提交，也可以下载模板包后在 GitHub 网页上传。</p><div class="service-context"><strong>模板发布</strong><br>最近仓库：${esc(c.repository||'尚未配置')}<br>资源类型：画册 HTML 模板、分镜剧本、ComfyUI 工作流</div><div class="row wrap" style="margin:20px 0">${btn('上传模板到 GitHub','upload','github-open','','primary')}${btn('查看网页上传教程','help','github-manual')}${btn('从仓库安装','download','github-import-open')}</div></section>`;}

function quickStartSettings(){const p=guidePreferences();return `<section class="settings-section"><h2>一份可以边做边看的教程</h2><p>从本地保存、画册集、角色与分镜，到生成、视觉审校、HTML 导出和 GitHub 托管。每一步都能跳到对应的真实功能。</p><div class="row wrap">${btn('从头开始教程','play','guide-open','data-step="0"','primary')}${btn('继续上次步骤','arrow','guide-open')}${btn('创建本地三幕练习','story','guide-practice')}</div><div class="divider"></div>${guideSteps.map((s,i)=>settingsRow(p.readSteps.includes(i)?s.title+' · 已阅读':s.title,s.description,btn('阅读','','guide-open',`data-step="${i}"`,'small'),s.icon)).join('')}</section>`}
...
function renderSettingCards(p,entries=mergedSettingEntries(p)){
  return entries.map(e=>{
    const help=characterSettingHelp(e.key),label=esc(settingLabel(e)),attrs=`id="setting-${p.id}-${e.id}" data-art-setting="${esc(e.key)}" data-settings-owner="${esc(p.id)}" aria-label="${label}" ${help?`aria-describedby="setting-help-${p.id}-${e.id}"`:''}`;
    const control=e.compute?`<div class="computed-card"><span>${icon('nodes')} 生成前计算 · ${esc(e.type)}</span><code>${esc(e.compute.script.slice(0,150))}</code>${btn('编辑脚本','edit','eco-macro-edit',`data-owner="${esc(p.id)}" data-key="${esc(e.key)}"`,'small ghost')}</div>`:e.type==='image'?imageVariableControl(e,p._presetEditorId?'preset-draft':'unified',p._presetEditorId||p.id):e.type==='boolean'?`<select ${attrs}>${opt('true','开启',String(e.value))}${opt('false','关闭',String(e.value))}</select>`:e.type==='number'?input('value',e.value,'number',attrs+' step="any"'):`<textarea ${attrs} >${esc(typeof e.value==='object'?JSON.stringify(e.value,null,2):e.value)}</textarea>`;
    const picked=typeof settingsGroupUI!=='undefined'&&settingsGroupUI.picked.has(p.id+'::'+e.key);
    return `<div class="character-setting ${help?'character-identity-setting':''} ${picked?'is-picked':''}" data-setting-key="${esc(e.key)}" data-setting-owner="${esc(p.id)}"><div class="character-setting-head"><span class="setting-drag-handle" draggable="true" data-drag-key="${esc(e.key)}" data-drag-owner="${esc(p.id)}" title="拖动到分组；先勾选多张卡片可一起拖动" aria-hidden="true">${icon('grip')}</span><input type="checkbox" class="setting-pick" data-pick-key="${esc(e.key)}" data-pick-owner="${esc(p.id)}" aria-label="选择 ${label}" title="勾选后可批量移动到分组" ${picked?'checked':''}><label for="setting-${p.id}-${e.id}" class="grow">${label}</label><code>{${esc(e.key)}}</code>${ibtn('nodes','eco-macro-edit',e.compute?'编辑可执行变量':'设为可执行变量',`data-key="${esc(e.key)}" data-owner="${esc(p.id)}"`)}${ibtn('edit','settings-meta-edit','编辑名称、变量标识符与归属分组',`data-key="${esc(e.key)}" data-owner="${esc(p.id)}"`)}${ibtn('trash','art-setting-remove','从当前编辑内容移除此变量',`data-key="${esc(e.key)}" data-owner="${esc(p.id)}"`)}</div>${control}${help?`<p class="help" id="setting-help-${p.id}-${e.id}">${esc(help)}</p>`:''}</div>`;
  }).join('');
}
function renderUnifiedSettings(){
  const p=currentBookSettings();if(!p)return '<div class="empty"><h3>先创建一本画册</h3>'+btn('新建画册','plus','new-book','','primary')+'</div>';
  const task=creationTask(),sets=projectVariableSets();
  return `<section class="character-settings settings-workbench"><header class="character-settings-head"><div class="grow"><span class="context-kicker">本册专属设定</span>${creationContextSelector(selectedPlan(),true)}<p class="settings-editor-context">${task?'自动保存后用于尚未发送的分镜。':'设定一次，贯穿所有分镜。修改仅属于这本画册。'}</p></div><span class="settings-save-status" data-settings-save-status>已保存</span></header><div class="settings-workbench-layout"><div class="settings-form">${settingsToolbar(p)}${renderSettingsGroups(p)}<details class="quiet-advanced"><summary>高级设置 · 节点映射与单幕覆盖</summary><p class="help">单幕变量优先于本册设定。LoRA 等变量需要对应的工作流节点映射。</p>${btn('打开节点映射','nodes','v3-settings-tab','data-tab="mapping"','small')}</details></div><aside class="settings-preset-rail"><div class="rail-icon">${icon('box')}</div><span class="context-kicker">公共预设库</span><h3>好设定，不必重写</h3><p>从公共预设复制角色与画风，<br>再为这本画册做一点改变。</p>${btn('浏览预设库','arrow','preset-library-open','','primary')}<div class="rail-divider"></div><div class="rail-list-heading">可用预设 <span>${sets.length}</span></div><div class="rail-preset-list">${sets.slice(0,4).map((v,i)=>`<button type="button" class="rail-preset" data-act="preset-library-preview" data-id="${esc(v.id)}"><span class="preset-monogram">${String(i+1).padStart(2,'0')}</span><span><strong>${esc(v.title)}</strong><small>${(v.entries||[]).length} 个变量 · 点击预览</small></span><span aria-hidden="true">↗</span></button>`).join('')||'<p class="help">还没有预设。把本册设定存入库，下次就能直接使用。</p>'}</div><details class="preset-independence"><summary>${icon('help')} 预设与本册是什么关系？</summary><div><p>应用预设，会将当前预设草稿的变量、分组和参考图复制为本册的一份独立设定，替换本册全局设定。</p><p>保存后的设定只用于尚未发送的分镜。</p></div></details></aside></div></section>`;
}
