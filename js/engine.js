...
function addRenderBindings(){const c=state.settings.comfy;let count=0;for(const[key,label,alt]of[['width','画面宽度'],['height','画面高度'],['steps','采样步数'],['cfg','CFG'],['denoise','去噪强度'],['seed','种子','noise_seed']]){const found=Object.entries(c.workflow).find(([,n])=>Object.hasOwn(n.inputs,key)||(alt&&Object.hasOwn(n.inputs,alt)));if(!found)continue;const actualKey=Object.hasOwn(found[1].inputs,key)?key:alt;if(c.bindings.some(b=>b.nodeId===found[0]&&inputPathParts(b.path).join('/')===actualKey))continue;const b=addInputBinding(found[0],actualKey,'sceneParameter');b.label=label;b.value='seed';b.type='number';count++}save();render();toast('已添加 '+count+' 个分镜参数映射。种子会为每一幕自动写入；宽高、步数等只在单幕启用渲染覆盖时写入。')}
