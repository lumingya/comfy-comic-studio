...
function installImageProviders(){
  ...
  const oldRender=render;render=function(){oldRender();const nonComfy=activeImageProfile().provider!=='comfyui';for(const id of ['ws-plan-workflow','ws-scene-workflow']){const el=$('#'+id);if(el){el.disabled=nonComfy;if(nonComfy)el.innerHTML='<option>无需工作流 · '+esc(activeImageProfile().title)+'</option>'}}document.querySelectorAll('[data-act="ws-edit-scene-workflow"]').forEach(el=>el.hidden=nonComfy);if(ui.workspace===3){const crumb=$('.breadcrumb strong');if(crumb)crumb.textContent='图像引擎'}};
