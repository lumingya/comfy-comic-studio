...
/* B19: the Python backend probes ComfyUI; the browser never calls /system_stats itself, so no CORS flags and no console noise. */
/* One probe implementation for boot, the 30 s poll and the workbench's 检查连接 button. Returns a record; never throws. */
...
function buildMappedWorkflow(frame,row,options={}){
 const execution=options.execution||frame._execution||{},comfy=state.settings.comfy,
  ...
  bindings=(execution.bindings||comfy.bindings).filter(b=>b.source!=='sceneParameter'||frame.renderOverride||String(b.value||'').trim()==='seed'),
 ...
 const compiled=WorkflowMapping.compile(workflow,bindings,{outputNodeId:execution.outputNodeId??comfy.outputNodeId,objectInfo:execution.objectInfo||comfy.objectInfo||{},resolve(binding){
    ...
    switch(binding.source){
      ...
      case'sceneParameter':value=frame[binding.value];if(value===undefined)throw Error('分镜参数 '+binding.value+' 不存在。');/* C1: a mapped seed is written for every scene; a scene's own seed only counts as a render override, otherwise each frame gets a fresh one. */if(binding.value==='seed'&&(Number(value)<0||!frame.renderOverride))value=options.preview?123456789:Math.floor(Math.random()*Number.MAX_SAFE_INTEGER);break;
 ...
 if(overrides&&(overrides.model||Array.isArray(overrides.loras)||overrides.unpin?.length)){
...
async function readComfyObjectInfo({quiet=false}={}){
  ...
  const kept={};for(const key of Object.keys(info))if(used.has(key)||['LoraLoader','LoraLoaderModelOnly'].includes(key))kept[key]=info[key];
  ...
  await ensureWorkflowSlotPlan(c,{force:true});for(const p of c.presets||[])await ensureWorkflowSlotPlan(p,{force:true});
...
/* The ordinary binding target receiving the positive prompt; v3 never falls back to prompt tags. */
...
function comfySlotsResolved(execution=state.settings.comfy){ensureWorkflowSlotPlan(execution).catch(()=>{});return WorkflowSlots.view(execution.slots)}
...
async function executeMappedGPU(frame,row,signal,sourceImage=null){
  ...
  const submitted=await(await request(endpoint+'/prompt',post({prompt:compiled.workflow,client_id:rt.clientId},signal))).json();if(!submitted.prompt_id)throw Error('ComfyUI 拒绝工作流：'+JSON.stringify(submitted.node_errors||submitted.error||submitted));log('提交工作流 '+snapshot.workflowTitle+' · '+submitted.prompt_id.slice(0,12));
...
function ensureImageProviders(s=state){
  s.settings.imageGeneration??={active:'comfyui',profiles:[
    ...
    {id:'openai',title:'OpenAI 兼容',provider:'openai',baseUrl:'https://api.openai.com/v1',model:'gpt-image-1',protocol:'images',size:'1024x1024',quality:'auto',sendSize:false,sendQuality:false,sendAspectHint:true,keyMode:'none'}]};
  ...
  for(const p of g.profiles){p.keyMode??='none';if(p.keyMode==='environment')p.keyMode='none';if(p.provider==='openai'){p.sendSize??=!!p.size;p.sendQuality??=!!p.quality;p.sendAspectHint??=true}}
...
function imageConfigSnapshot(p=activeImageProfile()){
  return Object.fromEntries(['id','title','provider','baseUrl','model','protocol','sampler','size','quality','sendSize','sendQuality','sendAspectHint','keyMode','keyId','keyIds','extraParams'].filter(k=>p[k]!==undefined).map(k=>[k,clone(p[k])]));
}
...
function imageProviderSelect(){const g=ensureImageProviders();return `<label class="label" for="image-provider-select">图像服务</label><select id="image-provider-select">${g.profiles.map(p=>opt(p.id,p.title,g.active)).join('')}</select>`}
...
function imageProviderPanel(){
  ...
  return `<section class="panel image-provider-panel"><div class="row between wrap"><h2>图像引擎</h2><div class="row wrap"><a class="btn small" href="/docs/index.html" target="_blank" rel="noopener">教程 / Docs ↗</a>${btn('新增图像服务','plus','image-provider-new','','small')}${p.provider!=='comfyui'?btn('复制','copy','image-provider-copy','','small')+btn('删除图像服务','trash','image-provider-delete','','small danger'):''}</div></div>${imageProviderSelect()}${providerSetupHTML(p)}${p.provider==='comfyui'?'':`<div class="grid2 provider-fields">${input('服务名称','title')}${input(p.provider==='novelai'?'NovelAI 图像服务地址':'API 基础地址','baseUrl')}<div class="provider-model-field">${p.provider==='openai'?`<label class="label" for="image-provider-model-input">模型 ID</label><div class="provider-model-line"><div class="provider-model-combobox"><input id="image-provider-model-input" type="text" data-image-config="model" value="${esc(p.model||'')}" role="combobox" aria-label="模型 ID" aria-autocomplete="list" aria-expanded="false" aria-controls="image-provider-model-results" autocomplete="off" spellcheck="false"><div id="image-provider-model-results" class="provider-model-results" role="listbox" aria-label="匹配模型" hidden></div></div>${btn(imageProviderUI.busy.has(p.id)?'正在获取…':'获取模型','refresh','image-provider-models',imageProviderUI.busy.has(p.id)?'disabled':'','small')}</div><p class="help" id="provider-model-status" role="status"></p>`:input('模型 ID','model')}</div>${imageProviderKeyFields(p)}${p.provider==='openai'?field('接口协议',`<select data-image-config="protocol">${opt('images','Images API',p.protocol)}${opt('chat','Chat Completions',p.protocol)}</select>`):input('采样器','sampler')}</div>${p.provider==='openai'?`<section class="provider-options"><h3>请求参数</h3>${images?`<div class="grid2">${optional('输出尺寸','size','sendSize','1024x1024 / auto')}${optional('质量','quality','sendQuality','auto / high')}</div>`:`<label class="row"><input type="checkbox" data-image-config="sendAspectHint" ${p.sendAspectHint!==false?'checked':''}><span>把分镜画幅比写进提示词</span></label><p class="help">Chat 协议没有尺寸字段。开启后，每一幕会在提示词末尾追加一句画幅说明，例如 “Output a single portrait image with aspect ratio 13:19 (832x1216).”，让模型按分镜的宽高比出图。</p>`}</section>`:''}<details class="quiet-advanced"><summary>高级请求参数 · JSON</summary><textarea id="provider-extra-params" spellcheck="false" aria-label="高级请求参数">${esc(JSON.stringify(p.extraParams||{},null,2))}</textarea></details>`}${providerSetupFooter()}</section>`;
}
...
async function handleImageProviderAction(action,d={},el){
  ...
  if(action==='image-provider-new'){modal('新增图像服务',field('接口类型',`<select id="image-new-type"><option value="openai">OpenAI 兼容</option><option value="novelai">NovelAI</option></select>`)+field('服务名称','<input id="image-new-title" autocomplete="off" placeholder="自定义图像服务">')+'<div class="modal-footer">'+btn('取消','','close-modal')+btn('创建','plus','image-provider-create','','primary')+'</div>');return true}
  if(action==='image-provider-create'){const type=$('#image-new-type').value,newProfile={id:uid('provider'),title:$('#image-new-title').value.trim()||(type==='novelai'?'NovelAI':'OpenAI 兼容'),provider:type,keyMode:'none',...(type==='novelai'?{baseUrl:'https://image.novelai.net',model:'nai-diffusion-4-5-full',sampler:'k_euler_ancestral'}:{baseUrl:'https://api.openai.com/v1',model:'gpt-image-1',protocol:'images',sendSize:false,sendQuality:false,sendAspectHint:true,size:'1024x1024',quality:'auto'})};g.profiles.push(newProfile);g.active=newProfile.id;closeModal();save();render();return true}
  ...
  if(action==='image-provider-delete'){
    if(p.provider==='comfyui')throw Error('内置 ComfyUI 入口无需删除；可以切换到其他图像服务。');
    if(providerHasPendingTasks(p.id))throw Error('有未完成任务引用此图像服务，请先停止并移除相关队列任务。');
    if(!await confirmAction('删除图像服务「'+p.title+'」？','删除图像服务及其密钥。','删除图像服务'))return true;
    if(providerHasPendingTasks(p.id))throw Error('图像服务已被新的任务引用，未删除。');
  ...
  if(action.startsWith('image-key-')){
    const target=g.profiles.find(x=>x.id===d.id);if(!target)throw Error('图像服务已删除。');
    if(action==='image-key-save'||action==='image-key-save-inline'){
      ...
      try{const result=await imageCredentialRequest('add',target,{key,label});input.value='';if(target.baseUrl!==endpoint||!g.profiles.includes(target)){throw Error('密钥已保存到原地址，请在该地址的密钥管理中选择。')}target.keyMode='stored';target.keyId=result.key.id;target.keyIds=[result.key.id];target.keyLabel=result.key.label;firstRunUI.keyNotices.delete(target.id);save();if(!await savePythonWorkspace())throw Error('密钥已保存，但图像服务引用尚未确认写入；请检查服务后重新选择密钥。');if(!inline&&$('#modal').contains(input))closeModal();toast('已保存')}finally{imageProviderUI.savingKeys.delete(target.id);if(el?.isConnected)el.disabled=false;if(input.isConnected&&!input.value)render()}return true;
    }
    if(action==='image-key-use'){target.keyMode='stored';target.keyId=d.key;target.keyIds=[d.key];target.keyLabel=d.label;firstRunUI.keyNotices.delete(target.id);save();if(!await savePythonWorkspace())throw Error('图像服务密钥引用尚未保存，请检查服务后重试。');closeModal();render();return true}
...
async function generateProviderFrame(frame,row,signal,source=null){
  ...
  if(!p||!['novelai','openai'].includes(ex.provider))throw Error('图像服务快照无效，请重新入队。');
  ...
  const result=await response.json();if(!result.image)throw Error('图像服务未返回图片。');return result;
}
function installImageProviders(){
  ...
  const oldBuild=buildMappedWorkflow;buildMappedWorkflow=function(frame,row,options={}){const ex=options.execution||frame._execution;if(ex?.provider&&ex.provider!=='comfyui'){validatePrompt(scopeText(frame.prompt,frame._scope||row._scope||row,true));if(!ex.config?.model||!ex.config?.baseUrl)throw Error('请先填写图像服务地址和模型。');return {workflow:{},changes:[]}}return oldBuild(frame,row,options)};
  ...
  const oldComposer=queueComposerHTML;queueComposerHTML=function(){const html=oldComposer();return '<div class="queue-provider-picker">'+imageProviderSelect()+btn('配置图像服务','settings','image-provider-settings','','small')+'</div>'+html};
...
/* Transport/cache only: the browser never infers semantic slots. Stale responses are discarded. */
const slotAnalysisRequests = new WeakMap();
async function ensureWorkflowSlotPlan(entry=state.settings.comfy, {force=false,manual}={}) {
  const info=entry.objectInfo||state.settings.comfy.objectInfo||{};
  const graph=JSON.stringify(entry.workflow||{}), infoKey=JSON.stringify(info), previous=slotAnalysisRequests.get(entry);
  const signature=graph+'\n'+infoKey;
  if(!force&&previous?.signature===signature)return previous.promise;
  if(!force&&!previous&&entry.slots?.plan?.version===3){slotAnalysisRequests.set(entry,{signature,promise:Promise.resolve(entry.slots.plan)});return entry.slots.plan;}
  const promise=(async()=>{
    const response=await request('/api/production/analyze-slots',post({workflow:entry.workflow||{},objectInfo:info,slots:entry.slots||{},manual:manual??entry.slots?.plan?.manual??[]}));
    const data=await response.json();if(!response.ok||data.error)throw Error(data.error||'映射分析失败');
    if(JSON.stringify(entry.workflow||{})!==graph||JSON.stringify(entry.objectInfo||state.settings.comfy.objectInfo||{})!==infoKey)return null;
    entry.slots={plan:data.data.plan};
    const c=state.settings.comfy;
    if(entry===c)storeActiveWorkflow();
    else if(entry.id===c.activeWorkflowId&&JSON.stringify(c.workflow)===graph)c.slots=clone(entry.slots);
    save();if(ui.workspace===3&&!document.querySelector('#modal')?.open)render();return entry.slots.plan;
  })();
  slotAnalysisRequests.set(entry,{signature,promise});return promise;
}
