...
function installImageProviders(){
  ...
  const oldLibrary=renderWorkflowLibrary;renderWorkflowLibrary=()=>activeImageProfile().provider==='comfyui'?renderWorkflowPageHeading()+'<details class="wm-connection" data-wm-connection '+(mapperUI.connectionOpen!==false?'open':'')+'><summary>'+icon('nodes','sm')+'<strong>ComfyUI</strong><span>图像引擎与连接设置</span><span class="wm-connection-mode">'+(state.settings.comfy.mode==='mock'?'离线演示':'真实服务')+'</span></summary>'+imageProviderPanel()+'</details>'+oldLibrary():imageProviderPanel();
