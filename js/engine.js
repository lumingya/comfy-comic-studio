...
async function testEngine(silent=false){if(state.settings.comfy.mode==='mock'){rt.connected=false;rt.latency=null;rt.vram=null;if(!silent)toast('离线 SVG 引擎已就绪，无需 GPU。');return true}if(rt.engineCheck)return rt.engineCheck;rt.engineCheck=(async()=>{try{const response=await request('/api/image/comfy-check',post({baseUrl:baseURL()}),12000),data=await response.json();if(!response.ok||data.error||!data.ok)throw Error(data.error||data.message||'连接失败');rt.latency=Number.isFinite(data.latencyMs)?data.latencyMs:null;rt.vram=Number.isFinite(data.vramPercent)?data.vramPercent:null;rt.connected=true;connectWS();renderShell();if($('#connection-result'))$('#connection-result').textContent='已连接 · '+(rt.latency??'—')+' ms'+(rt.vram!==null?' · VRAM '+rt.vram+'%':'');if(!silent)toast('ComfyUI 连接成功，延迟 '+(rt.latency??'—')+' ms');return true}catch(e){const message=String(e?.message||'连接失败').replace(/^HTTP \d+: /,'');rt.connected=false;rt.latency=null;renderShell();if($('#connection-result'))$('#connection-result').textContent='未连接 · '+message;if(!silent)toast('连接失败：'+message,'error');return false}finally{rt.engineCheck=null}})();return rt.engineCheck}
...
function installImageProviders(){
  ...
  // The connection panel holds the address, connection check and workflow import.
  // It stays open until the user collapses it, and the choice survives re-renders
  // (a full render() runs after every connection check), so results are never hidden.
  const oldLibrary=renderWorkflowLibrary;renderWorkflowLibrary=()=>activeImageProfile().provider==='comfyui'?'<details class="wm-connection" data-wm-connection '+(mapperUI.connectionOpen!==false?'open':'')+'><summary>'+icon('nodes','sm')+'<strong>ComfyUI</strong><span>图像引擎与连接设置</span><span class="wm-connection-mode">'+(state.settings.comfy.mode==='mock'?'离线演示':'真实服务')+'</span></summary>'+imageProviderPanel()+'</details>'+oldLibrary():imageProviderPanel();
