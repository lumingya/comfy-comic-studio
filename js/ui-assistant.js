...
function renderScopedAssistant(){
  ...
  <div class="assistant-scope"><div class="row"><span>${rt.chatBusy?'本次操作已锁定目标':'作用范围 / SOURCE TEMPLATE'}</span><span class="spacer"></span><span class="accent tiny">${llmMock()?'离线规则模式':'真实模型 · '+esc(state.settings.llm.model)}</span></div><div class="row"><strong class="grow">${esc(scope?.title||'尚未选择分镜模板')}</strong>${scope&&workspaceVisible(1)?ibtn('arrow','assistant-open-target','在分镜工作区查看此模板'):''}</div></div>
...
function openCriticSettings(){
  ...
  $('#critic-dialog').innerHTML=`<header class="modal-head">${icon('shield')}<div class="grow"><h2 id="critic-title">视觉审校 · API 设置</h2><p>只用于视觉审校，不影响其他工具或生成渠道。</p></div>${ibtn('close','critic-close','关闭审校配置')}</header><div class="service-body">${field('服务地址',input('baseUrl',c.baseUrl,'url','data-critic-field="baseUrl" placeholder="https://api.example.com/v1" autocomplete="off"'))}${field('API 密钥',`<div class="service-key">${input('key',c.key,'password','data-critic-field="key" autocomplete="off" spellcheck="false" aria-label="视觉审校 API 密钥"')}${ibtn('eye','critic-key-toggle','显示或隐藏密钥')}</div>`)}${field('视觉模型',input('model',c.model,'text','data-critic-field="model" placeholder="填写支持图像输入的模型名称"'))}<details class="critic-advanced"><summary>审校选项与连接测试</summary>${field('运行模式',`<select data-critic-field="mode">${opt('real','真实视觉 API',c.mode)}${opt('mock','离线演示',c.mode)}</select>`)}${field('超时（秒）',input('timeout',c.timeout,'number','data-critic-field="timeout" min="15" max="180"'))}${['includeReference','includePrevious'].map((key,i)=>`<label class="row" style="justify-content:space-between;margin:18px 0"><span>${i?'附带前一幕画面':'附带角色参考图'}</span><span class="switch"><input type="checkbox" role="switch" data-critic-field="${key}" ${c[key]?'checked':''}><span class="switch-track"></span></span></label>`).join('')}${field('审校重点',`<textarea data-critic-field="focus">${esc(c.focus)}</textarea>`)}<div id="critic-test-result" class="service-test-result">${esc(criticReadyLabel())}</div>${releaseUI.criticTesting?btn('停止测试','stop','critic-test-stop'):btn('带图测试连接','image','critic-test')}</details><p class="help">密钥遵循工作室的私密凭据保存策略。请勿填入不可信服务。</p></div><footer class="service-footer">${btn('保存并关闭','check','critic-close','','primary')}</footer>`;
