...
function renderScopedAssistant(){
  ...
  <div class="assistant-scope"><div class="row"><span>${rt.chatBusy?'本次操作已锁定目标':'作用范围 / SOURCE TEMPLATE'}</span><span class="spacer"></span><span class="accent tiny">${llmMock()?'离线规则模式':'真实模型 · '+esc(state.settings.llm.model)}</span></div><div class="row"><strong class="grow">${esc(scope?.title||'尚未选择分镜')}</strong>${scope&&workspaceVisible(1)?ibtn('arrow','assistant-open-target','在分镜工作区查看此模板'):''}</div></div>
...
function decorateAssistantContext(){
  ...
  if(scope)scope.innerHTML=`<div class="row"><span class="scope-project">${esc(proj?.title||'未绑定画册集')}</span><span class="spacer"></span><span class="tiny muted">${rt.chatBusy?'处理中 · 目标已锁定':llmMock()?'离线规则模式':'真实模型'}</span></div><div class="row"><strong class="grow">${esc(template?.title||'请明确选择要修改的源模板')}</strong>${template&&workspaceVisible(1)?ibtn('arrow','assistant-open-target','在工作区打开绑定模板'):''}</div>`;
  if(!$('.assistant-target-context',container))scope?.insertAdjacentHTML('afterend',`<div class="assistant-target-context"><label class="label" for="assistant-target">修改目标</label><select class="assistant-target-select" id="assistant-target" ${rt.chatBusy?'disabled':''}>${state.templates.map(t=>opt(t.id,(state.projects.find(p=>p.id===t.projectId)?.title||'画册集')+' / '+t.title,template?.id)).join('')||'<option value="">尚无模板</option>'}</select></div>`);
...
function openCriticSettings(){
  const c=state.settings.critic;releaseUI.criticDraft=c;c.connection='independent';
  $('#critic-dialog').innerHTML=`<header class="modal-head">${icon('shield')}<div class="grow"><h2 id="critic-title">视觉审校 · API 设置</h2><p>只用于视觉审校。</p></div>${ibtn('close','critic-close','关闭审校配置')}</header><div class="service-body">${field('服务地址',input('baseUrl',c.baseUrl,'url','data-critic-field="baseUrl" placeholder="https://api.example.com/v1" autocomplete="off"'))}${field('API 密钥',`<div class="service-key">${input('key',c.key,'password','data-critic-field="key" autocomplete="off" spellcheck="false" aria-label="视觉审校 API 密钥"')}${ibtn('eye','critic-key-toggle','显示或隐藏密钥')}</div>`)}${field('视觉模型',input('model',c.model,'text','data-critic-field="model" placeholder="填写支持图像输入的模型名称"'))}<details class="critic-advanced"><summary>审校选项与连接测试</summary>${field('运行模式',`<select data-critic-field="mode">${opt('real','真实视觉 API',c.mode)}</select>`)}${field('超时（秒）',input('timeout',c.timeout,'number','data-critic-field="timeout" min="15" max="180"'))}${['includeReference','includePrevious'].map((key,i)=>`<label class="row" style="justify-content:space-between;margin:18px 0"><span>${i?'附带前一幕画面':'附带参考图'}</span><span class="switch"><input type="checkbox" role="switch" data-critic-field="${key}" ${c[key]?'checked':''}><span class="switch-track"></span></span></label>`).join('')}${field('审校重点',`<textarea data-critic-field="focus">${esc(c.focus)}</textarea>`)}<div id="critic-test-result" class="service-test-result">${esc(criticReadyLabel())}</div>${releaseUI.criticTesting?btn('停止测试','stop','critic-test-stop'):btn('带图测试连接','image','critic-test')}</details><p class="help">密钥遵循工作室的私密凭据保存策略。请勿填入不可信服务。</p></div><footer class="service-footer">${btn('保存并关闭','check','critic-close','','primary')}</footer>`;
...
function renderCriticEntry(){
  ...
  box.hidden=!featureEnabled('visualCritic');box.innerHTML=`<div class="row"><h3 class="grow">视觉审校</h3>${icon('shield','sm')}</div><p>${esc(criticReadyLabel())}</p><div class="row wrap">${''}${releaseUI.criticBatch?btn('停止整册审校','stop','critic-batch-stop','','small'):btn('审校整本画册','eye','critic-batch','','small')}</div>${releaseUI.criticBatch?`<div class="critic-progress">${releaseUI.criticBatch.done} / ${releaseUI.criticBatch.total} 页 · ${esc(releaseUI.criticBatch.title)}</div>`:''}`;
}
...
async function performVisualCritique(id,index,automatic=false,parentSignal=null){
  ...
  try{let report;
    if(cfg.mode==='mock'){await delay(600,controller.signal);report=normalizeCriticReport({score:7.5,summary:'离线演示报告，用于体验从审校到精修的操作流程。未向视觉模型发送此图。',anatomy:'示例：真实审校将检查手部指节、眼睛位置与肢体比例。',consistency:'示例：真实审校会对照可选的角色立绘与前序画面。',suggestions:'consistent character features, refined hands, coherent lighting',issues:[{area:'演示建议',severity:'low',observation:'连接视觉 API 后，此处会展示基于实际图片的问题与修改建议。',fixPrompt:'consistent character features, refined hands'}]})}
      ...
      let image;try{image=await rasterJPEG(snapshot)}catch(e){throw Error('无法读取当前原图，审校已停止。请用“图片替换”上传这张图的本地文件，或为图片服务启用 CORS。')}
  ...
  }catch(e){const message=safeServiceError(e);if(area?.isConnected)area.innerHTML=`<div class="service-note"><span class="danger">${esc(message)}</span><div class="critic-actions">${''}${btn('重试审校','refresh','critique',`data-index="${index}"`,'small')}</div></div>`;throw Error(message)}
