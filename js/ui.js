...
function renderQuietCreation(){const p=selectedPlan();if(p)createUI.planId=p.id;const tab=createUI.tab==='queue'?'queue':createUI.tab==='variables'||createUI.tab==='settings'?'settings':'story';return `<div class="quiet-create-head"><div class="grow"><h1>创作画册</h1><p>画面由你定义，故事自由发生。</p></div>${p?creationContextSelector(p):''}${btn('新画册','plus','new-book','','small')}${p?btn('生成画册','play','v3-generate-plan','','primary'):''}</div><nav class="quiet-tabs" aria-label="创作内容">${[['story','分镜故事'],['settings','角色与画面设定'],['queue','生成队列']].map(([id,title])=>`<button class="${tab===id?'active':''}" data-act="art-create-tab" data-tab="${id}" ${tab===id?'aria-current="page"':''}>${title}</button>`).join('')}</nav>${!p?`<div class="empty"><h3>先给一本新画册起名。</h3><p>再写下分镜，或从十二幕示范开始。</p>${btn('新建画册','plus','new-book','','primary')}</div>`:tab==='settings'?renderUnifiedSettings():tab==='queue'?renderCreationQueue():renderQuietStoryboard(p)}`}
...
function addSettingFromDialog(){const p=settingsTargetById(rt.settingTargetId);if(!p)throw Error('请先选择一本画册。');const key=$('#art-new-setting-key').value.trim(),label=$('#art-new-setting-label').value.trim(),type=$('#art-new-setting-type').value;checkVariableKey(key);if(mergedSettingEntries(p).some(e=>e.key===key))throw Error('已有同名属性，可直接编辑它。');p.variables=p.variables.filter(e=>e.key!==key);p.variables.push({id:uid('var'),key,label:label||key,type,value:type==='number'?0:type==='boolean'?false:type==='json'?'{}':''});p.excludedSettingKeys=(p.excludedSettingKeys||[]).filter(x=>x!==key);if(p._presetEditorId)p.dirty=true;save();closeModal();render();if(p._presetEditorId)openPresetLibrary(p._presetEditorId)}
