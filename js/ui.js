...
function renderStudioMarket(){
  ...
  $('#market-content').innerHTML=`<div class="tabbar market-subtabs" style="gap:20px">${[['all','全部'],['exports','画册 HTML 模板'],['templates','分镜剧本'],['characters','角色资产'],['workflows','工作流'],['rules','审校规则']].map(([v,l])=>`<button class="tab ${ui.marketTab===v?'active':''}" data-act="market-tab" data-tab="${v}">${l}</button>`).join('')}</div><div class="row" style="margin-top:18px"><div class="search-field" style="max-width:none">${icon('search')}<input id="market-search" value="${esc(ui.marketSearch)}" placeholder="搜索画册模板、作者或标签..." aria-label="搜索市场资源"></div>${studioUI.returnToExport?btn('返回画册导出','arrow','et-return-export','','small'):''}</div><div class="row wrap" style="margin-top:12px">${['画册','治愈','赛博','极简'].map(t=>btn(t,'','market-tag',`data-tag="${t}"`,'small ghost')).join('')}<span class="spacer"></span><span class="tiny muted">内置精选 · 可离线安装与下载</span></div><div class="market-grid">${items.map(c=>marketCardHTML(c,installedIds.has(c.id))).join('')||'<div class="empty">没有匹配的组件，试试其他标签。</div>'}</div><div class="divider"></div><h3 class="panel-title">从 GitHub / Gitee / 自有仓库导入</h3><div class="row"><input id="raw-url" placeholder="粘贴 Raw JSON 或 HTML 模板链接" aria-label="外部组件地址">${btn('解析并导入','download','import-raw')}</div>`;
}
...
function textualInterfaceCopy(text){return String(text).replaceAll('智能节点映射器','工作流配置').replaceAll('智能节点映射','工作流配置').replaceAll('实验室','可选功能').replaceAll('LABORATORY / OPTIONAL TOOLS','EXTENSIONS / OPTIONAL TOOLS').replaceAll('创作企划','画册集').replaceAll('企划','画册集').replaceAll('画册计划','画册').replaceAll('创作计划','画册创作').replaceAll('角色矩阵','角色与画面设定').replaceAll('可复用素材','画面设定').replaceAll('本册专属变量','角色与画面设定').replaceAll('变量素材','设定预设')}
