...
function backupModal(){modal('工程备份与迁移',`<div class="notice">备份包含所有画册集、模板、角色变量、版本、画册图片索引、渲染队列、工作流映射及配置。恢复前会执行结构校验与覆盖确认。</div><div class="row" style="margin-top:20px">${btn('导出全量工程 JSON','download','backup-export','','primary')}${btn('从 JSON 恢复','upload','import-project')}</div><label class="row small soft" style="margin-top:17px"><input type="checkbox" id="backup-secrets">包含 API 密钥（敏感，请妥善保管）</label><div class="divider"></div><h3 class="panel-title">${icon('disk')}双向远端存储</h3>${field('自有 JSON 存储端点',setting('syncUrl',state.settings.syncUrl),'此端点需支持 GET 与 PUT application/json，并允许浏览器 CORS。未配置时只保存在本地，不会上传数据。')}<div class="row">${btn('推送至远端','upload','sync-push')}${btn('拉取远端工程','download','sync-pull')}</div><p class="help" id="sync-status">${esc(rt.syncStatus||'远端未同步 · 防冲刷保护已开启')}</p><div class="divider"></div><p class="tiny muted">当前工程：${state.projects.length} 个画册集 / ${state.books.length} 本画册 / ${state.templates.length} 个模板</p>`,'LOCAL FIRST / FULL PROJECT BACKUP')}
...
function guideTaskStatus(index){const t=guidePreferences();if(index===0)return disk.root&&rt.saved?'已连接并写入本地目录。':'尚未落盘，也可以先在临时会话中练习。';if(index===3)return releaseUI.practiceBusy?'正在本地生成练习分镜...':bookBy(t.practiceBookId)?.status==='complete'?'三幕练习已生成，可进入下一步阅读。':'练习会新建独立画册集，不会改动你的已有作品。';if(index===4)return criticReadyLabel();if(index===6)return t.publishedAt?'已完成过一次 GitHub 提交。':'上传是可选步骤，不影响本地创作。';return ''}
...
function renderCurrentIdentity(){const identity=state.settings.identity;return `<section class="settings-section"><h2>你的工作室</h2><p>名称显示在工作室侧栏。随时可以修改，不改变已有画册集、画册或变量素材。</p>${field('工作室名称',input('workspaceName',identity.workspaceName,'text','id="identity-workspace" maxlength="40"'))}${field('创作者署名',input('creatorName',identity.creatorName||'','text','id="identity-creator" maxlength="60"'))}<label class="row small soft" style="margin-bottom:19px"><input id="identity-sync-signature" type="checkbox" checked>同时更新默认画册签名</label>${btn('保存名称与署名','check','identity-save','','primary')}</section><section class="settings-section"><h2>当前保存方式</h2><div class="service-context">${esc(backendStatusText())}</div><div class="row" style="margin-top:17px">${btn('配置服务与保存','disk','v3-settings-tab','data-tab="connections"')}${btn('导出备份','download','backup-export')}</div></section>`}
...
function updateArtFontStatus(){const label=$('#art-font-status');if(label)label.textContent=state.settings.presentation.fonts===false?'外部字体已关闭，使用系统衬线字体。':artUI.fontStatus==='loaded'?'艺术字体样式已加载，实际字形按需下载并由浏览器缓存。':artUI.fontStatus==='loading'?'正在加载艺术字体；现在仍可使用系统衬线字体。':'艺术字体暂不可用，已使用系统衬线字体，不影响创作与阅读。'}
...
function addUnifiedSetting(groupId=''){rt.settingTargetId=settingsEditorTarget()?.id;modal('新增画面属性',`${field('显示名称',input('label','','text','id="art-new-setting-label" placeholder="例如：道具"'))}${field('变量标识符',input('key','','text','id="art-new-setting-key" placeholder="例如：weapon"'))}${field('属性类型',`<select id="art-new-setting-type">${Object.entries(variableTypes).map(([key,label])=>opt(key,label,'text')).join('')}</select>`)}${field('归属分组',`<select id="art-new-setting-group">${settingsGroupOptions(settingsEditorTarget(),groupId)}</select>`)}<div class="modal-footer">${btn('取消','','close-modal')}${btn('添加属性','plus','art-setting-confirm','','primary')}</div>`)}
...
function laboratoryExtensions(){return [
  ...
  {key:'assistant',icon:'spark',title:'悬浮精修助手',description:'用自然语言调整源分镜，不改动已有画册。',enabled:featureEnabled('assistant')}
]}
