/* ComfyComic development module: ui. */
'use strict';
...
function modal(title,body,sub='',wide=false){$('#modal').classList.toggle('modal-wide',wide);$('#modal-title').textContent=title;$('#modal-subtitle').textContent=sub;$('#modal-body').innerHTML=body;if(!$('#modal').open)$('#modal').showModal()}
...
<article class="cc-book" data-cc-book>
  <header class="cc-cover">
    <p class="cc-eyebrow">COMFYCOMIC / {{layoutName}}</p>
    <h1>{{title}}</h1>
...
function renderScopedAssistant(){
  ...
  $('#assistant').innerHTML=`<header class="assistant-head" id="assistant-drag">${icon('spark')}<div class="grow"><strong>分镜精修助手</strong><small>用自然语言，修改提示词与台词</small></div>${ibtn('settings','llm-settings','助手模型与连接设置')}${ibtn('expand','assistant-full','展开或还原窗口')}${ibtn('close','assistant-close','收起精修助手')}</header>
  ...
  <div class="chat-messages" id="chat-messages">${!hasUser?`<section class="assistant-intro"><h3>把想改的分镜，<br>直接告诉我。</h3><p>改场景、润色台词、增删镜头，或重新编排顺序。修改前会展示目标并请你确认。</p>${[['edit','把第 1 幕改成雨夜车站','调整场景，保留角色宏变量'],['story','把第 2 幕的台词改成：我们终于又见面了。','只改台词，不改画面提示词'],['plus','新增一幕：她在海边找到那封信。','为当前故事接上一幕']].map(([ic,v,l])=>`<button class="assistant-suggestion" data-act="assistant-suggestion" data-text="${esc(v)}">${icon(ic)}<span class="grow">${esc(v)}<small>${l}</small></span>${icon('arrow','sm')}</button>`).join('')}<p style="margin-top:13px;margin-bottom:0;font-size:10px">${llmMock()?'离线模式可执行以上明确指令；图片理解和复杂讨论需要连接真实模型。':'你输入的文本与附件会发送到已配置的模型服务。'}</p></section>`:''}
  ${chat.messages.map(m=>{
...
function updateAssistantDisclosure(){const p=$('.assistant-intro p');if(p)p.textContent='改场景、润色台词、增删镜头，或重新编排顺序。'+(state.settings.studio.assistant.confirmChanges?'应用修改前会显示目标并请你确认。':'当前已关闭逐次确认，明确的修改指令会直接应用。');}
...
function renderStudioMarket(){
  ...
  $('#market-content').innerHTML=`<div class="tabbar market-subtabs" style="gap:20px">${[['all','全部'],['exports','画册 HTML 模板'],['templates','分镜剧本'],['characters','角色资产'],['workflows','工作流'],['rules','审校规则']].map(([v,l])=>`<button class="tab ${ui.marketTab===v?'active':''}" data-act="market-tab" data-tab="${v}">${l}</button>`).join('')}</div><div class="row" style="margin-top:18px"><div class="search-field" style="max-width:none">${icon('search')}<input id="market-search" value="${esc(ui.marketSearch)}" placeholder="搜索画册模板、作者或标签..." aria-label="搜索市场资源"></div>${studioUI.returnToExport?btn('返回画册导出','arrow','et-return-export','','small'):''}</div><div class="row wrap" style="margin-top:12px">${['画册','治愈','赛博','极简'].map(t=>btn(t,'','market-tag',`data-tag="${t}"`,'small ghost')).join('')}<span class="spacer"></span><span class="tiny muted">内置精选 · 可离线安装与下载</span></div><div class="market-grid">${items.map(c=>{
    const installed=state.installedPackages.some(p=>p.id===c.id);
    return `<article class="market-item">${c.type==='exports'?`<button class="market-preview-strip" style="width:100%;background:${c.options.background};--mini-accent:${c.options.accent}" data-act="et-market-preview" data-id="${c.id}" aria-label="预览 ${esc(c.title)}"><span class="mini-page"><i></i><span></span></span><span class="mini-page"><i></i><span></span></span></button>`:`<div class="row accent">${icon(c.icon)}<span class="spacer"></span><span class="tiny">v${esc(c.version)}</span></div>`}<div class="row"><h3 class="grow" style="margin:0 0 7px">${esc(c.title)}</h3>${c.type==='exports'?'<span class="utility-caption">HTML / CSS</span>':''}</div><p>${esc(c.desc)}</p><div class="tiny muted" style="margin-bottom:13px">${esc(c.author)} · v${esc(c.version)}</div><div class="row">${c.type==='exports'?btn('预览','eye','et-market-preview',`data-id="${c.id}"`,'small')+btn('下载模板','download','et-market-download',`data-id="${c.id}"`,'small'):''}<span class="spacer"></span>${btn(installed?'卸载':'安装',installed?'check':'plus','market-install',`data-id="${c.id}"`,'small '+(installed?'':'primary'))}</div></article>`;
  }).join('')||'<div class="empty">没有匹配的组件，试试其他标签。</div>'}</div><div class="divider"></div><h3 class="panel-title">从 GitHub / Gitee / 自有仓库导入</h3><div class="row"><input id="raw-url" placeholder="粘贴 Raw JSON 或 HTML 模板链接" aria-label="外部组件地址">${btn('解析并导入','download','import-raw')}</div><div class="help">支持画册 HTML 模板包与原有分镜、工作流 JSON。GitHub /blob/ 链接自动转换为 Raw，外部模板须通过离线安全校验。</div>`;
}
...
function hideTip(){clearTimeout(detailUI.tooltipTimer);const tip=$('#ui-tooltip');if(tip)tip.hidden=true;detailUI.tooltipTarget?.removeAttribute('aria-describedby');detailUI.tooltipTarget=null}
...
function decorateAssistantContext(){
  ...
  if(!$('.assistant-target-context',container))scope?.insertAdjacentHTML('afterend',`<div class="assistant-target-context"><label class="label" for="assistant-target">修改目标 / 企划与源分镜模板</label><select class="assistant-target-select" id="assistant-target" ${rt.chatBusy?'disabled':''}>${state.templates.map(t=>opt(t.id,(state.projects.find(p=>p.id===t.projectId)?.title||'企划')+' / '+t.title,template?.id)).join('')||'<option value="">尚无模板</option>'}</select></div>`);
...
function storageSettingsHTML(){
  ...
  return `<section class="settings-section"><h2>普通文件，真正属于你</h2><p>不再把浏览器数据库作为作品主存储。每个企划、每本画册都有自己的文件夹，图片单独保存。</p><div class="disk-info"><div class="disk-info-header">${icon('folder')}<div class="grow"><h3>${esc(connected?disk.root.name:disk.remembered?'待连接：'+disk.remembered.name:'尚未选择工作室根目录')}</h3><p id="disk-settings-status">${esc(diskStatusText())}</p></div><span class="chip ${connected?'green':''}">${connected?'磁盘目录':'临时会话'}</span></div><div class="row wrap">${btn(connected?'切换工作室目录':'选择本地根目录','folder','disk-connect','','primary')}${disk.remembered&&!connected||disk.phase==='permission'?btn('重新授权','refresh','disk-reconnect'):''}${btn('立即保存','disk','disk-save',connected&&!disk.busy?'':'disabled')}${connected?btn('重新读取磁盘','refresh','disk-reload',disk.busy?'disabled':''):''}</div><div class="disk-progress-caption" id="disk-save-progress">${esc(disk.progress||'只在所选目录范围内读写；不会访问其他文件夹。')}</div>${disk.lastError?`<div class="disk-warning">${esc(disk.lastError)}</div>`:''}${disk.phase==='recovery'?`<div class="disk-warning">当前索引未完整读取，已从 workspace.previous.json 恢复上一次完整记录。请检查画册后确认恢复，再继续保存。${btn('确认恢复并继续保存','check','disk-accept-recovery','style="margin-top:12px"','small')}</div>`:''}</div>${!supported?'<div class="disk-warning">当前浏览器或嵌入页面未开放本地目录写入。请在桌面 Chrome / Edge 的 HTTPS 或 localhost 页面直接打开；当前仍可下载 ZIP 目录包、只读载入已解压目录。</div>':''}<div class="settings-note">${icon('shield')}<span>浏览器仅记住目录授权、工作室名称和主题偏好。图片、剧情、模板与队列保存为磁盘文件。旧版浏览器工程只用于迁移，不再自动写回缓存。</span></div></section><section class="settings-section"><h2>清晰的目录层级</h2><p>名称便于辨认，短 ID 避免同名冲突。重命名只改变显示名称，已建立的目录路径保持稳定。</p><pre class="folder-tree">${esc(disk.root?.name||workspaceName())}/
  ...
  工作室/settings.json
  .history/ <span class="muted">完整索引和版本记录</span></pre></section><section class="settings-section"><h2>安全与迁移</h2>${settingsRow('把 API 密钥也写入本地目录','适用于 LLM 等旧服务设置。开启后 settings.json 可能含明文密钥；图像渠道密钥由独立本地文件管理，不受此开关控制。',`<label class="switch"><input role="switch" type="checkbox" data-disk-key="includeKeys" ${state.settings.disk.includeKeys?'checked':''} aria-label="将 API 密钥写入磁盘"><span class="switch-track"></span></label>`,'shield')}<div class="row wrap" style="padding:20px 0">${btn('下载完整目录 ZIP','download','disk-archive')}${btn('载入已解压目录','upload','disk-import-folder')}${btn('下载工程 JSON','disk','backup-export')}</div><p class="help">ZIP 保留相同目录层级，可解压后用“选择本地根目录”继续创作。浏览器不支持写入时，只读载入不会冒充自动保存。</p>${disk.legacy?`<div class="divider"></div><p class="small soft">检测到旧版浏览器工程。请确认已将需要的作品迁移至磁盘后，再清理旧缓存。</p>${btn('清理旧版浏览器工程缓存','trash','disk-clear-legacy',!connected||!rt.saved?'disabled':'','small danger')}`:''}</section>`;
}
...
async function clearLegacyCache(){
  ...
  if(disk.legacy?.books.some(b=>!state.books.some(x=>x.id===b.id)))throw Error('当前磁盘工程未包含所有旧版画册。为避免误删，请先迁移或备份旧工程。');
  if(!await confirmAction('清理旧版浏览器工程缓存？','只移除旧 IndexedDB/localStorage 中的工程副本。目录授权与磁盘原文件会保留。','清理旧缓存'))return;
  ...
  disk.legacy=null;render();toast('旧版浏览器工程缓存已清理，磁盘文件不受影响。');
}
...
function boundAssistantComposer(){
  ...
  const help=[...compose.children].find(e=>e.classList.contains('help'));if(help){help.classList.add('composer-help');help.textContent='Enter 发送 · Shift + Enter 换行 · 拖动上沿调整输入区'}
...
function queueLocalization(){cancelAnimationFrame(displayUI.frame);displayUI.frame=requestAnimationFrame(()=>localizeWorkspace())}
