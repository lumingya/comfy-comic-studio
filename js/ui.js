...
function createWorkspaceChromePolicy(){
  function navigation(visibility={}){return [[0,'book','画册集','1'],[1,'story','创作工坊','2'],[3,'nodes','工作流与 API 配置','3'],...(visibility.extensions===true?[[7,'toolbox','可选功能','']]:[]),...(visibility.marketplace===true?[[8,'box','模板与扩展市场','']]:[]),...(visibility.llm===true?[[4,'spark','AI 写故事','4']]:[]),[5,'settings','设置',','],...(visibility.logs===true?[[6,'terminal','运行日志','']]:[])]}
...
function changeInterfaceLanguage(language){if(!['en','zh-CN'].includes(language))return;state.settings.presentation.language=language;displayUI.languageTouched=true;try{localStorage.setItem('cc-language',language)}catch(e){}applyDisplayAttributes();save();localizeWorkspace();decorateDisplayPreferences();if(ui.workspace===0){patchMain('<div class="art-fade">'+renderResponsiveCollection()+'</div>');fitVisibleCovers()}updateCollectionFontStatus()}
