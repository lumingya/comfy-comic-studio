...
function installStyleStudio(){
  Object.assign(v3Actions,{
    ...
    'ss-theme-remove':async d=>{const t=ssThemeRecord(d.id);if(await confirmAction(t?.source==='link'?'取消链接主题？':'卸载主题？',t?.source==='link'?'':'主题样式、图标包和随包资源将删除。',t?.source==='link'?'取消链接':'卸载')){await ecoRequest('themes/uninstall',{id:d.id});await refreshEcosystem();render()}},
    ...
    'ss-token-reset-all':async()=>{if(!await confirmAction('清空全部令牌覆盖？','三种模式下的所有变量覆盖都会移除。','清空'))return;styleStudio.tokens={shared:{},dark:{},light:{}};ssApplyPreview();ssScheduleTokenSave();render()},
