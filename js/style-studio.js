...
function ssRenderTokens(){
  ...
  return `<div class="ss-token-bar"><div class="quiet-tabs ss-mode-tabs">${[['shared','通用（两种模式）'],['light','仅浅色'],['dark','仅深色']].map(([id,label])=>`<button type="button" class="${mode===id?'active':''}" data-act="ss-token-mode" data-mode="${id}">${label}</button>`).join('')}</div><input type="search" id="ss-token-search" value="${esc(styleStudio.tokenQuery)}" placeholder="搜索变量，例如 accent / bg / reader" aria-label="搜索设计令牌"><span class="soft small" id="ss-token-status">${count?'已覆盖 '+count+' 项':'自动保存'}</span>${count?btn('全部恢复','refresh','ss-token-reset-all','','small ghost'):''}</div><p class="soft small">这些是当前样式表里发现的 CSS 变量（官方 + 主题包）。留空表示不覆盖；输入任何合法的值：颜色、字体列表、尺寸、甚至 <code>color-mix()</code>。当前预览模式：${document.documentElement.dataset.theme==='light'?'浅色':'深色'}（顶栏 ☀ 可切换）。</p><div class="ss-token-grid">${rows.map(row).join('')||'<p class="soft">没有匹配的变量。</p>'}</div>`;
}
