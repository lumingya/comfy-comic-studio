...
class ComicRequestHandler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if not self.is_origin_allowed():
            ...
            return
        request_path = urllib.parse.urlparse(self.path).path
        if production_api.dispatch(self,sys.modules[__name__],request_path):return
        if ecosystem_api.dispatch(self,sys.modules[__name__],request_path):return
...
def main():
    ...
    try:
        ...
        ecosystem_api.service(sys.modules[__name__])
    ...
    try:
        ...
        ecosystem_api.service(sys.modules[__name__]).close()
        server.server_close()
