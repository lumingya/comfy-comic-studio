...
from backend.mio_library import FileLibrary, LibraryError, ID, atomic_write, decode, digest, encode, image_type, owned_path
...
class NativeStore(WorkspaceRepository):
    ...
    def image_path(self, url):
        ...
        if p.startswith('/images/production/assets/'):
            name=p.rsplit('/',1)[-1]
            if not re.fullmatch(r'[a-f0-9]{64}\.(png|jpg|jpeg|webp|svg)',name):raise LibraryError('Invalid production image path')
            return owned_path(self.root/'production/assets',name)
        if p.startswith('/images/ecosystem/macros/'):
            name=p.rsplit('/',1)[-1]
            if not re.fullmatch(r'[a-f0-9]{64}\.(png|jpg|jpeg|webp|svg)',name):raise LibraryError('Invalid computed image path')
